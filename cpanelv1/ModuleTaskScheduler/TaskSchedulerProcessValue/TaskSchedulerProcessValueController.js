app.controller("taskSchedulerProcessValueController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$stateParams', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $stateParams, $builder, $state, $filter) {
    ...
    taskScheduleProcessValue.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/ModuleTaskScheduler/taskschedulerProcessvalue/add.html',
                scope: $scope
            });
    ...
    taskScheduleProcessValue.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            taskScheduleProcessValue.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/ModuleTaskScheduler/taskschedulerProcessvalue/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    taskScheduleProcessValue.openPreviewModal = function (ProcessId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcess/', ProcessId, 'GET').success(function (response1) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', engine, 'POST').success(function (response2) {
                ...
                $modal.open({
                    templateUrl: 'cpanelv1/ModuleTaskScheduler/TaskSchedulerProcessValue/preview.html',
                    scope: $scope
                });
            }).error(function (data, errCode, c, d) {
                rashaErManage.checkAction(data, errCode);
