 ...
 serviceTag.LinkCategoryTagIdSelector = {
    ...
    serviceTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"servicetag/getall", serviceTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            serviceTag.gridOptions.fillData(serviceTag.ListItems, response.Access); // Sending Access as an argument
    ...
    serviceTag.addNewCategoryModel = function () {
        serviceTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'serviceCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/service/serviceCategorytag/add.html',
                        scope: $scope
                    });
                    serviceTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    serviceTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'serviceCategorytag/', serviceTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            serviceTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            serviceTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/service/serviceCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    serviceTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'serviceCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    serviceTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'serviceCategorytag/', serviceTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        serviceTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            serviceTag.gridOptions.advancedSearchData.engine.Filters = null;
                            serviceTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    serviceTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'servicetag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            serviceTag.selectedItem = response.Item;
            serviceTag.selectedItem.LinkCategoryTagId = serviceTag.CategoryTagId;
            //serviceTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/service/servicetag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    serviceTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'servicetag/', serviceTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            serviceTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/service/servicetag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    serviceTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                serviceTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"servicetag/", serviceTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    serviceTag.showbusy = false;
                    serviceTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    serviceTag.selectedItemForDelete = response.Item;
                    console.log(serviceTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"servicetag/", serviceTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        serviceTag.categoryBusyIndicator.isActive = false;
                        serviceTag.treeConfig.showbusy = false;
                        serviceTag.showIsBusy = false;
    ...
    serviceTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", serviceTag.fileIdToDelete, 'GET').success(function (response1) {
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    serviceTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    serviceTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/service/servicetag/report.html',
            scope: $scope
        });
    }
