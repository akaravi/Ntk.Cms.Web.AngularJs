app.controller("fileShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $rootScope, $filter) {
    ...
    fileShareServerCategory.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileShareServerCategory/getall", {}, 'POST').success(function (response) {
            ...
            fileShareServerCategory.gridOptions.fillData(fileShareServerCategory.ListItems, response.Access); // Sending Access as an argument
    ...
    fileShareServerCategory.EditCategoryModel = function () {
        ...
        fileShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareMainAdminSetting/', fileShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            fileShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            fileShareServerCategory.selectedItem = response.Item;
    ...
    fileShareServerCategory.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareMainAdminSetting/', fileShareServerCategory.selectedItem, 'POST').success(function (response) {
            fileShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    fileShareServerCategory.EditCategory = function (frm) {
        ...
        fileShareServerCategory.categoryBusyIndicator.isActive = true;
        fileShareServerCategory.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareMainAdminSetting/', fileShareServerCategory.selectedItem, "PUT").success(function (response) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'FileShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'FileShareMainAdminSetting/', fileShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        fileShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    fileShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareServerCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            fileShareServerCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/file/fileShareServerCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    fileShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareServerCategory/', fileShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            fileShareServerCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/file/fileShareServerCategory/edit.html',
                scope: $scope
            });
    ...
    fileShareServerCategory.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareServerCategory/', fileShareServerCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            fileShareServerCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    fileShareServerCategory.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'FileShareServerCategory/', fileShareServerCategory.selectedItem, "PUT").success(function (response) {
            fileShareServerCategory.categoryBusyIndicator.isActive = false;
            fileShareServerCategory.addRequested = false;
            fileShareServerCategory.treeConfig.showbusy = false;
    ...
    fileShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                fileShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"FileShareServerCategory/", fileShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    fileShareServerCategory.showbusy = false;
                    fileShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    fileShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(fileShareServerCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"FileShareServerCategory/", fileShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        fileShareServerCategory.categoryBusyIndicator.isActive = false;
                        fileShareServerCategory.treeConfig.showbusy = false;
                        fileShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    fileShareServerCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/File/fileShareServerCategory/report.html',
            scope: $scope
        });
    }
