app.controller("newsContentParameterTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    newsContentParameterType.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"newsContentParameterType/getall", engine, 'POST').success(function (response) {
            ...
            newsContentParameterType.gridOptions.fillData(newsContentParameterType.ListItems, response.Access);
    ...
    newsContentParameterType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameterType/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsContentParameterType.busyIndicator.isActive = false;
            newsContentParameterType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsContentParameterType/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    newsContentParameterType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameterType/', newsContentParameterType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsContentParameterType.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsContentParameterType/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameterType/', newsContentParameterType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameterType/', newsContentParameterType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    newsContentParameterType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/news/newsContentParameterType/report.html',
            scope: $scope
        });
    }
