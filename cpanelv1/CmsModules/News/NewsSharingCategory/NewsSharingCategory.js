app.controller("newsSharingCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    newssharingcategory.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"newssharingcategory/getall", newssharingcategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            newssharingcategory.gridOptions.Access = response.Access;
    ...
    newssharingcategory.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newssharingcategory/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newssharingcategory.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/newssharingcategory/add.html',
                scope: $scope
            });
    ...
    newssharingcategory.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newssharingcategory/',  newssharingcategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newssharingcategory.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/newssharingcategory/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'newssharingcategory/', newssharingcategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'newssharingcategory/',  newssharingcategory.selectedItemForDelete.Id , 'DELETE').success(function (res) {
    ...
    newssharingcategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/News/newssharingcategory/report.html',
            scope: $scope
        });
    }
