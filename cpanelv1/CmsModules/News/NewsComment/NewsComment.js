app.controller("newsCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    newsComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"newsComment/getall", engine, 'POST').success(function (response) {
            ...
            newsComment.gridOptions.fillData(newsComment.ListItems , response.Access);
    ...
    newsComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsComment.busyIndicator.isActive = false;
            newsComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/NewsComment/add.html',
                scope: $scope
            });
    ...
    newsComment.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsComment/', newsComment.selectedItem, 'POST').success(function (response) {
            newsComment.addRequested = false;
            newsComment.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    newsComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsComment/', newsComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/NewsComment/edit.html',
                scope: $scope
            });
    ...
    newsComment.editRow = function (frm) {
        ...
        newsComment.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'newsComment/', newsComment.selectedItem, "PUT").success(function (response) {
            newsComment.addRequested = true;
            rashaErManage.checkAction(response);
            newsComment.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'newsComment/', newsComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'newsComment/', newsComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    newsComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/News/NewsComment/report.html',
            scope: $scope
        });
    }
