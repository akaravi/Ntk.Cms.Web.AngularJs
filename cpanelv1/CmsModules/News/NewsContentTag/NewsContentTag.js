app.controller("newsContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    newsContentTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"NewsContenttag/getall", newsContentTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            newsContentTag.gridOptions.fillData(newsContentTag.ListItems , response.Access);
    ...
    newsContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/newscontentTag/add.html',
                scope: $scope
            });
    ...
    newsContentTag.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/', newsContentTag.selectedItem , 'POST').success(function (response) {
            newsContentTag.addRequested = false;
            newsContentTag.busyIndicator.isActive = false;
    ...
    newsContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/',  newsContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/newscontentTag/edit.html',
                scope: $scope
            });
    ...
    newsContentTag.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/',  newsContentTag.selectedItem , "PUT").success(function (response) {
            newsContentTag.addRequested = true;
            rashaErManage.checkAction(response);
            newsContentTag.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/',  newsContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'NewsContenttag/',  newsContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
    ...
    newsContentTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/News/newsContentTag/report.html',
            scope: $scope
        });
    }
