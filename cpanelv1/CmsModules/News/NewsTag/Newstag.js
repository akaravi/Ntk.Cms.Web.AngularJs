 ...
 newsTag.LinkCategoryTagIdSelector = {
    ...
    newsTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"newstag/getall", newsTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            newsTag.gridOptions.fillData(newsTag.ListItems, response.Access); // Sending Access as an argument
    ...
    newsTag.addNewCategoryModel = function () {
        newsTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'newsCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/news/newsCategorytag/add.html',
                        scope: $scope
                    });
                    newsTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    newsTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsCategorytag/', newsTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            newsTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            newsTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/news/newsCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    newsTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'newsCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    newsTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'newsCategorytag/', newsTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        newsTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            newsTag.gridOptions.advancedSearchData.engine.Filters = null;
                            newsTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    newsTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newstag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            newsTag.selectedItem = response.Item;
            newsTag.selectedItem.LinkCategoryTagId = newsTag.CategoryTagId;
            //newsTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newstag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    newsTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'newstag/', newsTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            newsTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newstag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    newsTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                newsTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"newstag/", newsTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    newsTag.showbusy = false;
                    newsTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    newsTag.selectedItemForDelete = response.Item;
                    console.log(newsTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"newstag/", newsTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        newsTag.categoryBusyIndicator.isActive = false;
                        newsTag.treeConfig.showbusy = false;
                        newsTag.showIsBusy = false;
    ...
    newsTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", newsTag.fileIdToDelete, 'GET').success(function (response1) {
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    newsTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    newsTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/news/newstag/report.html',
            scope: $scope
        });
    }
