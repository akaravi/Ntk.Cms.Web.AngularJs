app.controller("newsContentParameterController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    newsContentParameter.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsContentParameter/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    newsContentParameter.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/', newsContentParameter.selectedItem, 'POST').success(function (response) {
            newsContentParameter.addRequested = false;
            newsContentParameter.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    newsContentParameter.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/', newsContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsContentParameter/edit.html',
                scope: $scope
            });
    ...
    newsContentParameter.editRow = function (frm) {
        ...
        newsContentParameter.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/', newsContentParameter.selectedItem, "PUT").success(function (response) {
            newsContentParameter.addRequested = true;
            rashaErManage.checkAction(response);
            newsContentParameter.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/', newsContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'newsContentParameter/', newsContentParameter.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    newsContentParameter.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/news/newsContentParameter/report.html',
            scope: $scope
        });
    }
