app.controller("newsShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $rootScope, $filter) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/ViewModel', "", 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/News/NewsShareMainAdminSetting/add.html',
                        scope: $scope
                    });
                    newsShareServerCategory.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    newsShareServerCategory.EditCategoryModel = function () {
        ...
        newsShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/', newsShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            newsShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            newsShareServerCategory.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/News/NewsShareMainAdminSetting/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    newsShareServerCategory.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/', newsShareServerCategory.selectedItem, 'POST').success(function (response) {
            newsShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    newsShareServerCategory.EditCategory = function (frm) {
        ...
        newsShareServerCategory.categoryBusyIndicator.isActive = true;
        newsShareServerCategory.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/', newsShareServerCategory.selectedItem, "PUT").success(function (response) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'NewsShareMainAdminSetting/', newsShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        newsShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    newsShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'newsShareServerCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            newsShareServerCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsShareServerCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    newsShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'newsShareServerCategory/', newsShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            newsShareServerCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/news/newsShareServerCategory/edit.html',
                scope: $scope
            });
    ...
    newsShareServerCategory.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsShareServerCategory/', newsShareServerCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            newsShareServerCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    newsShareServerCategory.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'newsShareServerCategory/', newsShareServerCategory.selectedItem, "PUT").success(function (response) {
            newsShareServerCategory.categoryBusyIndicator.isActive = false;
            newsShareServerCategory.addRequested = false;
            newsShareServerCategory.treeConfig.showbusy = false;
    ...
    newsShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                newsShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"newsShareServerCategory/", newsShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    newsShareServerCategory.showbusy = false;
                    newsShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    newsShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(newsShareServerCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"newsShareServerCategory/", newsShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        newsShareServerCategory.categoryBusyIndicator.isActive = false;
                        newsShareServerCategory.treeConfig.showbusy = false;
                        newsShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    newsShareServerCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/News/newsShareServerCategory/report.html',
            scope: $scope
        });
    }
