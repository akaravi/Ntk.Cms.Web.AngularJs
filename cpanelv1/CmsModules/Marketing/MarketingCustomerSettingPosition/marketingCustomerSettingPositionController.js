app.controller("marketingCustomerSettingPositionController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Marketing/MarketingCustomerSettingPosition/add.html',
            scope: $scope
        });
    }
    ...
    $scope.OpenSpecificationsCustomer=function(Customer) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Marketing/MarketingCustomerSettingPosition/CustomerView.html',
            scope: $scope
        });
    }
    $scope.OpenSettingPosition=function(positionId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MarketingCustomerSettingPosition/", positionId, 'GET').success(function (response) {
   ...
   $scope.selectCustomer=function(customerId)
    {
        ajax.call(cmsServerConfig.configApiServerPath+"MarketingCustomer/", customerId, 'GET').success(function (response) {
            marketingPosition.positionList=response.Item;
           $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Marketing/MarketingCustomerSettingPosition/selectCustomer.html',
                scope: $scope
            });     
        });
    }
    ...
    {
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Marketing/MarketingCustomerSettingPosition/AddActivity.html',
                scope: $scope
            });
        console.log('test');
    }
