app.controller("cmsUserBadLoginController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmdUserBadLogin.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUserBadLogin/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserBadLogin/add.html',
                scope: $scope
            });
    ...
    cmdUserBadLogin.openEditModal=function() {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUserBadLogin/', cmdUserBadLogin.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            ...
            cmdUserBadLogin.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserBadLogin/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreUserBadLogin/', cmdUserBadLogin.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreUserBadLogin/', cmdUserBadLogin.selectedItemForDelete.Id , 'DELETE').success(function (res) {
...
cmdUserBadLogin.gridOptions = {
   ...
   cmdUserBadLogin.toggleExportForm = function () {
       ...
       $modal.open({
           templateUrl: 'cpanelv1/CmsModules/Core/CmsUserBadLogin/report.html',
           scope: $scope
       });
   }
