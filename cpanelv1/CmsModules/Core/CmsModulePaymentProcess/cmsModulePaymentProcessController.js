app.controller("cmsModulePaymentProcessCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $filter) {
    ...
    cmsMdlPayPrc.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcess/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcess/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsMdlPayPrc.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcess/', cmsMdlPayPrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsMdlPayPrc.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcess/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcess/', cmsMdlPayPrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcess/', cmsMdlPayPrc.selectedItemForDelete.Id, 'DELETE').success(function (res) {
