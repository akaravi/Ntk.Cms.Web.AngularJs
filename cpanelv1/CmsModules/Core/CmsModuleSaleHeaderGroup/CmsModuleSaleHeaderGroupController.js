app.controller("CmsModuleSaleHeaderGroupController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    CmsModuleHeaderG.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreModuleSaleHeaderGroup/getall", engine, 'POST').success(function (response) {
            ...
            CmsModuleHeaderG.gridOptions.fillData(CmsModuleHeaderG.ListItems , response.Access);
    ...
    CmsModuleHeaderG.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleSaleHeaderGroup/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            CmsModuleHeaderG.busyIndicator.isActive = false;
            CmsModuleHeaderG.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsModuleSaleHeaderGroup/add.html',
                scope: $scope
            });
    ...
    CmsModuleHeaderG.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleSaleHeaderGroup/', CmsModuleHeaderG.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            CmsModuleHeaderG.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsModuleSaleHeaderGroup/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleSaleHeaderGroup/', CmsModuleHeaderG.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleSaleHeaderGroup/', CmsModuleHeaderG.selectedItemForDelete.Id, 'DELETE').success(function (res) {
 ...
 CmsModuleHeaderG.LinkCmsSiteCategoryIdSelector = {
    ...
    CmsModuleHeaderG.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/CmsModuleSaleHeaderGroup/report.html',
            scope: $scope
        });
    }
