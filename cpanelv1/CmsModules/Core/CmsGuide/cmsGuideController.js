   ...
   cmsGuide.summernoteOptions = {
    ...
    cmsGuide.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreGuide/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsGuide/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsGuide.openEditContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreGuide/', cmsGuide.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsGuide.filePickerFiles.fileId = null;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsGuide/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreGuide/', cmsGuide.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreGuide/', cmsGuide.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsGuide.deleteAttachedfieldName = function(index) {
      ajax
        .call(
          cmsServerConfig.configApiServerPath+"CoreGuide/",
          cmsGuide.contractsList[index].Id,
          "DELETE"
        )
