app.controller("cmsModuleOptimizerGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsModuleOptimizer.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'cmsModuleOptimizer/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModuleOptimizer/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsModuleOptimizer.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'cmsModuleOptimizer/', cmsModuleOptimizer.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsModuleOptimizer.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModuleOptimizer/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'cmsModuleOptimizer/', cmsModuleOptimizer.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'cmsModuleOptimizer/', cmsModuleOptimizer.selectedItemForDelete.Id, 'DELETE').success(function (res) {
