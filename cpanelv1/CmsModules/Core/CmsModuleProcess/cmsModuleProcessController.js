app.controller("cmsModuleProcessGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $filter) {
    ...
    cmsModulePrc.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleProcess/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModuleProcess/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsModulePrc.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleProcess/', cmsModulePrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsModulePrc.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModuleProcess/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleProcess/', cmsModulePrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreModuleProcess/', cmsModulePrc.selectedItemForDelete.Id, 'DELETE').success(function (res) {
