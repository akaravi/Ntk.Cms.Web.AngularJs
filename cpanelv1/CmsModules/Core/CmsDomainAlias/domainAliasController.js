app.controller("domainAliasController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$rootScope', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $rootScope, $window, $filter) {
    ...
    domainAlias.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreSiteDomainAlias/getall", domainAlias.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            domainAlias.gridOptions.fillData(domainAlias.ListItems, response.Access);
    ...
    domainAlias.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteDomainAlias/ViewModel', "", 'GET').success(function (response) {
            ...
            domainAlias.addRequested = false;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsDomainAlias/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    domainAlias.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteDomainAlias/', domainAlias.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            domainAlias.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsDomainAlias/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteDomainAlias/', domainAlias.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteDomainAlias/', domainAlias.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    domainAlias.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/CmsDomainAlias/report.html',
            scope: $scope
        });
    }
