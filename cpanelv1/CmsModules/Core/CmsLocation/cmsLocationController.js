app.controller("cmsLocationController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    cmsLocation.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreLocation/getall", cmsLocation.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"CoreEnum/EnumLocationType", "", 'GET').success(function (response) {
    ...
    cmsLocation.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreLocation/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsLocation/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsLocation.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreLocation/', cmsLocation.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsLocation/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreLocation/', cmsLocation.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreLocation/', cmsLocation.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsLocation.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/CmsLocation/report.html',
            scope: $scope
        });
    }
