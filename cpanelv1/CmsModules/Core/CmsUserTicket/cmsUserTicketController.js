app.controller("cmsUserTicketGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsUserTicketgrd.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUser/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicket/add.html',
                scope: $scope
            });
    ...
    cmsUserTicketgrd.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUser/', cmsUserTicketgrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsUserTicketgrd.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicket/edit.html',
                scope: $scope
            });
    ...
    cmsUserTicketgrd.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicketg/report.html',
            scope: $scope
        });
    }
