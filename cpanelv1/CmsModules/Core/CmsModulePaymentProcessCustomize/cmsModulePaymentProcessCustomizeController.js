app.controller("cmsModulePaymentProcessCustomizeCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$stateParams', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $stateParams, $state, $filter) {
    ...
    cmsMdlPayPrcCust.init = function () {
        ...
        var filterModel = {
            ...
            SearchType: 0,
            value: cmsMdlPayPrcCust.selectedModulePaymentProcess.Id
        };
        cmsMdlPayPrcCust.gridOptions.advancedSearchData.engine.Filters.push(filterModel);
        cmsMdlPayPrcCust.gridOptions.advancedSearchData.engine.AccessLoad = true;
    ...
    cmsMdlPayPrcCust.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcessCustomize/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcessCustomize/add.html',
                scope: $scope
            });
    ...
    cmsMdlPayPrcCust.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcessCustomize/', cmsMdlPayPrcCust.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcessCustomize/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            cmsMdlPayPrcCust.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcessCustomize/', cmsMdlPayPrcCust.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CmsModulePaymentProcessCustomize/', cmsMdlPayPrcCust.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsMdlPayPrcCust.showFormMainAdmin = function (preview, selectedId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcessCustomize/formMainAdmin.html',
            scope: $scope
        });
    }
    ...
    cmsMdlPayPrcCust.showFormEndUser = function (preview, selectedId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcessCustomize/formEndUser.html',
            scope: $scope
        });
    }
    // Save Input Value Form
    ...
    cmsMdlPayPrcCust.showFormSiteAdmin = function (preview, selectedId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsModulePaymentProcessCustomize/formSiteAdmin.html',
            scope: $scope
        });
    }
    // Save Input Value Form
