app.controller("cmsUserTicketLogController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsUserTicketLog.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreTokenUserLog/getall", engine, 'POST').success(function (response) {
            ...
            cmsUserTicketLog.gridOptions.fillData(cmsUserTicketLog.ListItems , response.Access);
    ...
    cmsUserTicketLog.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUserLog/ViewModel', '', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsUserTicketLog.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicketLog/add.html',
                scope: $scope
            });
    ...
    cmsUserTicketLog.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUserLog/', cmsUserTicketLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsUserTicketLog.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicketLog/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUserLog/', cmsUserTicketLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreTokenUserLog/', cmsUserTicketLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsUserTicketLog.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsUserTicketLog/report.html',
            scope: $scope
        });
    }
