app.controller("cmsModulePageDependencyGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsModulePageDependencygrd.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'WebDesignerMainPageDependency/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsModulePageDependency/add.html',
                scope: $scope
            });
    ...
    cmsModulePageDependencygrd.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'WebDesignerMainPageDependency/', cmsModulePageDependencygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsModulePageDependencygrd.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/CmsModulePageDependency/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'WebDesignerMainPageDependency/', cmsModulePageDependencygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'WebDesignerMainPageDependency/', cmsModulePageDependencygrd.selectedItemForDelete.Id, 'DELETE').success(function (res) {
