app.controller("cmsUserGroupGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsUserGroupgrd.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreEnum/enumAccessControllerTypes", "", 'GET').success(function (response1) {
    ...
    cmsUserGroupgrd.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUserGroup/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserGroup/add.html',
                scope: $scope
            });
    ...
    cmsUserGroupgrd.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUserGroup/', cmsUserGroupgrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            cmsUserGroupgrd.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsUserGroup/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreUserGroup/', cmsUserGroupgrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreUserGroup/', cmsUserGroupgrd.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsUserGroupgrd.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/CmsUserGroup/report.html',
            scope: $scope
        });
    }
