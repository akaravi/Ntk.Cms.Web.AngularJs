app.controller("cmsSiteCategoryGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsSiteCategorygrd.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreSiteCategory/getall", cmsSiteCategorygrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            cmsSiteCategorygrd.gridOptions.fillData(cmsSiteCategorygrd.ListItems , response.Access);
    ...
    cmsSiteCategorygrd.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteCategory/ViewModel', '', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteCategorygrd.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsSiteCategory/add.html',
                scope: $scope
            });
    ...
    cmsSiteCategorygrd.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteCategory/', cmsSiteCategorygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteCategorygrd.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Core/cmsSiteCategory/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteCategory/', cmsSiteCategorygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreSiteCategory/', cmsSiteCategorygrd.selectedItemForDelete.Id, 'DELETE').success(function (res) {
