...
cmsLog.listforDel=listforDel;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                console.log(cmsLog.gridOptions.selectedRow.item);
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreLogError/DeleteList',cmsLog.listforDel.Id , 'DELETE').success(function (res) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'CoreLogError/', cmsLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'CoreLogError/', cmsLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        rashaErManage.checkAction(res);
                        if (res.IsSuccess) {
                            cmsLog.replaceItem(cmsLog.selectedItemForDelete.Id);
                            cmsLog.gridOptions.fillData(cmsLog.ListItems);
    ...
    cmsLog.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Core/cmsLog/report.html',
            scope: $scope
        });
    }
