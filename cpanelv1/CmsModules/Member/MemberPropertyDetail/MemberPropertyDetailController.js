...
memberPropertyDetail.LinkCategoryIdSelector = {
    ...
    memberPropertyDetail.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MemberPropertyDetail/EnumUiPropertyDesign", "", 'GET').success(function (response) {
            memberPropertyDetail.UiDesignType = response.ListItems;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
        // ajax.call(cmsServerConfig.configApiServerPath+"MemberPropertyDetail/EnumInputDataType", "", 'GET').success(function (response) {
        ...
        var filterValue = {
            ...
            value: parseInt(memberPropertyDetail.propertyTypeId),
            SearchType: 0
        }
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MemberPropertyDetailGroup/getall", memberPropertyDetail.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            memberPropertyDetail.groupResultAccess = response.Access;
    ...
    memberPropertyDetail.openAddCategoryModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            memberPropertyDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberPropertyDetailGroup/add.html',
                scope: $scope
            });
    ...
    memberPropertyDetail.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/', memberPropertyDetail.selectedItem, 'POST').success(function (response) {
            memberPropertyDetail.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    memberPropertyDetail.openEditCategoryModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/', memberPropertyDetail.treeConfig.currentNode.Id, 'GET').success(function (response1) {
            memberPropertyDetail.showbusy = false;
            rashaErManage.checkAction(response1);
            memberPropertyDetail.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberPropertyDetailGroup/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberPropertyDetail.editCategory = function (frm) {
        ...
        memberPropertyDetail.categorybusyIndicator.isActive = true;
        memberPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/', memberPropertyDetail.selectedItem, "PUT").success(function (response) {
            memberPropertyDetail.addRequested = true;
    ...
    memberPropertyDetail.deleteCategory = function () {
        ...
        if (memberPropertyDetail.ListItems.length == 0) {
            rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
                if (isConfirmed) {
                    memberPropertyDetail.categorybusyIndicator.isActive = true;
                    ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/', node.Id, 'GET').success(function (response) {
                        rashaErManage.checkAction(response);
                        memberPropertyDetail.selectedItemForDelete = response.Item;
                        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetailGroup/', memberPropertyDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    memberPropertyDetail.selectContent = function (node) {
        ...
        var filterValue2 = {
            PropertyName: "LinkPropertyTypeId",
            value: memberPropertyDetail.propertyTypeId,
            SearchType: 0
        }
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"memberPropertyDetail/getall", memberPropertyDetail.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            memberPropertyDetail.gridOptions.fillData(memberPropertyDetail.ListItems, response.Access);
    ...
    memberPropertyDetail.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            memberPropertyDetail.selectedItem = response.Item;
            memberPropertyDetail.selectedItem.LinkPropertyDetailGroupId = node.Id;
            memberPropertyDetail.onMultipleChoiceChange();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/memberPropertyDetail/add.html',
                scope: $scope
            });
    ...
    memberPropertyDetail.addNewContent = function (frm) {
        ...
        memberPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetail/', memberPropertyDetail.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                // filter text
    ...
    memberPropertyDetail.openEditContentModal = function () {
        ...
        memberPropertyDetail.FieldName = "";
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetail/', memberPropertyDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            memberPropertyDetail.selectedItem = response1.Item;
            ...
            memberPropertyDetail.onMultipleChoiceChange();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/memberPropertyDetail/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberPropertyDetail.editContent = function (frm) {
        ...
        memberPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        memberPropertyDetail.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetail/', memberPropertyDetail.selectedItem, "PUT").success(function (response) {
            // memberPropertyDetail.addRequested = false;
            memberPropertyDetail.treeConfig.showbusy = false;
            memberPropertyDetail.showIsBusy = false;
            rashaErManage.checkAction(response);
    ...
    memberPropertyDetail.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                memberPropertyDetail.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"memberPropertyDetail/", memberPropertyDetail.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    memberPropertyDetail.showbusy = false;
                    memberPropertyDetail.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    memberPropertyDetail.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"memberPropertyDetail/", memberPropertyDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        memberPropertyDetail.treeConfig.showbusy = false;
                        memberPropertyDetail.showIsBusy = false;
    ...
    memberPropertyDetail.editStepGoDown = function (item, index) {
        ...
        memberPropertyDetail.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetail/', item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            memberPropertyDetail.selectedItem = response1.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetail/', memberPropertyDetail.selectedItem, "PUT").success(function (response2) {
                rashaErManage.checkAction(response2);
                if (response2.IsSuccess) {
                    ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetail/', memberPropertyDetail.ListItems[index + 1].Id, 'GET').success(function (response3) {
                        ...
                        ajax.call(cmsServerConfig.configApiServerPath+'MemberPropertyDetail/', memberPropertyDetail.selectedItem, "PUT").success(function (response4) {
    ...
    memberPropertyDetail.editStepGoDownGroup = function (item) {
        ...
        memberPropertyDetail.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetailGroup/', item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            memberPropertyDetail.selectedItem = response1.Item;
            var temp = response1.Item.ShowInFormOrder;
            memberPropertyDetail.selectedItem.ShowInFormOrder = memberPropertyDetail.treeConfig.Items[index + 1].ShowInFormOrder;
            ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetailGroup/', memberPropertyDetail.selectedItem, "PUT").success(function (response2) {
                rashaErManage.checkAction(response2);
                if (response2.IsSuccess) {
                    ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetailGroup/', memberPropertyDetail.treeConfig.Items[index + 1].Id, 'GET').success(function (response3) {
                        rashaErManage.checkAction(response3);
                        memberPropertyDetail.selectedItem = response3.Item;
                        memberPropertyDetail.selectedItem.ShowInFormOrder = temp;
                        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyDetailGroup/', memberPropertyDetail.selectedItem, "PUT").success(function (response4) {
                            rashaErManage.checkAction(response4);
                            if (response4.IsSuccess) {
    ...
    memberPropertyDetail.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Member/MemberPropertyDetail/report.html',
            scope: $scope
        });
    }
