...
// Open Add Modal property
    memberUser.openAddPropertyModal = function (PropertyTypeId,UserId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/member/MemberUser/addValue.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
// Open Add Modal History
   /* memberUser.openAddModalHistory = function () {
      ...
      //if (buttonIsPressed) return;
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItemhistory = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/addhistory.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
/* memberUser.addNewContenthistory = function (frm) {
       ...
       /* if (memberUser.selectedItemhistory.LinkPropertyId ==null) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberUser.selectedItemhistory, 'POST').success(function (response) {
            //memberUser.addRequested = false;
            //memberUser.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
...
// Open Edit Modal History
   /* memberUser.openEditModelHistory = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberUser.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            memberUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItemhistory = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/edithistory.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
/* memberUser.editNewContenthistory = function (frm) {
        ...
        //memberUser.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberUser.selectedItemhistory, "PUT").success(function (response) {
            //memberUser.addRequested = true;
            rashaErManage.checkAction(response);
            //memberUser.busyIndicator.isActive = false;
            if (response.IsSuccess) {
...
/* memberUser.deleteContentHistory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberUser.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberUser.selectedItemForDelete, 'POST').success(function (res) {
  ...
  /*  memberUser.addRowValueProperty = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/', memberUser.selectedItemValue, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                memberUser.closeModal();
                ajax.call(cmsServerConfig.configApiServerPath+"memberpropertydetailvalue/ViewModel", "", 'GET').success(function (response1) {
    ...
    memberUser.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Member/MemberUser/modalMenu.html",
            scope: $scope
        });
    }
    //#help//download file
...
//memberUser.showlistHistory=false;
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/getall", engine, "POST").success(function (response) {
            ...
            rashaErManage.checkAction(response);
            memberUser.gridOptionsfile.fillData(memberUser.listComments, response.Access);
    ...
    memberUser.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"memberuser/getall", memberUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            memberUser.gridOptions.fillData(memberUser.ListItems, response.Access); // Sending Access as an argument
    ...
    memberUser.addNewCategoryModel = function () {
        ...
        memberUser.addRequested = false;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberUser.openEditCategoryModel = function () {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/', memberUser.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            memberUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberUser.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/', memberUser.selectedItem, 'POST').success(function (response) {
            memberUser.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    memberUser.editCategory = function (frm) {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/', memberUser.selectedItem, "PUT").success(function (response) {
            memberUser.addRequested = true;
    ...
    memberUser.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                memberUser.categoryBusyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberUser.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'MemberGroup/', memberUser.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        if (res.IsSuccess) {
                            memberUser.gridOptions.advancedSearchData.engine.Filters = null;
                            memberUser.gridOptions.advancedSearchData.engine.Filters = [];
                            memberUser.gridOptions.fillData();
    ...
    memberUser.openAddModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberuser/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItem = response.Item;
            if (node.Id != 0 || node.Id) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/addUser.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
//#help فرم اتصال عضو به سایت
    memberUser.openAddMemberinSiteModal= function () {
         ...
         $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/addUserinSite.html',
                scope: $scope
            });
    }
...
//#help اتصال عضو به سایت
    ...
    memberUser.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberuser/', memberUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            memberUser.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/editUser.html',
                scope: $scope
            });
    ...
    memberUser.addNewContent = function (frm) {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        memberUser.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberuser/', memberUser.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            memberUser.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
                memberUser.ListItems.unshift(response.Item);
                ...
                memberUser.closeModal();
                //Save inputTags
                //ajax.call(cmsServerConfig.configApiServerPath+'memberusergroup/', memberUser.selectedItem.ContentTags, 'POST').success(function (response) {
    ...
    memberUser.editContent = function (frm) {
        ...
        memberUser.addRequested = true;
        memberUser.categoryBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberuser/', memberUser.selectedItem, "PUT").success(function (response) {
            memberUser.addRequested = false;
            memberUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    memberUser.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'memberUser/', memberUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberUser.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'memberUser/', memberUser.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        //memberUser.busyIndicator.isActive = false;
                        rashaErManage.checkAction(res);
                        if (res.IsSuccess) {
                            memberUser.replaceItem(memberUser.selectedItemForDelete.Id);
    ...
    memberUser.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    memberUser.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", memberUser.fileIdToDelete, 'GET').success(function (response1) {
    ...
    memberUser.saveNewFile = function () {
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", memberUser.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    memberUser.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
   ...
   //Export Report 
    ...
    memberUser.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Member/MemberUser/report.html',
            scope: $scope
        });
    }
