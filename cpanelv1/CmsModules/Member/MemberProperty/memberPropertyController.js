...
///#help اضافه کردن property 
        ...
        memberProperty.openAddPropertyinSiteModal= function () {
                ...
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/Member/MemberProperty/addPropertyinSite.html',
                    scope: $scope
                });
        }
...
// Open Add Modal History
 ...
 if (buttonIsPressed) return;
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/member/memberProperty/addhistory.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
//  Add History
 memberProperty.addNewContenthistory = function (frm) {
       ...
       /* if (memberProperty.selectedItemhistory.LinkPropertyId ==null) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberProperty.selectedItemhistory, 'POST').success(function (response) {
            //memberProperty.addRequested = false;
            //memberProperty.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                    ...
                    for (var i = 0; i < memberProperty.propertyDetailsListItems.length; i++) {
                        ...
                        if (memberProperty.propertyDetailsListItems[i].DefaultValue != null) {
                            if (memberProperty.propertyDetailsListItems[i].DefaultValue.multipleChoice) {
                                ...
                                valueItem.Value = memberProperty.selectionValueNames;
                                if(valueItem.Value){
...
// Open Edit Modal History
    memberProperty.openEditModelHistory = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberProperty.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            ...
            //#help
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/memberProperty/edithistory.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
//  Edit History
 memberProperty.editNewContenthistory = function (frm) {
        ...
        //memberProperty.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberProperty.selectedItemhistory, "PUT").success(function (response) {
            //memberProperty.addRequested = true;
            rashaErManage.checkAction(response);
            //memberProperty.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberProperty.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'MemberHistory/', memberProperty.selectedItemForDelete.Id, 'DELETE').success(function (res) {
...
memberProperty.PropertyID=PropertyId;
 ajax.call(cmsServerConfig.configApiServerPath+"MemberHistory/getall",engine2, 'POST').success(function (response1) {
            ...
            rashaErManage.checkAction(response1);
            memberProperty.gridOptionsHistory.fillData(memberProperty.listHistorys, response1.Access);
    ...
    memberProperty.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Member/memberPropertyType/modalMenu.html",
            scope: $scope
        });
    }
    //#help//download file
    ...
    memberProperty.showlistfile = function (sItem) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/getall", engine, "POST").success(function (response) {
            ...
            rashaErManage.checkAction(response);
            memberProperty.gridOptionsfile.fillData(memberProperty.listComments, response.Access);
    ...
    if (memberProperty.selectedPublicConfig.member!= null || memberProperty.selectedPublicConfig.member != undefined)
    {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MemberProperty/getall", engine, 'POST').success(function (response) {
            ...
            memberProperty.ListItems = response.ListItems;
            memberProperty.gridOptions.fillData(memberProperty.ListItems, response.Access); // Sending Access as an argument
...
////////////////////////////////////////////////////////////////
    ...
    /*memberProperty.addNewCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/memberPropertyType/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
   ...
   /* memberProperty.openEditCategoryModel = function () {
        ...
        memberProperty.categoryBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/', memberProperty.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            memberProperty.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            memberProperty.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Member/memberPropertyType/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    /*memberProperty.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/', memberProperty.selectedItem, 'POST').success(function (response) {
            memberProperty.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    /*memberProperty.editCategory = function (frm) {
        ...
        memberProperty.categoryBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/', memberProperty.selectedItem, "PUT").success(function (response) {
            memberProperty.addRequested = true;
    ...
    /*memberProperty.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                memberProperty.categoryBusyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberProperty.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'memberPropertyType/', memberProperty.selectedItemForDelete, 'POST').success(function (res) {
                        if (res.IsSuccess) {
                            memberProperty.gridOptions.advancedSearchData.engine.Filters = null;
                            memberProperty.gridOptions.advancedSearchData.engine.Filters = [];
                            memberProperty.gridOptions.fillData();
...
*/
    ...
    memberProperty.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/member/memberProperty/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberProperty.addNewRow = function (frm) {
        ...
        var valueItem = {};
        memberProperty.valueItems = [];
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/', memberProperty.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                memberProperty.closeModal();
                ajax.call(cmsServerConfig.configApiServerPath+"memberpropertydetailvalue/ViewModel", "", 'GET').success(function (response1) {
    ...
    memberProperty.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/', parseInt(memberProperty.gridOptions.selectedRow.item.Id), 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberProperty.selectedItem = response.Item;
            ...
            //*****************************************************************
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/member/memberProperty/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberProperty.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/', memberProperty.selectedItem, "PUT").success(function (response) {
            memberProperty.addRequested = true;
            rashaErManage.checkAction(response);
            //memberProperty.busyIndicator.isActive = false;
            if (response.IsSuccess) {
            ...
            for (var i = 0; i < memberProperty.propertyDetailsListItems.length; i++) {
                ...
                for (var j = 0; j < memberProperty.propertyDetailValuesListItems.length; j++) {
                    if (memberProperty.propertyDetailsListItems[i].Id == memberProperty.propertyDetailValuesListItems[j].LinkPropertyDetailId) {
                        ...
                        if (memberProperty.propertyDetailsListItems[i].DefaultValue.multipleChoice == false) { // Detail is not CheckBox
                            ...
                            var checkboxName = "selection" + memberProperty.propertyDetailsListItems[i].Id;
                            memberProperty.propertyDetailValuesListItems[j].Value =JSON.stringify( memberProperty[checkboxName]);
                        }
                    }
                }
                if (!memberProperty.propertyDetailsListItems[i].valueFound) {
                    ...
                    if (memberProperty.propertyDetailsListItems[i].DefaultValue.multipleChoice == false) { // Detail is not CheckBox
                        ...
                        var checkboxName = "selection" + memberProperty.propertyDetailsListItems[i].Id;
                        var newValue={ Id: 0, LinkPropertyId: memberProperty.selectedItem.Id, LinkPropertyDetailId: memberProperty.propertyDetailsListItems[i].Id, Value: memberProperty[checkboxName] }
                        ...
                        memberProperty.propertyDetailValuesListItems.push(newValue);
                    }
                }
            }
   ...
   memberProperty.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                //memberProperty.busyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'memberproperty/', memberProperty.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberProperty.selectedItemForDelete = response.Item;
                            ...
                            if (memberProperty.propertyDetailValuesListItems[j].Value != null) {
                                if (jsonDefaultValue != undefined && jsonDefaultValue != null && jsonDefaultValue.nameValue != undefined && jsonDefaultValue.nameValue != null && 0 < jsonDefaultValue.nameValue.length) {
                                    if (jsonDefaultValue.multipleChoice) {   // Detail is CheckBox
                                        var multipleValues =JSON.parse( memberProperty.propertyDetailValuesListItems[j].Value);//.split(',');
...
function stringfyLinkFileIds(arrayOfFiles) {
 ...
 // Filter Texts for CmsUser
    ...
    memberProperty.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Member/memberPropertyType/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    memberProperty.saveNewFile = function () {
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", memberProperty.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
   ...
   //Export Report 
    ...
    memberProperty.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Member/MemberProperty/report.html',
            scope: $scope
        });
    }
