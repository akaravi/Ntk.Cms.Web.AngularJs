app.controller("reservationCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    reservationComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"reservationComment/getall", engine, 'POST').success(function (response) {
            ...
            reservationComment.gridOptions.fillData(reservationComment.ListItems , response.Access);
    ...
    reservationComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            reservationComment.busyIndicator.isActive = false;
            reservationComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationComment/add.html',
                scope: $scope
            });
    ...
    reservationComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationComment/', reservationComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            reservationComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'reservationComment/', reservationComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'reservationComment/', reservationComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    reservationComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/reservation/reservationComment/report.html',
            scope: $scope
        });
    }
