 ...
 reservationTag.LinkCategoryTagIdSelector = {
    ...
    reservationTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"reservationtag/getall", reservationTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            reservationTag.gridOptions.fillData(reservationTag.ListItems, response.Access); // Sending Access as an argument
    ...
    reservationTag.addNewCategoryModel = function () {
        reservationTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'reservationCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/reservation/reservationCategorytag/add.html',
                        scope: $scope
                    });
                    reservationTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    reservationTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationCategorytag/', reservationTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            reservationTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            reservationTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/reservation/reservationCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    reservationTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'reservationCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    reservationTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'reservationCategorytag/', reservationTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        reservationTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            reservationTag.gridOptions.advancedSearchData.engine.Filters = null;
                            reservationTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    reservationTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationtag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            reservationTag.selectedItem = response.Item;
            reservationTag.selectedItem.LinkCategoryTagId = reservationTag.CategoryTagId;
            //reservationTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationtag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    reservationTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'reservationtag/', reservationTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            reservationTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationtag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    reservationTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                reservationTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"reservationtag/", reservationTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    reservationTag.showbusy = false;
                    reservationTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    reservationTag.selectedItemForDelete = response.Item;
                    console.log(reservationTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"reservationtag/", reservationTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        reservationTag.categoryBusyIndicator.isActive = false;
                        reservationTag.treeConfig.showbusy = false;
                        reservationTag.showIsBusy = false;
    ...
    reservationTag.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/reservation/reservationContent/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    reservationTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", reservationTag.fileIdToDelete, 'GET').success(function (response1) {
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    reservationTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    reservationTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/reservation/reservationtag/report.html',
            scope: $scope
        });
    }
