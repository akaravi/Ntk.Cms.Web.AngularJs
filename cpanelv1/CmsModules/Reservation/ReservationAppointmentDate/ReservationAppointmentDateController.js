app.controller("reservationAppDateController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $state, $filter) {
    ...
    appDate.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ReservationAppointmentDate/getall", appDate.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            appDate.gridOptions.fillData(appDate.ListItemsDate, response.Access);
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ReservationAppointmentDateDetail/getall", appDate.gridContentDateDetail.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            // Call Excerpt Function to shorten the length of long strings
            appDate.gridContentDateDetail.fillData(appDate.ListItemsDateDetail, response.Access);
    ...
    appDate.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDate/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            appDate.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    appDate.openAddModalDetail = function () {
        appDate.modalTitle = 'اضافه';
        ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDateDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            appDate.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/addDetail.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    appDate.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDate/', appDate.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            appDate.selectedItem = response.Item;
            ...
            if (response.Item.LinkFileReportId != null) {
                ajax.call(cmsServerConfig.configApiServerPath+'FileContent/', response.Item.LinkFileReportId, 'GET').success(function (response2) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/edit.html',
                scope: $scope
            });
    ...
    appDate.openEditModaldetail = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDateDetail/', appDate.gridContentDateDetail.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            appDate.selectedItemDetail = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/editDetail.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDateDetail/', appDate.gridContentDateDetail.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDateDetail/', appDate.selectedItemForDelete.Id, 'DELETE').success(function (res) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDate/', appDate.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    appDate.selectedItemForDelete = response.Item;
                    console.log(appDate.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'ReservationAppointmentDate/', appDate.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        rashaErManage.checkAction(res);
                        appDate.busyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    appDate.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/setTimes.html',
            scope: $scope
        });
    ...
    appDate.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    appDate.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", appDate.fileIdToDelete, 'GET').success(function (response1) {
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (appDate.fileIsExist(uploadFile.name)) { // File already exists
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    appDate.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    appDate.toggleExportFormDetail = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDateDetail/report.html',
            scope: $scope
        });
    }
    ...
    appDate.toggleExportForm = function () {
        ...
        appDate.exportDownloadLink = null;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/ReservationAppointmentDate/report.html',
            scope: $scope
        });
    }
    //Row Count Export Input Change
