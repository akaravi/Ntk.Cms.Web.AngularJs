app.controller("reservationplaceController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$stateParams', '$filter', '$rootScope', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $stateParams, $filter, $rootScope) {
    ...
    place.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"Reservationplace/getall", place.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            place.gridOptions.fillData(place.ListItems, response.Access);
    ...
    place.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'Reservationplace/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            place.busyIndicator.isActive = false;
            place.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/Reservationplace/add.html',
                scope: $scope
            });
    ...
    place.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'Reservationplace/', place.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            place.selectedItem = response.Item;
            place.filePickerMainImage.filename = null;
            place.filePickerMainImage.fileId = null;
            if (response.Item.LinkBackgroundImageId != null) {
                ajax.call(cmsServerConfig.configApiServerPath+'FileContent/', response.Item.LinkBackgroundImageId, 'GET').success(function (response2) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Reservation/Reservationplace/edit.html',
                scope: $scope
            });
    ...
    place.DesignPlace = function (item) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'Reservationplace/', item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            place.selectedItem = response.Item;
            place.filePickerMainImage.filename = null;
            place.filePickerMainImage.fileId = null;
    ...
    place.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/Reservationplace/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (place.fileIsExist(uploadFile.name)) { // File already exists
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    place.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    place.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Reservation/Reservationplace/report.html',
            scope: $scope
        });
    }
