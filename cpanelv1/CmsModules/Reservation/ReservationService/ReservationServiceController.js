app.controller("reservationServiceController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window','$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window,$state, $filter) {
    ...
    reservationService.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationservice/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationservice/add.html',
                scope: $scope
            });
    ...
    reservationService.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ReservationService/', reservationService.selectedItem, 'POST').success(function (response) {
            reservationService.addRequested = false;
            reservationService.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    reservationService.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationservice/', reservationService.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            reservationService.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationservice/edit.html',
                scope: $scope
            });
    ...
    reservationService.editRow = function (frm) {
        ...
        reservationService.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'reservationservice/', reservationService.selectedItem, "PUT").success(function (response) {
            reservationService.addRequested = true;
            rashaErManage.checkAction(response);
            reservationService.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'reservationservice/', reservationService.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'reservationservice/', reservationService.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    reservationService.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/reservation/reservationservice/report.html',
            scope: $scope
        });
    }
