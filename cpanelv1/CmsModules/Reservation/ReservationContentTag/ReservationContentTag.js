app.controller("reservationContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    reservationContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationContenttag/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationcontentTag/add.html',
                scope: $scope
            });
    ...
    reservationContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'reservationContenttag/',  reservationContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            ...
            reservationContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/reservation/reservationcontentTag/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'reservationContenttag/',  reservationContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'reservationContenttag/',  reservationContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
    ...
    reservationContentTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/reservation/reservationContentTag/report.html',
            scope: $scope
        });
    }
