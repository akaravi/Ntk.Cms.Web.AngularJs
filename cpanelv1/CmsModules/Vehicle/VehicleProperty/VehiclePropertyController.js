app.controller("vehiclePropertyController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    vehicleProperty.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"vehicleproperty/getAllwithalias", vehicleProperty.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            vehicleProperty.gridOptions.fillData(vehicleProperty.ListItems, response.Access);
    ...
    vehicleProperty.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehicleproperty/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleProperty/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    vehicleProperty.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehicleproperty/', vehicleProperty.selectedItem, 'POST').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"vehiclepropertydetailvalue/ViewModel", "", 'GET').success(function (response1) {
    ...
    vehicleProperty.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'vehicleproperty/', parseInt(vehicleProperty.gridOptions.selectedRow.item.Id), 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            vehicleProperty.selectedItem = response.Item;
            ...
            //*****************************************************************
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleProperty/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    vehicleProperty.editRow = function (frm) {
        ...
        if (vehicleProperty.oldLinkPropertyTypeId != vehicleProperty.selectedItem.LinkPropertyTypeId) {
            var filterValue = {
                ...
                value: parseInt(vehicleProperty.selectedItem.Id),
                SearchType: 0
            }
            var engine = {};
            engine.Filters = [];
    ...
    vehicleProperty.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                vehicleProperty.busyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'vehicleproperty/', vehicleProperty.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'VehicleContract/ViewModel', "", 'GET').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'VehicleContract/getall', model, 'POST').success(function (response) {
                    ...
                    vehicleProperty.contractsListresultAccess = response.Access;
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleProperty/addContract.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    rashaErManage.checkAction(data, errCode);
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    vehicleProperty.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    vehicleProperty.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleProperty/report.html',
            scope: $scope
        });
    }
