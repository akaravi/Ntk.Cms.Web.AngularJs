app.controller("vehiclePropertyFavoriteController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $filter) {
    ...
    vehiclePropertyFavorite.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"vehiclePropertyFavorite/getall", vehiclePropertyFavorite.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            vehiclePropertyFavorite.gridOptions.fillData(vehiclePropertyFavorite.ListItems, response.Access);
    ...
    vehiclePropertyFavorite.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclePropertyFavorite/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            vehiclePropertyFavorite.busyIndicator.isActive = false;
            vehiclePropertyFavorite.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehiclePropertyFavorite/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    vehiclePropertyFavorite.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclePropertyFavorite/', vehiclePropertyFavorite.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            vehiclePropertyFavorite.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehiclePropertyFavorite/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'VehiclePropertyFavorite/', vehiclePropertyFavorite.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'VehiclePropertyFavorite/', vehiclePropertyFavorite.selectedItemForDelete.Id, 'DELETE').success(function (res) {
