app.controller("vehicleContractController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    vehicleContract.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleContract/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    vehicleContract.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/', vehicleContract.selectedItem, 'POST').success(function (response) {
            vehicleContract.addRequested = false;
            vehicleContract.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    vehicleContract.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/', vehicleContract.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleContract/edit.html',
                scope: $scope
            });
    ...
    vehicleContract.editRow = function (frm) {
        ...
        vehicleContract.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/', vehicleContract.selectedItem, "PUT").success(function (response) {
            vehicleContract.addRequested = true;
            rashaErManage.checkAction(response);
            vehicleContract.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/', vehicleContract.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'vehiclecontract/', vehicleContract.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    cmsSitegrd.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Vehicle/VehicleContract/report.html',
            scope: $scope
        });
    }
