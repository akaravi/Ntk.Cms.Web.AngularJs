app.controller("vehiclePropertyTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $window, $filter) {
    ...
    vehiclePropertyType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclepropertytype/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehiclePropertyType/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    vehiclePropertyType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'vehiclepropertytype/', vehiclePropertyType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            vehiclePropertyType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Vehicle/VehiclePropertyType/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'vehiclepropertytype/', vehiclePropertyType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'vehiclepropertytype/', vehiclePropertyType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    vehiclePropertyType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Vehicle/VehiclePropertyType/report.html',
            scope: $scope
        });
    }
