app.controller("inputLogGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    inputLogCtrl.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"inputLogCtrl/getall", engine, 'POST').success(function (response) {
            ...
            inputLogCtrl.gridOptions.fillData(inputLogCtrl.ListItems, response.Access);
    ...
    inputLogCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'inputLogCtrl/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            inputLogCtrl.busyIndicator.isActive = false;
            inputLogCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/inputLogCtrl/add.html',
                scope: $scope
            });
    ...
    inputLogCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'inputLogCtrl/', inputLogCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            inputLogCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/inputLogCtrl/edit.html',
                scope: $scope
            });
