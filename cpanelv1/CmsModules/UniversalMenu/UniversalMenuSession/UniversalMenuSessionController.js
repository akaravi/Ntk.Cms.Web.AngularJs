app.controller("sessionGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    sessionCtrl.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"universalmenusession/getall", engine, 'POST').success(function (response) {
            ...
            sessionCtrl.gridOptions.fillData(sessionCtrl.ListItems,response.Access);
    ...
    sessionCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenusession/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            sessionCtrl.busyIndicator.isActive = false;
            sessionCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/Session/add.html',
                scope: $scope
            });
    ...
    sessionCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenusession/', sessionCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            sessionCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/Session/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'universalmenusession/', sessionCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'universalmenusession/', sessionCtrl.selectedItemForDelete.Id, 'DELETE').success(function (res) {
