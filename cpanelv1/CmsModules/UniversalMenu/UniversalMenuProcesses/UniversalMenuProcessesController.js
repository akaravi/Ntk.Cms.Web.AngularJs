app.controller("processGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$validator', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $validator, $window, $filter) {
    ...
    processCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenuProcesses/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuProcesses/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    processCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenuProcesses/', processCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            // Open the modal
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuProcesses/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
    ...
    processCtrl.showSubmitValueForm = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuProcesses/submitValueForm.html',
            scope: $scope
        });
    }
    ...
    processCtrl.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuProcesses/report.html',
            scope: $scope
        });
    }
