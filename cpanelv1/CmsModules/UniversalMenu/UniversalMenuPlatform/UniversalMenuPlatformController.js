app.controller("platformGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    platformCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenuplatform/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuPlatform/add.html',
                scope: $scope
            });
    ...
    platformCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'universalmenuplatform/', platformCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            platformCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuPlatform/edit.html',
                scope: $scope
            });
    ...
    platformCtrl.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/UniversalMenu/UniversalMenuPlatform/report.html',
            scope: $scope
        });
    }
