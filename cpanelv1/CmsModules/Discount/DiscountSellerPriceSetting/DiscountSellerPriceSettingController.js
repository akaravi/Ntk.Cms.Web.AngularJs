app.controller("discountSellerPriceSettingController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    discountSellerPriceSetting.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"discountSellerPriceSetting/getall", engine, 'POST').success(function (response) {
            ...
            discountSellerPriceSetting.gridOptions.fillData(discountSellerPriceSetting.ListItems, response.Access);
    ...
    discountSellerPriceSetting.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'discountSellerPriceSetting/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountSellerPriceSetting.busyIndicator.isActive = false;
            discountSellerPriceSetting.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/discountSellerPriceSetting/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    discountSellerPriceSetting.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'discountSellerPriceSetting/', discountSellerPriceSetting.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountSellerPriceSetting.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/discountSellerPriceSetting/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'discountSellerPriceSetting/', discountSellerPriceSetting.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'discountSellerPriceSetting/', discountSellerPriceSetting.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    discountSellerPriceSetting.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/discountSellerPriceSetting/report.html',
            scope: $scope
        });
    }
