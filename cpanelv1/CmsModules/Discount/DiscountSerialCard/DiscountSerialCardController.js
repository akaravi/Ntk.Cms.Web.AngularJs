app.controller("discountSerialCardController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    discountSerialCard.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"DiscountSerialCard/getall", discountSerialCard.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            discountSerialCard.gridOptions.fillData(discountSerialCard.ListItems, response.Access);
    ...
    discountSerialCard.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountSerialCard/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountSerialCard.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSerialCard/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    //    ajax.call(cmsServerConfig.configApiServerPath+'DiscountSerialCard/', discountSerialCard.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
    //        rashaErManage.checkAction(response);
    //        discountSerialCard.selectedItem = response.Item;
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSerialCard/sale.html',
    //            scope: $scope
    //        });
    ...
    discountSerialCard.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountSerialCard/', discountSerialCard.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            if (discountSerialCard
                ...
                0) discountSerialCard.selectUniversalMenuOnUndetectableKey = true;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSerialCard/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DiscountSerialCard/', discountSerialCard.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DiscountSerialCard/', discountSerialCard.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    discountSerialCard.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSerialCard/report.html',
            scope: $scope
        });
    }
