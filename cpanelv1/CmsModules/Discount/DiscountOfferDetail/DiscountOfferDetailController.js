app.controller("discountOfferDetailController", ["$scope", "$rootScope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $rootScope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    discountOfferDetail.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"DiscountOfferDetail/getall", engine, 'POST').success(function (response) {
            ...
            discountOfferDetail.gridOptions.fillData(discountOfferDetail.ListItems, response.Access);
    ...
    discountOfferDetail.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountOfferDetail.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferDetail/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    discountOfferDetail.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferDetail/', discountOfferDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountOfferDetail.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferDetail/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferDetail/', discountOfferDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferDetail/', discountOfferDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    //    ajax.call(cmsServerConfig.configApiServerPath+"ApiTelegramLogInput/", engine, "POST").success(function (response) { //Get Telegram user info from input logs
    //        discountOfferDetail.addRequested = false;
    //        discountOfferDetail.busyIndicator.isActive = false;
    ...
    //    discountOfferDetail.addRequested = true;
    //    discountOfferDetail.busyIndicator.isActive = true;
    //    ajax.call(cmsServerConfig.configApiServerPath+"ApiTelegramMemberInfo/", selectedId, "GET").success(function (response) {
    //        discountOfferDetail.selectedItem = response.Item;
    ...
    //            discountOfferDetail.busyIndicator.isActive = false;
    //            $modal.open({
    //                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramMemberInfo/discountOfferDetail.html',
    //                scope: $scope
    //            });
    //        }).error(function (data, errCode, c, d) {
    //            rashaErManage.checkAction(data, errCode);
    ...
    //        discountOfferDetail.addRequested = true;
    //        ajax.call(cmsServerConfig.configApiServerPath+"MemberUser/", selected.originalObject.Id, "GET").success(function (response) {
    //            discountOfferDetail.addRequested = false;
    //            rashaErManage.checkAction(response);
    ...
    discountOfferDetail.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferDetail/report.html',
            scope: $scope
        });
    }
