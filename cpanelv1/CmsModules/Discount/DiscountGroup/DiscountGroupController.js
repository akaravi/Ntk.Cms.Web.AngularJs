app.controller("discountGroupController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    discountGroup.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountGroup/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountGroup/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    discountGroup.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountGroup/', discountGroup.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountGroup/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DiscountGroup/', discountGroup.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DiscountGroup/', discountGroup.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    discountGroup.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountGroup/report.html',
            scope: $scope
        });
    }
