app.controller("discountSellerController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    discountSeller.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"DiscountSeller/getall", engine, 'POST').success(function (response) {
            ...
            discountSeller.gridOptions.fillData(discountSeller.ListItems, response.Access);
    ...
    discountSeller.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'discountSeller/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountSeller.busyIndicator.isActive = false;
            discountSeller.selectedItem = response.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSeller/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    discountSeller.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'discountSeller/', discountSeller.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            discountSeller.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/discountSeller/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DiscountSeller/', discountSeller.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DiscountSeller/', discountSeller.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    discountSeller.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountSeller/report.html',
            scope: $scope
        });
    }
