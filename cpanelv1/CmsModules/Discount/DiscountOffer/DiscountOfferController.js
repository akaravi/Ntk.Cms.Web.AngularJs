app.controller("discountOfferController", ["$scope", "$rootScope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $rootScope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOffer/add.html',
    //            scope: $scope
    //        });
    //    }).error(function (data, errCode, c, d) {
    //        rashaErManage.checkAction(data, errCode);
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOffer/edit.html',
    //            scope: $scope
    //        });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DiscountOffer/', discountOffer.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DiscountOffer/', discountOffer.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    discountOffer.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOffer/report.html',
            scope: $scope
        });
    }
