app.controller("discountOfferTransactionController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    discountOfferTransaction.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferTransaction/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferTransaction/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    discountOfferTransaction.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferTransaction/', discountOfferTransaction.selectedItem, 'POST').success(function (response) {
            discountOfferTransaction.addRequested = false;
            discountOfferTransaction.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    discountOfferTransaction.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferTransaction/', discountOfferTransaction.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferTransaction/edit.html',
                scope: $scope
            });
    ...
    discountOfferTransaction.editRow = function (frm) {
        ...
        discountOfferTransaction.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'DiscountOfferTransaction/', discountOfferTransaction.selectedItem, "PUT").success(function (response) {
            discountOfferTransaction.addRequested = true;
            rashaErManage.checkAction(response);
            discountOfferTransaction.busyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    discountOfferTransaction.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Discount/DiscountOfferTransaction/report.html',
            scope: $scope
        });
    }
