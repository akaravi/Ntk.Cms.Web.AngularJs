app.controller("campaignDetailMemberController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignDetailMember.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"campaignDetailMember/getall", engine, 'POST').success(function (response) {
            ...
            campaignDetailMember.gridOptions.fillData(campaignDetailMember.ListItems, response.Access);
    ...
    campaignDetailMember.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailMember/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailMember.busyIndicator.isActive = false;
            campaignDetailMember.selectedItem = response.Item;
           
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailMember/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    //    ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailMember/', campaignDetailMember.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
    //        rashaErManage.checkAction(response);
    //        campaignDetailMember.selectedItem = response.Item;
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailMember/edit.html',
    //            scope: $scope
    //        });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailMember/', campaignDetailMember.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailMember/', campaignDetailMember.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    campaignDetailMember.deleteAttachedfieldName = function (index) {
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailMember/', campaignDetailMember.contractsList[index].Id, 'DELETE').success(function (res) {
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
    ...
    campaignDetailMember.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailMember/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    campaignDetailMember.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", campaignDetailMember.fileIdToDelete, 'GET').success(function (response1) {
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    campaignDetailMember.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    campaignDetailMember.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailMember/report.html',
            scope: $scope
        });
    }
