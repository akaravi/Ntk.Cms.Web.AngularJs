app.controller("campaignDetailController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignDetail.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"campaignDetail/getall", engine, 'POST').success(function (response) {
            ...
            campaignDetail.gridOptions.fillData(campaignDetail.ListItems, response.Access);
    ...
    campaignDetail.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetail.busyIndicator.isActive = false;
            campaignDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetail/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    campaignDetail.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/', campaignDetail.selectedItem, 'POST').success(function (response) {
            campaignDetail.addRequested = false;
            campaignDetail.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    campaignDetail.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/', campaignDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetail.selectedItem = response.Item;
            if (campaignDetail
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetail/edit.html',
                scope: $scope
            });
    ...
    campaignDetail.editRow = function (frm) {
        ...
        campaignDetail.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/', campaignDetail.selectedItem, "PUT").success(function (response) {
            campaignDetail.addRequested = true;
            rashaErManage.checkAction(response);
            campaignDetail.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/', campaignDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'campaignDetail/', campaignDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    campaignDetail.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetail/report.html',
            scope: $scope
        });
    }
