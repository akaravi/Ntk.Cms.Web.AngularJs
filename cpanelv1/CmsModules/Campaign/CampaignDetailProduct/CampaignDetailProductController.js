app.controller("campaignDetailProductController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignDetailProduct.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"campaignDetailProduct/getall", engine, 'POST').success(function (response) {
            ...
            campaignDetailProduct.gridOptions.fillData(campaignDetailProduct.ListItems, response.Access);
    ...
    campaignDetailProduct.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailProduct.busyIndicator.isActive = false;
            campaignDetailProduct.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProduct/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    campaignDetailProduct.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/', campaignDetailProduct.selectedItem, 'POST').success(function (response) {
            campaignDetailProduct.addRequested = false;
            campaignDetailProduct.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    campaignDetailProduct.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/', campaignDetailProduct.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailProduct.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProduct/edit.html',
                scope: $scope
            });
    ...
    campaignDetailProduct.editRow = function (frm) {
        ...
        campaignDetailProduct.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/', campaignDetailProduct.selectedItem, "PUT").success(function (response) {
            campaignDetailProduct.addRequested = true;
            rashaErManage.checkAction(response);
            campaignDetailProduct.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/', campaignDetailProduct.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProduct/', campaignDetailProduct.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    campaignDetailProduct.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProduct/report.html',
            scope: $scope
        });
    }
