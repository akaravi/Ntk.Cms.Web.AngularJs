app.controller("campaignDetailProgramController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignDetailProgram.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"campaignDetailProgram/getall", engine, 'POST').success(function (response) {
            ...
            campaignDetailProgram.gridOptions.fillData(campaignDetailProgram.ListItems, response.Access);
    ...
    campaignDetailProgram.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProgram/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailProgram.busyIndicator.isActive = false;
            campaignDetailProgram.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProgram/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    campaignDetailProgram.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProgram/', campaignDetailProgram.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailProgram.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProgram/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProgram/', campaignDetailProgram.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailProgram/', campaignDetailProgram.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    campaignDetailProgram.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailProgram/report.html',
            scope: $scope
        });
    }
