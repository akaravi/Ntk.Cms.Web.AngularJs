app.controller("campaignDetailLogController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignDetailLog.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"campaignDetailLog/getall", engine, 'POST').success(function (response) {
            ...
            campaignDetailLog.gridOptions.fillData(campaignDetailLog.ListItems, response.Access);
    ...
    campaignDetailLog.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailLog/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailLog.busyIndicator.isActive = false;
            campaignDetailLog.selectedItem = response.Item;
            campaignDetailLog.selectedItem.LinkFileIds = null;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailLog/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    campaignDetailLog.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'campaignDetailLog/', campaignDetailLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            campaignDetailLog.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailLog/edit.html',
                scope: $scope
            });
    ...
    campaignDetailLog.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailLog/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    campaignDetailLog.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    campaignDetailLog.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignDetailLog/report.html',
            scope: $scope
        });
    }
