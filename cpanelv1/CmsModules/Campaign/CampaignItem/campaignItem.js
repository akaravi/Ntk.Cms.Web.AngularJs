app.controller("campaignItemController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    campaignItem.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CampaignItem/getall",engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            campaignItem.ListItems = response.ListItems;
            campaignItem.gridOptions.fillData(campaignItem.ListItems, response.Access); // Sending Access as an argument
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"newsTag/ViewModel", "", 'GET').success(function (response) {    //Get a ViewModel for newsTag
    ...
    campaignItem.addNewCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/ViewModel', "", 'GET').success(function (response) {
    ...
    campaignItem.EditCategoryModel = function () {
        ...
        campaignItem.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/', campaignItem.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            campaignItem.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            campaignItem.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/campaign/CampaignItemGroup/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    campaignItem.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/', campaignItem.selectedItem, 'POST').success(function (response) {
            campaignItem.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    campaignItem.EditCategory = function (frm) {
        ...
        campaignItem.categoryBusyIndicator.isActive = true;
        campaignItem.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/', campaignItem.selectedItem, "PUT").success(function (response) {
    ...
    campaignItem.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    campaignItem.selectedItemForDelete = response.Item;
                    console.log(campaignItem.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'CampaignItemGroup/', campaignItem.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        campaignItem.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    campaignItem.selectContent = function (node) {
        ...
        campaignItem.gridOptions.advancedSearchData.engine.AccessLoad = true;
    ...
    campaignItem.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            campaignItem.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/CampaignItem/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    campaignItem.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/', campaignItem.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            campaignItem.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Campaign/CampaignItem/edit.html',
                scope: $scope
            });
    ...
    campaignItem.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/', campaignItem.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            campaignItem.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    campaignItem.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/', campaignItem.selectedItem, "PUT").success(function (response) {
            campaignItem.categoryBusyIndicator.isActive = false;
            campaignItem.addRequested = false;
            campaignItem.treeConfig.showbusy = false;
    ...
    campaignItem.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                campaignItem.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"CampaignItem/", campaignItem.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    campaignItem.showbusy = false;
                    campaignItem.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    campaignItem.selectedItemForDelete = response.Item;
                    //console.log(campaignItem.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"CampaignItem/", campaignItem.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        campaignItem.categoryBusyIndicator.isActive = false;
                        campaignItem.treeConfig.showbusy = false;
                        campaignItem.showIsBusy = false;
    ...
    campaignItem.confirmUnConfirmcampaignItem = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/', campaignItem.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'CampaignItem/', campaignItem.selectedItem, "PUT").success(function (response2) {
    ...
    campaignItem.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/CampaignItem/report.html',
            scope: $scope
        });
    }
