app.controller("campaignAdminController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignAdmin/add.html',
    //            scope: $scope
    //        });
    //    }).error(function (data, errCode, c, d) {
    //        rashaErManage.checkAction(data, errCode);
    ...
    //        $modal.open({
    //            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignAdmin/edit.html',
    //            scope: $scope
    //        });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'campaignAdmin/', campaignAdmin.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'campaignAdmin/', campaignAdmin.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    campaignAdmin.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Campaign/campaignAdmin/report.html',
            scope: $scope
        });
    }
