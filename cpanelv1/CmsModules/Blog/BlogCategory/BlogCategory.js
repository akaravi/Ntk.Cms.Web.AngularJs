app.controller("blogCategoryCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    blogCategory.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogCategory/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            //-----
            //$modal.open({
            //    templateUrl: 'cpanelv1/CmsModules/Article/blogCategory/add.html',
            //    scope: $scope
            //});
    ...
    blogCategory.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogCategory/', blogCategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogCategory.selectedItem = response.Item;
            //Set dataForTheTree
            ...
            //---
            //$modal.open({
            //    templateUrl: 'cpanelv1/CmsModules/Article/blogCategory/edit.html',
            //    scope: $scope
            //});
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'blogCategory/',  blogCategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'blogCategory/', blogCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        blogCategory.busyIndicator.isActive = false;
