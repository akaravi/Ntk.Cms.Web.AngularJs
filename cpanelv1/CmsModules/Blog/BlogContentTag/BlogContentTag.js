app.controller("blogContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    blogContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'BlogContenttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Blog/blogcontentTag/views/add.html',
                scope: $scope
            });
    ...
    blogContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'BlogContenttag/',  blogContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Blog/blogcontentTag/views/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'BlogContenttag/',  blogContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'BlogContenttag/',  blogContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
