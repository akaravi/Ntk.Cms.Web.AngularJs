app.controller("blogCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    blogComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"blogComment/getall", blogComment.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            blogComment.gridOptions.fillData(blogComment.ListItems, response.Access);
    ...
    blogComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogComment.busyIndicator.isActive = false;
            blogComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Blog/BlogComment/add.html',
                scope: $scope
            });
    ...
    blogComment.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogComment/', blogComment.selectedItem, 'POST').success(function (response) {
            blogComment.addRequested = false;
            blogComment.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    blogComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogComment/', blogComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Blog/BlogComment/edit.html',
                scope: $scope
            });
    ...
    blogComment.editRow = function (frm) {
        ...
        blogComment.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'blogComment/', blogComment.selectedItem, "PUT").success(function (response) {
            blogComment.addRequested = true;
            rashaErManage.checkAction(response);
            blogComment.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'blogComment/', blogComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'blogComment/', blogComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    blogComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Blog/BlogComment/report.html',
            scope: $scope
        });
    }
