app.controller("blogContentParameterController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    blogContentParameter.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"blogContentParameter/getall", engine, 'POST').success(function (response) {
            ...
            blogContentParameter.gridOptions.fillData(blogContentParameter.ListItems, response.Access);
    ...
    blogContentParameter.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogContentParameter.busyIndicator.isActive = false;
            blogContentParameter.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/blog/blogContentParameter/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    blogContentParameter.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/', blogContentParameter.selectedItem, 'POST').success(function (response) {
            blogContentParameter.addRequested = false;
            blogContentParameter.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    blogContentParameter.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/', blogContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogContentParameter.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/blog/blogContentParameter/edit.html',
                scope: $scope
            });
    ...
    blogContentParameter.editRow = function (frm) {
        if (frm.$invalid)
            return;
        blogContentParameter.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/', blogContentParameter.selectedItem, "PUT").success(function (response) {
            blogContentParameter.addRequested = true;
            rashaErManage.checkAction(response);
            blogContentParameter.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/', blogContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'blogContentParameter/', blogContentParameter.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    blogContentParameter.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/blog/blogContentParameter/report.html',
            scope: $scope
        });
    }
