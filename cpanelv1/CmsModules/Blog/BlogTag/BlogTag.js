 ...
 blogTag.LinkCategoryTagIdSelector = {
    ...
    blogTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"blogtag/getall", blogTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            blogTag.gridOptions.fillData(blogTag.ListItems, response.Access); // Sending Access as an argument
    ...
    blogTag.addNewCategoryModel = function () {
        blogTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'blogCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/blog/blogCategorytag/add.html',
                        scope: $scope
                    });
                    blogTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    blogTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogCategorytag/', blogTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            blogTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            blogTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/blog/blogCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    blogTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'blogtag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            blogTag.selectedItem = response.Item;
            blogTag.selectedItem.LinkCategoryTagId = blogTag.CategoryTagId;
            //blogTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/blog/blogtag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    blogTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'blogtag/', blogTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            blogTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/blog/blogtag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    blogTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                blogTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"blogtag/", blogTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    blogTag.showbusy = false;
                    blogTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    blogTag.selectedItemForDelete = response.Item;
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    blogTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    blogTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/blog/blogtag/report.html',
            scope: $scope
        });
    }
