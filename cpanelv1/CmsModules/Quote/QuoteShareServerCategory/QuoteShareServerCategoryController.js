app.controller("quoteShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    quoteShareServerCategory.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"quoteShareServerCategory/getall", {AccessLoad : true}, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            quoteShareServerCategory.ListItems = response.ListItems;
            quoteShareServerCategory.gridOptions.fillData(quoteShareServerCategory.ListItems, response.Access); // Sending Access as an argument
    ...
    quoteShareServerCategory.EditCategoryModel = function () {
        ...
        quoteShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'QuoteShareMainAdminSetting/', quoteShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            quoteShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            quoteShareServerCategory.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Quote/QuoteShareMainAdminSetting/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    quoteShareServerCategory.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'QuoteShareMainAdminSetting/', quoteShareServerCategory.selectedItem, 'POST').success(function (response) {
            quoteShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    quoteShareServerCategory.EditCategory = function (frm) {
        ...
        quoteShareServerCategory.categoryBusyIndicator.isActive = true;
        quoteShareServerCategory.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'QuoteShareMainAdminSetting/', quoteShareServerCategory.selectedItem, "PUT").success(function (response) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'QuoteShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'QuoteShareMainAdminSetting/', quoteShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        quoteShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    quoteShareServerCategory.selectContent = function (node) {
        ...
        quoteShareServerCategory.gridOptions.advancedSearchData.engine.AccessLoad = true;
    ...
    quoteShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'quoteShareServerCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            quoteShareServerCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/quote/quoteShareServerCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    quoteShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'quoteShareServerCategory/', quoteShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            quoteShareServerCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/quote/quoteShareServerCategory/edit.html',
                scope: $scope
            });
    ...
    quoteShareServerCategory.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'quoteShareServerCategory/', quoteShareServerCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            quoteShareServerCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    quoteShareServerCategory.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'quoteShareServerCategory/', quoteShareServerCategory.selectedItem, "PUT").success(function (response) {
            quoteShareServerCategory.categoryBusyIndicator.isActive = false;
            quoteShareServerCategory.addRequested = false;
            quoteShareServerCategory.treeConfig.showbusy = false;
    ...
    quoteShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                quoteShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"quoteShareServerCategory/", quoteShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    quoteShareServerCategory.showbusy = false;
                    quoteShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    quoteShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(quoteShareServerCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"quoteShareServerCategory/", quoteShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        quoteShareServerCategory.categoryBusyIndicator.isActive = false;
                        quoteShareServerCategory.treeConfig.showbusy = false;
                        quoteShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    quoteShareServerCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Quote/quoteShareServerCategory/report.html',
            scope: $scope
        });
    }
