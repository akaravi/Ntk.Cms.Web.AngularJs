app.controller("formBuilderFormSubmitController", ["$scope", "$state", "$stateParams", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$validator', '$filter', '$window', function ($scope, $state,$stateParams, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $validator, $filter, $window) {
    ...
    value.openAddModal = function () {
        ...
        value.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'formBuilderFormSubmit/ViewModel', "", 'GET').success(function (response) {
            value.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            value.selectedItem = response.Item;
            $builder.removeAllFormObject('default');
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderFormSubmit/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    value.openEditModal = function () {
        ...
        value.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FormBuilderFormSubmit/', value.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            value.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            value.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderFormSubmit/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'formBuilderFormSubmit/', value.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'formBuilderFormSubmit/', value.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        value.busyIndicator.isActive = false;
 ...
 //Export Report 
    ...
    value.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderFormSubmit/report.html',
            scope: $scope
        });
    }
    ...
    value.onLinkFormIdChange = function (formId, values) {
        ajax.call(cmsServerConfig.configApiServerPath+'formbuilderform/', formId, 'GET').success(function (response) {
            value.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
