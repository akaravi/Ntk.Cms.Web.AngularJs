app.controller("formController", ["$scope","$state", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$filter', '$window', function ($scope,$state, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $filter, $window) {
    ...
    form.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'formbuilderform/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderForm/add.html',
                scope: $scope
            });
    ...
    form.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'formbuilderform/', form.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            form.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderForm/edit.html',
                scope: $scope
            });
            form.selectedItem.LinkModuleId = null;
        }).error(function (data, errCode, c, d) {
    ...
    form.showForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderForm/preview.html',
            scope: $scope
        });
    }
 ...
 //Export Report 
    ...
    form.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/FormBuilder/FormBuilderForm/report.html',
            scope: $scope
        });
    }
