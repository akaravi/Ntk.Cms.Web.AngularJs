app.controller("articleContentAndParameterValueController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    articleContentAndParameterValue.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"articleContentAndParameterValue/getall", engine, 'POST').success(function (response) {
            ...
            articleContentAndParameterValue.gridOptions.fillData(articleContentAndParameterValue.ListItems, response.Access);
    ...
    articleContentAndParameterValue.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentAndParameterValue.busyIndicator.isActive = false;
            articleContentAndParameterValue.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/articleContentAndParameterValue/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    articleContentAndParameterValue.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/', articleContentAndParameterValue.selectedItem, 'POST').success(function (response) {
            articleContentAndParameterValue.addRequested = false;
            articleContentAndParameterValue.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    articleContentAndParameterValue.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/', articleContentAndParameterValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentAndParameterValue.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/articleContentAndParameterValue/edit.html',
                scope: $scope
            });
    ...
    articleContentAndParameterValue.editRow = function (frm) {
        ...
        articleContentAndParameterValue.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/', articleContentAndParameterValue.selectedItem, "PUT").success(function (response) {
            articleContentAndParameterValue.addRequested = true;
            rashaErManage.checkAction(response);
            articleContentAndParameterValue.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/', articleContentAndParameterValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'articleContentAndParameterValue/', articleContentAndParameterValue.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    articleContentAndParameterValue.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Article/articleContentAndParameterValue/report.html',
            scope: $scope
        });
    }
