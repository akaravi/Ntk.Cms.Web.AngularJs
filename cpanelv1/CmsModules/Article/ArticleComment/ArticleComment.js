app.controller("articleCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    articleComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"articleComment/getall", engine, 'POST').success(function (response) {
            ...
            articleComment.gridOptions.fillData(articleComment.ListItems , response.Access);
    ...
    articleComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleComment.busyIndicator.isActive = false;
            articleComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articleComment/add.html',
                scope: $scope
            });
    ...
    articleComment.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleComment/', articleComment.selectedItem, 'POST').success(function (response) {
            articleComment.addRequested = false;
            articleComment.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    articleComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleComment/', articleComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            articleComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articleComment/edit.html',
                scope: $scope
            });
    ...
    articleComment.editRow = function (frm) {
        ...
        articleComment.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleComment/', articleComment.selectedItem, "PUT").success(function (response) {
            articleComment.addRequested = true;
            rashaErManage.checkAction(response);
            articleComment.busyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    articleComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Article/ArticleComment/report.html',
            scope: $scope
        });
    }
