app.controller("articleShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    articleShareServerCategory.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"articleShareServerCategory/getall", engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            articleShareServerCategory.ListItems = response.ListItems;
            articleShareServerCategory.gridOptions.fillData(articleShareServerCategory.ListItems, response.Access); // Sending Access as an argument
    ...
    articleShareServerCategory.EditCategoryModel = function () {
        ...
        articleShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ArticleShareMainAdminSetting/', articleShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            articleShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            articleShareServerCategory.selectedItem = response.Item;
    ...
    articleShareServerCategory.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ArticleShareMainAdminSetting/', articleShareServerCategory.selectedItem, 'POST').success(function (response) {
            articleShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    articleShareServerCategory.EditCategory = function (frm) {
        ...
        articleShareServerCategory.categoryBusyIndicator.isActive = true;
        articleShareServerCategory.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ArticleShareMainAdminSetting/', articleShareServerCategory.selectedItem, "PUT").success(function (response) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ArticleShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ArticleShareMainAdminSetting/', articleShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        articleShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    articleShareServerCategory.selectContent = function (node) {
        ...
        articleShareServerCategory.gridOptions.advancedSearchData.engine.AccessLoad = true;
    ...
    articleShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleShareServerCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            articleShareServerCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articleShareServerCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    articleShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleShareServerCategory/', articleShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            articleShareServerCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articleShareServerCategory/edit.html',
                scope: $scope
            });
    ...
    articleShareServerCategory.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleShareServerCategory/', articleShareServerCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            articleShareServerCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    articleShareServerCategory.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleShareServerCategory/', articleShareServerCategory.selectedItem, "PUT").success(function (response) {
            articleShareServerCategory.categoryBusyIndicator.isActive = false;
            articleShareServerCategory.addRequested = false;
            articleShareServerCategory.treeConfig.showbusy = false;
    ...
    articleShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                articleShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"articleShareServerCategory/", articleShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    articleShareServerCategory.showbusy = false;
                    articleShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    articleShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(articleShareServerCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"articleShareServerCategory/", articleShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        articleShareServerCategory.categoryBusyIndicator.isActive = false;
                        articleShareServerCategory.treeConfig.showbusy = false;
                        articleShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    articleShareServerCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Article/articleShareServerCategory/report.html',
            scope: $scope
        });
    }
