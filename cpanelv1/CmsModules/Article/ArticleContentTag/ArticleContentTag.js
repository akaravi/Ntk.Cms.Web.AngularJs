app.controller("articleContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    articleContentTag.init = function () {
        ...
        articleContentTag.gridOptions.advancedSearchData.engine.AccessLoad = true;
        ajax.call(cmsServerConfig.configApiServerPath+"articleContenttag/getall", articleContentTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            articleContentTag.gridOptions.fillData(articleContentTag.ListItems , response.Access);
    ...
    articleContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articlecontentTag/add.html',
                scope: $scope
            });
    ...
    articleContentTag.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/', articleContentTag.selectedItem , 'POST').success(function (response) {
            articleContentTag.addRequested = false;
            articleContentTag.busyIndicator.isActive = false;
    ...
    articleContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/',  articleContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/article/articlecontentTag/edit.html',
                scope: $scope
            });
    ...
    articleContentTag.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/',  articleContentTag.selectedItem , "PUT").success(function (response) {
            articleContentTag.addRequested = true;
            rashaErManage.checkAction(response);
            articleContentTag.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/',  articleContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'articleContenttag/',  articleContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
