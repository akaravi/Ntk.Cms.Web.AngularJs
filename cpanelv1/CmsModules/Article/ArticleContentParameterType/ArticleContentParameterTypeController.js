app.controller("articleContentParameterTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    articleContentParameterType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/ArticleContentParameterType/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    articleContentParameterType.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/', articleContentParameterType.selectedItem, 'POST').success(function (response) {
            articleContentParameterType.addRequested = false;
            articleContentParameterType.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    articleContentParameterType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/', articleContentParameterType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/articleContentParameterType/edit.html',
                scope: $scope
            });
    ...
    articleContentParameterType.editRow = function (frm) {
        ...
        articleContentParameterType.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/', articleContentParameterType.selectedItem, "PUT").success(function (response) {
            articleContentParameterType.addRequested = true;
            rashaErManage.checkAction(response);
            articleContentParameterType.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/', articleContentParameterType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameterType/', articleContentParameterType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    articleContentParameterType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Article/articleContentParameterType/report.html',
            scope: $scope
        });
    }
