app.controller("articleContentParameterController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    articleContentParameter.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"articleContentParameter/getall", engine, 'POST').success(function (response) {
            ...
            articleContentParameter.gridOptions.fillData(articleContentParameter.ListItems, response.Access);
    ...
    articleContentParameter.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentParameter.busyIndicator.isActive = false;
            articleContentParameter.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/articleContentParameter/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    articleContentParameter.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/', articleContentParameter.selectedItem, 'POST').success(function (response) {
            articleContentParameter.addRequested = false;
            articleContentParameter.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    articleContentParameter.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/', articleContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            articleContentParameter.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/articleContentParameter/edit.html',
                scope: $scope
            });
    ...
    articleContentParameter.editRow = function (frm) {
        ...
        articleContentParameter.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/', articleContentParameter.selectedItem, "PUT").success(function (response) {
            articleContentParameter.addRequested = true;
            rashaErManage.checkAction(response);
            articleContentParameter.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/', articleContentParameter.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'articleContentParameter/', articleContentParameter.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    articleContentParameter.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Article/articleContentParameter/report.html',
            scope: $scope
        });
    }
