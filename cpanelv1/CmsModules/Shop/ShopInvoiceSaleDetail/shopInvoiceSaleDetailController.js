app.controller("shopInvoiceSaleDetailController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$state', '$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $stateParams, $filter) {
    ...
    shopInvoiceSaleDetail.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Shop/shopInvoiceSaleDetail/modalMenu.html",
            scope: $scope
        });
    }
    ...
    shopInvoiceSaleDetail.init = function () {
        if (shopInvoiceSaleDetail.stateMode == "edit") {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"shopinvoicesaledetail/getall", shopInvoiceSaleDetail.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
                ...
                shopInvoiceSaleDetail.gridOptions.Access = response.Access;
                //shopInvoiceSaleDetail.gridOptions.fillData(shopInvoiceSaleDetail.ListItems, response.Access); // Sending Access as an argument
        ...
        if ($stateParams.invoiceId > 0)
            ajax.call(cmsServerConfig.configApiServerPath+"shopinvoicesale/", $stateParams.invoiceId, 'GET').success(function (response) {
                rashaErManage.checkAction(response);
    ...
    shopInvoiceSaleDetail.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoicesaledetail/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSaleDetail/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopInvoiceSaleDetail.openEditModel = function () {
        ...
        shopInvoiceSaleDetail.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleDetail/', shopInvoiceSaleDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            shopInvoiceSaleDetail.addRequested = false;
            rashaErManage.checkAction(response);
            shopInvoiceSaleDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSaleDetail/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSaleDetail/", shopInvoiceSaleDetail.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSaleDetail/", shopInvoiceSaleDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        shopInvoiceSaleDetail.categoryBusyIndicator.isActive = false;
