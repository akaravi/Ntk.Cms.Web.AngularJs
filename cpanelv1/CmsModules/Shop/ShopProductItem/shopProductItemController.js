app.controller("shopProductItemController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    shopItem.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Shop/ShopProductItem/modalMenu.html",
            scope: $scope
        });
    }
    ...
    shopItem.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductItem/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductItem/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopItem.openEditContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductItem/', shopItem.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            shopItem.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductItem/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopItem.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductItem/', shopItem.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            shopItem.busyIndicator.isActive = false;
    ...
    shopItem.editContent = function (frm) {
        ...
        shopItem.busyIndicator.isActive = true;
        shopItem.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductItem/', shopItem.selectedItem, "PUT").success(function (response) {
            shopItem.busyIndicator.isActive = false;
            shopItem.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"ShopProductItem/", shopItem.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+"ShopProductItem/", shopItem.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        shopItem.busyIndicator.isActive = false;
                        shopItem.showIsBusy = false;
    ...
    shopItem.toggleExportForm = function () {
        ...
        shopItem.addRequested = false;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/shopproductItem/report.html',
            scope: $scope
        });
    }
