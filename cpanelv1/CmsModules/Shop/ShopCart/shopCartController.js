app.controller("shopCartController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$state', '$window', '$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $window, $stateParams, $filter) {
    ...
    shopCart.selectionChangedUser = function (item) {
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUser/', shopCart.selectedItem.LinkCmsUserId, 'GET').success(function (response2) {
    ...
    shopCart.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Shop/shopCart/modalMenu.html",
            scope: $scope
        });
    }
    //init Function
    shopCart.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopCart/getall", shopCart.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopCart.gridOptions.fillData(shopCart.ListItems, response.Access);
    ...
    shopCart.printDiv = function (divName) {
        ...
        if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
            ...
            popupWin.document.write('<!DOCTYPE html><html><head>' +
                ...
                '<link href="cpanelv1/CmsModules/Shop/shopCart/print.css" rel="stylesheet" />' +
    ...
    shopCart.selectContent = function (node) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopCart/getAll", shopCart.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopCart.busyIndicator.isActive = false;
            shopCart.ListItems = response.ListItems;
            shopCart.gridOptions.fillData(shopCart.ListItems, response.Access); // Sending Access as an argument
            shopCart.gridOptions.currentPageNumber = response.CurrentPageNumber;
            shopCart.gridOptions.totalRowCount = response.TotalRowCount;
            shopCart.gridOptions.rowPerPage = response.RowPerPage;
        }).error(function (data, errCode, c, d) {
    ...
    shopCart.addNewContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopCart/ViewModel', "", 'GET').success(function (response) {
            shopCart.addRequested = false;
            rashaErManage.checkAction(response);
            shopCart.selectedItem = response.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/addInvoiceByItem.html',
                scope: $scope,
                size: 'lg'
            });
        }).error(function (data, errCode, c, d) {
    ...
    shopCart.openEditModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopCart/', shopCart.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/editInvoiceByItem.html',
                scope: $scope,
                size: 'lg'
            });
    ...
    shopCart.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"shopCart/", shopCart.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    shopCart.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"shopCart/", shopCart.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        shopCart.busyIndicator.isActive = false;
                            ...
                            if (value.virtual_PublicConfig.LinkModuleFileLogoId != null)
                                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/PreviewImage", { id: value.virtual_PublicConfig.LinkModuleFileLogoId, name: null }, "POST").success(function (response) {
                                    ...
                                    if (key == 0) {
                                        ...
                                        shopCart.busyIndicator.isActive = false;
                                        $modal.open({
                                            templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/payment.html',
                                            scope: $scope
                                        });
                                    }
                                }).error(function (data, errCode, c, d) {
    ...
    shopCart.loadivocesaledetail = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopCart/", shopCart.selectedItem.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            shopCart.gridOptionIvoceSaleDetail.ListItems = response.Item.CartDetails;
            shopCart.gridOptionIvoceSaleDetail.Access = response.Access;
    ...
    shopCart.openAddModalSaleDetail = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopCartdetail/ViewModel', "", 'GET').success(function (response) {
            shopCart.addRequested = false;
            rashaErManage.checkAction(response);
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/addInvoiceSaleDetail.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopCart.openEditModelSaleDetail = function () {
        ...
        shopCart.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'shopCartDetail/', shopCart.gridOptionIvoceSaleDetail.selectedRow.item.Id, 'GET').success(function (response) {
            shopCart.addRequested = false;
            rashaErManage.checkAction(response);
            shopCart.selectedItemSaleDetail = response.Item;
            shopCart.selectedItemSaleDetail.LinkInvoiceSaleId = shopCart.selectedItem.Id;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/editInvoiceSaleDetail.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"shopCartDetail/", shopCart.gridOptionIvoceSaleDetail.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    rashaErManage.checkAction(response);
                    shopCart.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"shopCartDetail/", shopCart.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    shopCart.toggleExportForm = function () {
        ...
        shopCart.addRequested = false;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/shopCart/report.html',
            scope: $scope
        });
    }
