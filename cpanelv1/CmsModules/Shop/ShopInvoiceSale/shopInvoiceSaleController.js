app.controller("shopInvoiceSaleController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$state', '$window','$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $window,$stateParams, $filter) {
    ...
    shopInvoiceSale.selectionChangedUser = function (item) {
        ajax.call(cmsServerConfig.configApiServerPath+'CoreUser/', shopInvoiceSale.selectedItem.LinkCmsUserId, 'GET').success(function (response2) {
    ...
    //        ajax.call(cmsServerConfig.configApiServerPath+'Memberuser/', shopInvoiceSale.selectedItem.LinkMemberUserId, 'GET').success(function (response2) {
    ...
    shopInvoiceSale.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Shop/shopInvoiceSale/modalMenu.html",
            scope: $scope
        });
    }
    //init Function
    shopInvoiceSale.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopinvoicesale/getall", shopInvoiceSale.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopInvoiceSale.gridOptions.fillData(shopInvoiceSale.ListItems, response.Access);
   ...
   var printContents = document.getElementById(divName).innerHTML;
    ...
    if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
        ...
        popupWin.document.write('<!DOCTYPE html><html><head>' +
            ...
            '<link href="cpanelv1/CmsModules/Shop/ShopInvoiceSale/print.css" rel="stylesheet" />'+
    ...
    shopInvoiceSale.selectContent = function (node) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopinvoicesale/getAll", shopInvoiceSale.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopInvoiceSale.busyIndicator.isActive = false;
            shopInvoiceSale.ListItems = response.ListItems;
            shopInvoiceSale.gridOptions.fillData(shopInvoiceSale.ListItems, response.Access); // Sending Access as an argument
            shopInvoiceSale.gridOptions.currentPageNumber = response.CurrentPageNumber;
            shopInvoiceSale.gridOptions.totalRowCount = response.TotalRowCount;
            shopInvoiceSale.gridOptions.rowPerPage = response.RowPerPage;
        }).error(function (data, errCode, c, d) {
    ...
    shopInvoiceSale.addNewContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSale/ViewModel', "", 'GET').success(function (response) {
            shopInvoiceSale.addRequested = false;
            rashaErManage.checkAction(response);
            shopInvoiceSale.selectedItem = response.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopInvoiceSale/addInvoiceByItem.html',
                scope: $scope,
                size: 'lg'
            });
        }).error(function (data, errCode, c, d) {
    ...
    shopInvoiceSale.openEditModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSale/', shopInvoiceSale.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            ...
            //if (shopInvoiceSale.selectedItem.LinkMemberUserId > 0) {
            //    ajax.call(cmsServerConfig.configApiServerPath+'Memberuser/', shopInvoiceSale.selectedItem.LinkMemberUserId, 'GET').success(function (response3) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopInvoiceSale/editInvoiceByItem.html',
                scope: $scope,
                size: 'lg'
            });
    ...
    shopInvoiceSale.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"shopinvoiceSale/", shopInvoiceSale.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    shopInvoiceSale.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSale/", shopInvoiceSale.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        shopInvoiceSale.busyIndicator.isActive = false;
                            ...
                            if (value.virtual_PublicConfig.LinkModuleFileLogoId != null)
                                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/PreviewImage", { id: value.virtual_PublicConfig.LinkModuleFileLogoId, name: null }, "POST").success(function (response) {
                                    ...
                                    if (key == 0) {
                                        ...
                                        shopInvoiceSale.busyIndicator.isActive = false;
                                        $modal.open({
                                            templateUrl: 'cpanelv1/CmsModules/Shop/ShopInvoiceSale/payment.html',
                                            scope: $scope
                                        });
                                    }
                                }).error(function (data, errCode, c, d) {
    ...
    shopInvoiceSale.loadivocesaledetail = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopinvoicesaledetail/getall", shopInvoiceSale.gridOptionIvoceSaleDetail.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopInvoiceSale.gridOptionIvoceSaleDetail.Access = response.Access;
    ...
    shopInvoiceSale.openAddModalSaleDetail = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoicesaledetail/ViewModel', "", 'GET').success(function (response) {
            shopInvoiceSale.addRequested = false;
            rashaErManage.checkAction(response);
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSale/addInvoiceSaleDetail.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopInvoiceSale.openEditModelSaleDetail = function () {
        ...
        shopInvoiceSale.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleDetail/', shopInvoiceSale.gridOptionIvoceSaleDetail.selectedRow.item.Id, 'GET').success(function (response) {
            shopInvoiceSale.addRequested = false;
            rashaErManage.checkAction(response);
            shopInvoiceSale.selectedItemSaleDetail = response.Item;
            shopInvoiceSale.selectedItemSaleDetail.LinkInvoiceSaleId = shopInvoiceSale.selectedItem.Id;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSale/editInvoiceSaleDetail.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSaleDetail/", shopInvoiceSale.gridOptionIvoceSaleDetail.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    rashaErManage.checkAction(response);
                    shopInvoiceSale.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSaleDetail/", shopInvoiceSale.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    shopInvoiceSale.toggleExportForm = function () {
        ...
        shopInvoiceSale.addRequested = false;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSale/report.html',
            scope: $scope
        });
    }
