app.controller("shopProductServiceController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    shopService.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ShopProductService/getall", shopService.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            shopService.gridOptions.fillData(shopService.ListItems, response.Access); // Sending Access as an argument
    ...
    shopService.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductService/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            shopService.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductService/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopService.openEditContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductService/', shopService.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            shopService.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductService/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopService.toggleExportForm = function () {
        ...
        shopService.addRequested = false;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductService/report.html',
            scope: $scope
        });
    }
