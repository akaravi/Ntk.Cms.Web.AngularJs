app.controller("shopProductFileSerialController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    shopProductFileSerial.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopProductFileSerial/getall", engine, 'POST').success(function (response) {
            ...
            shopProductFileSerial.gridOptions.fillData(shopProductFileSerial.ListItems , response.Access);
    ...
    shopProductFileSerial.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            shopProductFileSerial.busyIndicator.isActive = false;
            shopProductFileSerial.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopProductFileSerial/add.html',
                scope: $scope
            });
    ...
    shopProductFileSerial.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/', shopProductFileSerial.selectedItem, 'POST').success(function (response) {
            shopProductFileSerial.addRequested = false;
            shopProductFileSerial.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    shopProductFileSerial.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/', shopProductFileSerial.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            shopProductFileSerial.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopProductFileSerial/edit.html',
                scope: $scope
            });
    ...
    shopProductFileSerial.editRow = function (frm) {
        ...
        shopProductFileSerial.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/', shopProductFileSerial.selectedItem, "PUT").success(function (response) {
            shopProductFileSerial.addRequested = true;
            rashaErManage.checkAction(response);
            shopProductFileSerial.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/', shopProductFileSerial.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'shopProductFileSerial/', shopProductFileSerial.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    shopProductFileSerial.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/shopProductFileSerial/report.html',
            scope: $scope
        });
    }
