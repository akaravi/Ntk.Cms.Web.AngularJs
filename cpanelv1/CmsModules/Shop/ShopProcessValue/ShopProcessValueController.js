app.controller("shopProcessValueController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$stateParams', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $stateParams, $builder, $state, $filter) {
    ...
    taskScheduleProcessValue.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProcessvalue/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            taskScheduleProcessValue.selectedItem = response.Item;
            taskScheduleProcessValue.busyIndicator.isActive = false;
            taskScheduleProcessValue.addRequested = false;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopProcessvalue/add.html',
                scope: $scope
            });
    ...
    taskScheduleProcessValue.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProcessvalue/', taskScheduleProcessValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            taskScheduleProcessValue.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopProcessvalue/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'shopProcessvalue/', taskScheduleProcessValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'shopProcessvalue/', taskScheduleProcessValue.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    taskScheduleProcessValue.openPreviewModal = function (ProcessId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopProcess/', ProcessId, 'GET').success(function (response1) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'shopProcessvalue/', engine, 'POST').success(function (response2) {
                ...
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/Shop/ShopProcessValue/preview.html',
                    scope: $scope
                });
            }).error(function (data, errCode, c, d) {
                rashaErManage.checkAction(data, errCode);
