app.controller("shopProductCombineController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    shopCombine.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductCombine/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductCombine/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopCombine.openEditContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductCombine/', shopCombine.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductCombine/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopCombine.openItemsModal = function (selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ShopProductCombine/', selectedId, 'GET').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductCombine/addItem.html',
                    scope: $scope
                });
            }
        }).error(function (data, errCode, c, d) {
    ...
    shopCombine.toggleExportForm = function () {
        ...
        shopCombine.addRequested = false;
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/ShopProductCombine/report.html',
            scope: $scope
        });
    }
