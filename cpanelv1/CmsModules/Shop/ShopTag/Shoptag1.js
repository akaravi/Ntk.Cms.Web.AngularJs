...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 shopTag.LinkCategoryTagIdSelector = {
    ...
    // Add New Content
    shopTag.addNewRow = function (frm) {
        if (frm.$invalid)
        {
            rashaErManage.showMessage($filter('translatentk')('form_values_full_have_not_been_entered'));
            return;
    ...
    //upload file
    shopTag.uploadFile = function (index, name) {
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (shopTag.fileIsExist(name)) { // File already exists
                if (confirm('File "' + name + '" already exists! Do you want to replace the new file?')) {
                    //------------ shopTag.replaceFile(name);
                    shopTag.itemClicked(null, shopTag.fileIdToDelete, "file");
                    shopTag.fileTypes = 1;
                    shopTag.fileIdToDelete = shopTag.selectedIndex;
                     // replace the file
    ...
    shopTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/shop/shoptag/report.html',
            scope: $scope
        });
    }
