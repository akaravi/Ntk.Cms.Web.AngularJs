app.controller("shopInvoiceSaleWorkFlowController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    shopInvoiceSaleWorkFlow.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"shopInvoiceSaleWorkFlow/getall", engine, 'POST').success(function (response) {
            ...
            shopInvoiceSaleWorkFlow.gridOptions.fillData(shopInvoiceSaleWorkFlow.ListItems, response.Access);
    ...
    shopInvoiceSaleWorkFlow.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleWorkFlow/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            shopInvoiceSaleWorkFlow.busyIndicator.isActive = false;
            shopInvoiceSaleWorkFlow.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSaleWorkFlow/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    shopInvoiceSaleWorkFlow.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleWorkFlow/', shopInvoiceSaleWorkFlow.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            shopInvoiceSaleWorkFlow.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSaleWorkFlow/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleWorkFlow/', shopInvoiceSaleWorkFlow.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'shopInvoiceSaleWorkFlow/', shopInvoiceSaleWorkFlow.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    shopInvoiceSaleWorkFlow.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Shop/shopInvoiceSaleWorkFlow/report.html',
            scope: $scope
        });
    }
