app.controller("advertisementContractTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    advertisementContractType.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"advertisementcontracttype/getall", advertisementContractType.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            advertisementContractType.gridOptions.fillData(advertisementContractType.ListItems, response.Access);
    ...
    advertisementContractType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'advertisementcontracttype/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            advertisementContractType.busyIndicator.isActive = false;
            advertisementContractType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Advertisement/AdvertisementContractType/add.html',
                scope: $scope
            });
    ...
    advertisementContractType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'advertisementcontracttype/', advertisementContractType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            advertisementContractType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Advertisement/AdvertisementContractType/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'advertisementcontracttype/', advertisementContractType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'advertisementcontracttype/', advertisementContractType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    advertisementContractType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Advertisement/AdvertisementContractType/report.html',
            scope: $scope
        });
    }
