app.controller("advertisementPropertyFavoriteController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $filter) {
    ...
    advertisementPropertyFavorite.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'advertisementPropertyFavorite/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Advertisement/AdvertisementPropertyFavorite/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    advertisementPropertyFavorite.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'advertisementPropertyFavorite/', advertisementPropertyFavorite.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            advertisementPropertyFavorite.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Advertisement/AdvertisementPropertyFavorite/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'AdvertisementPropertyFavorite/', advertisementPropertyFavorite.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'AdvertisementPropertyFavorite/', advertisementPropertyFavorite.selectedItemForDelete.Id, 'DELETE').success(function (res) {
