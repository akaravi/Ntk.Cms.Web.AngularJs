 ...
 biographyTag.LinkCategoryTagIdSelector = {
    ...
    biographyTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"biographytag/getall", biographyTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            biographyTag.gridOptions.fillData(biographyTag.ListItems, response.Access); // Sending Access as an argument
    ...
    biographyTag.addNewCategoryModel = function () {
        biographyTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'biographyCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/biography/biographyCategorytag/add.html',
                        scope: $scope
                    });
                    biographyTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    biographyTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographyCategorytag/', biographyTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            biographyTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            biographyTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/biography/biographyCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    biographyTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'biographyCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    biographyTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'biographyCategorytag/', biographyTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        biographyTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            biographyTag.gridOptions.advancedSearchData.engine.Filters = null;
                            biographyTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    biographyTag.treeConfig.onNodeSelect = function () {
        var node = biographyTag.treeConfig.currentNode;
        biographyTag.showGridComment = false;
        biographyTag.CategoryTagId = node.Id;
    ...
    biographyTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            biographyTag.selectedItem = response.Item;
            biographyTag.selectedItem.LinkCategoryTagId = biographyTag.CategoryTagId;
            //biographyTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographytag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    biographyTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'biographytag/', biographyTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            biographyTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographytag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    // Add New Content
    biographyTag.addNewContent = function (frm) {
        if (frm.$invalid)
        {
            rashaErManage.showMessage($filter('translatentk')('form_values_full_have_not_been_entered'));
            return;
    ...
    //Edit Content
    biographyTag.editContent = function (frm) {
        if (frm.$invalid)
        {
            rashaErManage.showMessage($filter('translatentk')('form_values_full_have_not_been_entered'));
            return;
    ...
    biographyTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                biographyTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"biographytag/", biographyTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    biographyTag.showbusy = false;
                    biographyTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    biographyTag.selectedItemForDelete = response.Item;
                    console.log(biographyTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"biographytag/", biographyTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        biographyTag.categoryBusyIndicator.isActive = false;
                        biographyTag.treeConfig.showbusy = false;
                        biographyTag.showIsBusy = false;
    ...
    biographyTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", biographyTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    //upload file
    biographyTag.uploadFile = function (index, uploadFile) {
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (biographyTag.fileIsExist(uploadFile.name)) { // File already exists
                if (confirm('File "' + uploadFile.name + '" already exists! Do you want to replace the new file?')) {
                    //------------ biographyTag.replaceFile(uploadFile.name);
                    biographyTag.itemClicked(null, biographyTag.fileIdToDelete, "file");
                    biographyTag.fileTypes = 1;
                    biographyTag.fileIdToDelete = biographyTag.selectedIndex;
                     // replace the file
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    biographyTag.FileItem = response.Item;
                    biographyTag.FileItem.FileName = uploadFile.name;
                    biographyTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
                    biographyTag.FileItem.Extension = uploadFile.name.split('.').pop();
                    biographyTag.FileItem.FileSrc = uploadFile.name;
    ...
    biographyTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Biography/Biographytag/report.html',
            scope: $scope
        });
    }
