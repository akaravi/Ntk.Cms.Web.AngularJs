app.controller("biographyContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    biographyContentTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"biographyContenttag/getall", biographyContentTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            biographyContentTag.gridOptions.fillData(biographyContentTag.ListItems , response.Access);
    ...
    biographyContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographyContenttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            biographyContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographycontentTag/add.html',
                scope: $scope
            });
    ...
    biographyContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographyContenttag/',  biographyContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            biographyContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographycontentTag/edit.html',
                scope: $scope
            });
