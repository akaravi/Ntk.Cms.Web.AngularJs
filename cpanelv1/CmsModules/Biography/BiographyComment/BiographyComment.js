app.controller("biographyCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    biographyComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"biographyComment/getall", engine, 'POST').success(function (response) {
            ...
            biographyComment.gridOptions.fillData(biographyComment.ListItems , response.Access);
    ...
    biographyComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographyComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            biographyComment.busyIndicator.isActive = false;
            biographyComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographyComment/add.html',
                scope: $scope
            });
    ...
    biographyComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'biographyComment/', biographyComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            biographyComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/biography/biographyComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'biographyComment/', biographyComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'biographyComment/', biographyComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    biographyComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Biography/biographyComment/report.html',
            scope: $scope
        });
    }
