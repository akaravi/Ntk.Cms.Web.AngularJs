app.controller("pollingOptionCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    pollingOption.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'pollingOption/', pollingOption.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            pollingOption.selectedItem = response.Item;
            $modal.open({
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'pollingOption/', pollingOption.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'pollingOption/', pollingOption.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    pollingOption.openGridConfigModal = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Polling/pollingOption/gridConfig.html',
            scope: $scope
        });
    }
}]);