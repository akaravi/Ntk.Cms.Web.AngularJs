app.controller("pollingLogCtrl", ["$scope", "$http", "ajax", "rashaErManage", "$modal", "$modalStack", "SweetAlert", "$timeout", "$state", '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $window, $filter) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"pollingCategory/", node.Id, "GET").success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+"pollingCategory/", pollingLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        if (res.IsSuccess) {
                            pollingLog.categoryBusyIndicator.isActive = false;
    ...
    pollingLog.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"pollingLog/ViewModel", "", "GET").success(function (response) {
            ...
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingLog/add.html",
                scope: $scope
            });
    ...
    pollingLog.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"pollingLog/", pollingLog.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
            ...
            pollingLog.selectedItem = response.Item;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingLog/edit.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"pollingLog/", pollingLog.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    rashaErManage.checkAction(response);
                    pollingLog.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"pollingLog/", pollingLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        pollingLog.treeConfig.showbusy = false;
                        pollingLog.showIsBusy = false;
    ...
    pollingLog.openOptionModal = function (selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"pollingoption/getall", filterModel, "POST").success(function (response) {
            ...
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingLog/addOption.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingLog.editDeleteOptions = function (frm) {
        ...
        for (var i = 0; i < pollingLog.listComments.length; i++) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"pollingOption/", pollingLog.listComments[i].Id, "GET").success(function (response) {
                ...
                rashaErManage.checkAction(response);
                pollingLog.selectedItemForDelete = response.Item;
                ajax.call(cmsServerConfig.configApiServerPath+"pollingOption/", pollingLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                    pollingLog.treeConfig.showbusy = false;
                    pollingLog.showIsBusy = false;
                    rashaErManage.checkAction(res);
                    if (res.IsSuccess) {
    ...
    pollingLog.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Polling/pollingLog/report.html',
            scope: $scope
        });
    }
