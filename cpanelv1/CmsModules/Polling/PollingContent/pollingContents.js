 ...
 function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $window, $filter) {
    ...
    pollingContent.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingContent/getall", {AccessLoad : true}, "POST").success(function (response) {
            ...
            pollingContent.gridOptions.fillData(pollingContent.ListItems, response.Access);
    ...
    pollingContent.gridOptions.onRowSelected = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/getall", filterModel, "POST").success(function (response) {
            ...
            pollingContent.gridOptions2.fillData(pollingContent.OptionList, response.Access);
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingCategory/ViewModel", "", "GET").success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath + "FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath + "FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Polling/pollingCategory/add.html',
                        scope: $scope
                    });
                    pollingContent.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    pollingContent.openEditCategoryModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath + "pollingCategory/", pollingContent.treeConfig.currentNode.Id, "GET").success(function (response) {
            buttonIsPressed = false;
            ...
            ajax.call(cmsServerConfig.configApiServerPath + "FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath + "FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Polling/pollingCategory/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    pollingContent.Showstatistics = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'pollingContent/', pollingContent.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            pollingContent.selectedItem = response1.Item;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingContent/statistics.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingCategory/", pollingContent.selectedItem, "POST").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    pollingContent.editCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingCategory/", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = true;
            //pollingContent.showbusy = false;
    ...
    pollingContent.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingcontent/ViewModel", "", "GET").success(function (response) {
            rashaErManage.checkAction(response);
            pollingContent.selectedItem = response.Item;
            pollingContent.selectedItem.LinkCategoryId = node.Id;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingContent/add.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingcontent/", pollingContent.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
            ...
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingContent/edit.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.addNewContent = function (frm) {
        ...
        pollingContent.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath + "pollingcontent/", pollingContent.selectedItem, "POST").success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    pollingContent.editContent = function (frm) {
        ...
        pollingContent.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath + "pollingcontent/", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    pollingContent.updateContentOption = function (optionIndex) {
        if ($("#option" + optionIndex).is('[readonly]')) { // Update is pressed
            ...
            ajax.call(cmsServerConfig.configApiServerPath + "pollingOption/", pollingContent.OptionList[optionIndex].Id, "GET").success(function (response) {
                rashaErManage.checkAction(response);
                ...
                ajax.call(cmsServerConfig.configApiServerPath + "pollingOption/", pollingContent.selectedItemForUpdate, "PUT").success(function (res) {
                    rashaErManage.checkAction(res);
                }).error(function (data2, errCode2, c2, d2) {
                    rashaErManage.checkAction(data2);
                    pollingContent.treeConfig.showbusy = false;
    ...
    pollingContent.openOptionModal = function (selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/getall", filterModel, "POST").success(function (response) {
            ...
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingContent/addOption.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.editDeleteOptions = function (frm) {
        ...
        for (var i = 0; i < pollingContent.listComments.length; i++) {
            ajax.call(cmsServerConfig.configApiServerPath + "pollingOption/", pollingContent.listComments[i].Id, "GET").success(function (response) {
                pollingContent.showbusy = false;
                pollingContent.showIsBusy = false;
                rashaErManage.checkAction(response);
                pollingContent.selectedItemForDelete = response.Item;
        ...
        pollingContent.addRequested = true;
        for (var i = 0; i < pollingContent.OptionList.length; i++) {
            ajax.call(cmsServerConfig.configApiServerPath + 'pollingOption/', pollingContent.OptionList[i], 'POST').success(function (response) {
                pollingContent.addRequested = false;
                pollingContent.contentBusyIndicator.isActive = false;
                rashaErManage.checkAction(response);
                if (response.IsSuccess) {
    ...
    pollingContent.openAddOptionModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/ViewModel", "", "GET").success(function (response) {
            ...
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingOption/add.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.addNewOption = function (frm, addtoGrid) {
        ...
        pollingContent.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/", pollingContent.selectedItem, "POST").success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                if (addtoGrid) {
    ...
    pollingContent.openEditOptionModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/", pollingContent.gridOptions2.selectedRow.item.Id, "GET").success(function (response) {
            ...
            //     pollingContent.ExpireDate.defaultDate = "2018-11-01 23:59:59.997";
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/Polling/pollingOption/edit.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    pollingContent.editOption = function (frm) {
        ...
        pollingContent.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    pollingContent.deleteContentOption = function (index) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), "آیا می خواهید این گزینه را حذف کنید؟", function (isConfirmed) {
            if (isConfirmed) {
                pollingContent.addOptionBusyIndicator = true;
                ajax.call(cmsServerConfig.configApiServerPath + "pollingoption/", pollingContent.OptionList[index].Id, "GET").success(function (response) {
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (pollingContent.fileIsExist(uploadFile.name)) { // File already exists
                ...
                ajax.call(cmsServerConfig.configApiServerPath + "FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    pollingContent.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
                    ...
                    var result = 0;
                    ajax.call(cmsServerConfig.configApiServerPath + "FileContent/", pollingContent.FileItem, 'POST').success(function (response) {
                        if (response.IsSuccess) {
                            pollingContent.FileItem = response.Item;
                            pollingContent.showSuccessIcon();
                            $("#save-icon" + index).removeClass("fa-save");
    ...
    pollingContent.toggleExportForm = function (options) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Polling/PollingContent/report.html',
            scope: $scope
        });
    }
