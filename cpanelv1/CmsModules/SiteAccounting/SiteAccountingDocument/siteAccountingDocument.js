app.controller("siteSccountingDocumentController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    ServiceComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ServiceComment/getall", engine, 'POST').success(function (response) {
            ...
            ServiceComment.gridOptions.fillData(ServiceComment.ListItems , response.Access);
    ...
    ServiceComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ServiceComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ServiceComment.busyIndicator.isActive = false;
            ServiceComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Service/ServiceComment/add.html',
                scope: $scope
            });
    ...
    ServiceComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ServiceComment/', ServiceComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ServiceComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Service/ServiceComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ServiceComment/', ServiceComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ServiceComment/', ServiceComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    ServiceComment.gridOptions = {
        columns: [
            ...
            { name: 'ServiceContent.Title', displayName: 'انتخاب محتوا', sortable: true, type: "link" ,displayForce:true},
        ],
        data: {},
        multiSelect: false,
        advancedSearchData: {
