app.controller("siteAccountingDocumentDetailTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    siteAccDocumentDetailType.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"siteAccDocumentDetailType/getall", engine, 'POST').success(function (response) {
            ...
            siteAccDocumentDetailType.gridOptions.fillData(siteAccDocumentDetailType.ListItems, response.Access);
    ...
    siteAccDocumentDetailType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'siteAccountingDocumentDetailType/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            siteAccDocumentDetailType.busyIndicator.isActive = false;
            siteAccDocumentDetailType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Service/SiteAccountingDocumentDetailType/add.html',
                scope: $scope
            });
    ...
    siteAccDocumentDetailType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'serviceTag/', siteAccDocumentDetailType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            siteAccDocumentDetailType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Service/siteAccountingDocumentDetailType/edit.html',
                scope: $scope
            });
    ...
    siteAccDocumentDetailType.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'siteAccDocumentDetailType/', siteAccDocumentDetailType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    //rashaErManage.checkAction(response);
                    siteAccDocumentDetailType.selectedItemForDelete = response.Item;
                    console.log(siteAccDocumentDetailType.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'siteAccountingDocumentDetailType/', siteAccDocumentDetailType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
