app.controller("memberInfoController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $state, $window, $filter) {
    ...
    memberInfo.openViewModal= function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Application/ApplicationMemberInfo/view.html',
            scope: $scope
        });
    }
    memberInfo.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationmemberInfo/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Application/ApplicationMemberInfo/add.html',
                scope: $scope
            });
    ...
    memberInfo.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationMemberInfo/', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            memberInfo.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Application/ApplicationMemberInfo/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationMemberInfo/', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationMemberInfo/', memberInfo.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    memberInfo.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Application/ApplicationMemberInfo/report.html',
            scope: $scope
        });
    }
    ...
    memberInfo.openSendToModal = function (selectedIndex, selected) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Application/ApplicationMemberInfo/sendNotificationModal.html',
            scope: $scope
        });
    }
