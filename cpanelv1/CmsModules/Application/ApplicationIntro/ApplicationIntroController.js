app.controller("applicationIntroController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $state, $window, $filter) {
    ...
    applicationIntro.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Application/ApplicationIntro/add.html',
                scope: $scope
            });
    ...
    applicationIntro.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/', applicationIntro.selectedItem, 'POST').success(function (response) {
            applicationIntro.addRequested = false;
            applicationIntro.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    applicationIntro.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/', applicationIntro.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            applicationIntro.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Application/ApplicationIntro/edit.html',
                scope: $scope
            });
    ...
    applicationIntro.editRow = function (frm) {
        ...
        applicationIntro.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/', applicationIntro.selectedItem, "PUT").success(function (response) {
            applicationIntro.addRequested = true;
            rashaErManage.checkAction(response);
            applicationIntro.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/', applicationIntro.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'ApplicationIntro/', applicationIntro.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    applicationIntro.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Application/ApplicationIntro/report.html',
            scope: $scope
        });
    }
