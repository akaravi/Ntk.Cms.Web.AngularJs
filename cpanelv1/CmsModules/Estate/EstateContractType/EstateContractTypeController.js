app.controller("estateContractTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    estateContractType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'estatecontracttype/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Estate/EstateContractType/add.html',
                scope: $scope
            });
    ...
    estateContractType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'estatecontracttype/', estateContractType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            estateContractType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Estate/EstateContractType/edit.html',
                scope: $scope
            });
    ...
    estateContractType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Estate/EstateContractType/report.html',
            scope: $scope
        });
    }
