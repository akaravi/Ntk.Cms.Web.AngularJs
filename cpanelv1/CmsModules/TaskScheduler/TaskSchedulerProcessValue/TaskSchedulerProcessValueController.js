app.controller("taskSchedulerProcessValueController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$stateParams', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $stateParams, $builder, $state, $filter) {
    ...
    if ($stateParams.appid == null || $stateParams.appid <= 0 || $stateParams.sourceid == null || $stateParams.sourceid <= 0)
        taskScheduleProcessValue.changeState("taskschedulerapplication");
    ...
    taskScheduleProcessValue.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/TaskScheduler/taskschedulerProcessvalue/add.html',
                scope: $scope
            });
    ...
    taskScheduleProcessValue.addNewRow = function (frm) {
        ...
        taskScheduleProcessValue.busyIndicator.isActive = true;
        taskScheduleProcessValue.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.selectedItem, 'POST').success(function (response) {
            taskScheduleProcessValue.addRequested = false;
            taskScheduleProcessValue.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    taskScheduleProcessValue.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            taskScheduleProcessValue.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/TaskScheduler/taskschedulerProcessvalue/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    taskScheduleProcessValue.editRow = function (frm) {
        ...
        taskScheduleProcessValue.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.selectedItem, "PUT").success(function (response) {
            taskScheduleProcessValue.addRequested = true;
            rashaErManage.checkAction(response);
            taskScheduleProcessValue.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', taskScheduleProcessValue.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    taskScheduleProcessValue.openPreviewModal = function (ProcessId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcess/', ProcessId, 'GET').success(function (response1) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'taskschedulerProcessvalue/', engine, 'POST').success(function (response2) {
                ...
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/TaskScheduler/TaskSchedulerProcessValue/preview.html',
                    scope: $scope
                });
            }).error(function (data, errCode, c, d) {
                rashaErManage.checkAction(data, errCode);
