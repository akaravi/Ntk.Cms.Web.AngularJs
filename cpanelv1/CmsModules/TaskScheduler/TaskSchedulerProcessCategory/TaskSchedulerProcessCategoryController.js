app.controller("taskSchedulerProcessCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$builder', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $builder, $filter) {
   ...
   taskSchedulerProcessCategory.filePickerMainImage = {
    ...
    taskSchedulerProcessCategory.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"TaskSchedulerProcessCategory/getall", taskSchedulerProcessCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            taskSchedulerProcessCategory.gridOptions.fillData(taskSchedulerProcessCategory.ListItems, response.Access);
 ...
 //     ajax.call(cmsServerConfig.configApiServerPath+'TaskSchedulerProcessCategory/ViewModel', "", 'GET').success(function (response) {
 //         buttonIsPressed = false;
 //         rashaErManage.checkAction(response);
 //         taskSchedulerProcessCategory.busyIndicator.isActive = false;
 //         taskSchedulerProcessCategory.selectedItem = response.Item;
 //         $modal.open({
 //             templateUrl: 'cpanelv1/CmsModules/TaskScheduler/TaskSchedulerProcessCategory/add.html',
 //             scope: $scope
 //         });
 //
 //     }).error(function (data, errCode, c, d) {
...
//   }
    ...
    taskSchedulerProcessCategory.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'TaskSchedulerProcessCategory/', taskSchedulerProcessCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            taskSchedulerProcessCategory.selectedItem = response.Item;
         ...
         if (response.Item.LinkMainImageId != null) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/TaskScheduler/TaskSchedulerProcessCategory/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'TaskSchedulerProcessCategory/', taskSchedulerProcessCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'TaskSchedulerProcessCategory/', taskSchedulerProcessCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
          ...
          ajax
            ...
            .success(function(response) {
              taskSchedulerProcessCategory.FileItem = response.Item;
                ...
                taskSchedulerProcessCategory.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
