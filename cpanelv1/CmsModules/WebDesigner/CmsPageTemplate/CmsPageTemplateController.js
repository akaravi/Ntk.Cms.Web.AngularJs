app.controller("cmsPageTemplateGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsPageTemplategrd.init = function() {
        cmsPageTemplategrd.gridOptions.advancedSearchData.engine.AccessLoad = true;
        ajax.call(cmsServerConfig.configApiServerPath + "WebDesignerMainPageTemplate/getall", cmsPageTemplategrd.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
            ...
            cmsPageTemplategrd.gridOptions.fillData(cmsPageTemplategrd.ListItems, response.Access);
    ...
    cmsPageTemplategrd.openAddModal = function() {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'WebDesignerMainPageTemplate/ViewModel', '', 'GET').success(function(response) {
            rashaErManage.checkAction(response);
    ...
    cmsPageTemplategrd.openEditModal = function() {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'WebDesignerMainPageTemplate/', cmsPageTemplategrd.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            cmsPageTemplategrd.selectedItem = response.Item;
            $modal.open({
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function(isConfirmed) {
            if (isConfirmed) {
                ajax.call(cmsServerConfig.configApiServerPath + 'WebDesignerMainPageTemplate/', cmsPageTemplategrd.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'WebDesignerMainPageTemplate/', cmsPageTemplategrd.selectedItemForDelete.Id, 'DELETE').success(function(res) {
