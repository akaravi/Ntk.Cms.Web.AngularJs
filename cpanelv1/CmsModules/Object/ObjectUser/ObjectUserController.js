...
// Open Add Modal property
    objectUser.openAddPropertyModal = function (PropertyTypeId,UserId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'objectproperty/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/addValue.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    objectUser.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/object/objectUser/modalMenu.html",
            scope: $scope
        });
    }
    //#help//download file
...
//objectUser.showlistHistory=false;
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/getall", engine, "POST").success(function (response) {
            ...
            rashaErManage.checkAction(response);
            objectUser.gridOptionsfile.fillData(objectUser.listComments, response.Access);
    ...
    objectUser.addNewCategoryModel = function () {
        ...
        objectUser.addRequested = false;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'objectGroup/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            objectUser.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    objectUser.openEditCategoryModel = function () {
        ...
        objectUser.categoryBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'objectGroup/', objectUser.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            objectUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            objectUser.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    objectUser.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                objectUser.categoryBusyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'objectGroup/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
    ...
    objectUser.openAddModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'objectuser/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            objectUser.selectedItem = response.Item;
            if (node.Id != 0 || node.Id) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/addUser.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
//#help فرم اتصال عضو به سایت
    objectUser.openAddobjectinSiteModal= function () {
         ...
         $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/addUserinSite.html',
                scope: $scope
            });
    }
...
//#help اتصال عضو به سایت
    ...
    objectUser.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'objectuser/', objectUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/object/objectUser/editUser.html',
                scope: $scope
            });
    ...
    objectUser.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'objectUser/', objectUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    objectUser.selectedItemForDelete = response.Item;
    ...
    objectUser.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/object/objectUser/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    objectUser.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
   ...
   //Export Report 
    ...
    objectUser.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/object/objectUser/report.html',
            scope: $scope
        });
    }
