app.controller("imgGalleryCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    imgGalleryComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ImageGalleryComment/getall", engine, 'POST').success(function (response) {
            ...
            imgGalleryComment.gridOptions.fillData(imgGalleryComment.ListItems , response.Access);
    ...
    imgGalleryComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            imgGalleryComment.busyIndicator.isActive = false;
            imgGalleryComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryComment/add.html',
                scope: $scope
            });
    ...
    imgGalleryComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryComment/', imgGalleryComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            imgGalleryComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryComment/', imgGalleryComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryComment/', imgGalleryComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    imgGalleryComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryComment/report.html',
            scope: $scope
        });
    }
