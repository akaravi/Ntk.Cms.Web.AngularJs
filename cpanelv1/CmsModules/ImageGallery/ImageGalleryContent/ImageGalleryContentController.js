app.controller("imageGalleryCtrl", ["$scope", "$state", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', '$timeout', '$window', '$stateParams', '$filter', function ($scope, $state,$http, ajax, rashaErManage, $modal, $modalStack, $timeout, $window, $stateParams, $filter) {
    ...
    imgGallery.openUpload = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGallery/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            imgGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/cmsImageGallery/upload.html',
                scope: $scope,
                size: 'lg'
            });
        }).error(function (data, errCode, c, d) {
...
imgGallery.LinkCategoryIdSelector = {
    ...
    imgGallery.openNewFolder = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            imgGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/cmsImageGallery/folder.html',
                scope: $scope
            });
    ...
    imgGallery.getGalleriesByCategory = function (id) {
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGallery/', id, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    imgGallery.Showstatistics = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryContent/', imgGallery.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            imgGallery.selectedItem = response1.Item;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/ImageGallery/ImageGalleryContent/statistics.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    imgGallery.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'imagegallerycontent/', imgGallery.selectedItem, 'POST').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                if (imgGallery.selectedItem.ContentTags && imgGallery.selectedItem.ContentTags.length > 0) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryCategory/add.html',
                        scope: $scope
                    });
                    imgGallery.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    imgGallery.openEditCategoryModal = function () {
        ...
        imgGallery.addRequested = true;
        imgGallery.treeBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryCategory/', imgGallery.treeConfig.currentNode.Id, 'GET').success(function (response) {
            imgGallery.addRequested = false;
            imgGallery.treeBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            imgGallery.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryCategory/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    imgGallery.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryCategory/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    imgGallery.selectedItemForDelete = response.Item;
                    console.log(imgGallery.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryCategory/', imgGallery.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        imgGallery.treeBusyIndicator.isActive = false;
                        imgGallery.addRequested = true;
                        if (res.IsSuccess) {
    ...
    imgGallery.openEditModal = function () {
        ...
        imgGallery.modalTitle = 'ویرایش';
        ajax.call(cmsServerConfig.configApiServerPath+'ImageGalleryContent/', imgGallery.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            imgGallery.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryContent/edit.html',
                scope: $scope
            });
            //Load tagsInput
    ...
    imgGallery.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"imagegallerycontent/DeleteFilterModel", deleteFilterModel.Id, 'DELETE').success(function (res) {
    ...
    imgGallery.gridOptions.advancedSearchData.engine.Filters = [];
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ImageGalleryContent/getall", imgGallery.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            imgGallery.ListItems = response.ListItems;
            imgGallery.fetchImageURLs(imgGallery.ListItems);
            //imgGallery.gridOptions.fillData(imgGallery.ListItems, response.Access); // Sending Access as an argument
            imgGallery.gridOptions.currentPageNumber = response.CurrentPageNumber;
            imgGallery.gridOptions.totalRowCount = response.TotalRowCount;
            imgGallery.gridOptions.rowPerPage = response.RowPerPage;
    ...
    imgGallery.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'imagegallerycontent/ViewModel', "", 'GET').success(function (response) {
            imgGallery.addRequested = false;
            imgGallery.treeBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            imgGallery.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryContent/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    imgGallery.openAddFromFolderModal = function (dirSelectable) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                ...
                Array.prototype.push.apply(imgGallery.dataForTheTree, response2.ListItems);
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryContent/addFolder.html',
                    scope: $scope
                });
                imgGallery.addRequested = false;
            }).error(function (data, errCode, c, d) {
...
//#help similar & otherinfo
   ...
   imgGallery.editOtherInfo = function(y) {
    ...
    imgGallery.addFromFolder = function () {
        ...
        imgGallery.addRequested = true;
        if (imgGallery.treeOptions.dirSelectable) {
            ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/"+ selectedFolder.Id,"", 'GET').success(function (response) {
    ...
    imgGallery.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ImageGallery/ImageGalleryContent/report.html',
            scope: $scope
        });
    }
