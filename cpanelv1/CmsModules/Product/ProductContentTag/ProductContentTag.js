app.controller("productContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    productContentTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ProductContenttag/getall", productContentTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            productContentTag.gridOptions.fillData(productContentTag.ListItems , response.Access);
    ...
    productContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            productContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Product/ProductcontentTag/add.html',
                scope: $scope
            });
    ...
    productContentTag.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/', productContentTag.selectedItem , 'POST').success(function (response) {
            productContentTag.addRequested = false;
            productContentTag.busyIndicator.isActive = false;
    ...
    productContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/',  productContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            productContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Product/ProductcontentTag/edit.html',
                scope: $scope
            });
    ...
    productContentTag.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/',  productContentTag.selectedItem , "PUT").success(function (response) {
            productContentTag.addRequested = true;
            rashaErManage.checkAction(response);
            productContentTag.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/',  productContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ProductContenttag/',  productContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
