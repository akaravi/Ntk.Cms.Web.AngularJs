...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 productTag.LinkCategoryTagIdSelector = {
    ...
    productTag.init = function () {
        ...
        productTag.gridOptions.advancedSearchData.engine.AccessLoad = true;
        ajax.call(cmsServerConfig.configApiServerPath+"producttag/getall", productTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            productTag.gridOptions.fillData(productTag.ListItems, response.Access); // Sending Access as an argument
    ...
    productTag.addNewCategoryModel = function () {
        productTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'productCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/product/productCategorytag/add.html',
                        scope: $scope
                    });
                    productTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    productTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'productCategorytag/', productTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            productTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            productTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/product/productCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    productTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'productCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    productTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'productCategorytag/', productTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        productTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            productTag.gridOptions.advancedSearchData.engine.Filters = null;
                            productTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    productTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'producttag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            productTag.selectedItem = response.Item;
            productTag.selectedItem.LinkCategoryTagId = productTag.CategoryTagId;
            //productTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/product/producttag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    productTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'producttag/', productTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            productTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/product/producttag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    productTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                productTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"producttag/", productTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    productTag.showbusy = false;
                    productTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    productTag.selectedItemForDelete = response.Item;
                    console.log(productTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"producttag/", productTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        productTag.categoryBusyIndicator.isActive = false;
                        productTag.treeConfig.showbusy = false;
                        productTag.showIsBusy = false;
    ...
    productTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", productTag.fileIdToDelete, 'GET').success(function (response1) {
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    productTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    productTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/product/producttag/report.html',
            scope: $scope
        });
    }
