app.controller("ProductCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    ProductComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductComment/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Product/ProductComment/add.html',
                scope: $scope
            });
    ...
    ProductComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ProductComment/', ProductComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            ProductComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Product/ProductComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ProductComment/', ProductComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ProductComment/', ProductComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
