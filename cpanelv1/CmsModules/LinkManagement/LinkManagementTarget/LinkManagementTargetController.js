app.controller("linkManagementTargetController", ["$scope", "$stateParams", "$state", "$rootScope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function($scope, $stateParams, $state, $rootScope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    linkManagementTarget.init = function() {
        ...
        //ajax.call(cmsServerConfig.configApiServerPath+"linkManagementTag/ViewModel", "", 'GET').success(function (response) {    //Get a ViewModel for BiographyTag
        //    linkManagementTarget.ModuleTag = response.Item;
        //}).error(function (data, errCode, c, d) {
        //    console.log(data);
        //});
        //ajax.call(cmsServerConfig.configApiServerPath+"linkManagementContentTag/ViewModel", "", 'GET').success(function (response) { //Get a ViewModel for BiographyContentTag
    ...
    //            //linkManagementTarget.gridOptions.Access = response.Access; // دسترسی ها نمایش
    //            linkManagementTarget.gridContentOptions.fillData(linkManagementTarget.listComments, response.Access);
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (linkManagementTarget.fileIsExist(uploadFile.name)) { // File already exists
                ...
                ajax.call(cmsServerConfig.configApiServerPath + "FileContent/ViewModel", "", 'GET').success(function(response) {
                    ...
                    linkManagementTarget.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    linkManagementTarget.toggleExportForm = function() {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/linkManagement/LinkManagementTarget/report.html',
            scope: $scope
        });
    }
