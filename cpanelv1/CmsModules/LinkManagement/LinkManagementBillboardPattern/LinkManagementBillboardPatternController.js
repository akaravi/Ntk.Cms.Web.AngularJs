app.controller("linkManagementBillboardPatternController", ["$scope", "$state", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $state, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    linkManagementBillboardPattern.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"linkManagementBillboardPattern/getall", engine, 'POST').success(function (response) {
            ...
            linkManagementBillboardPattern.gridOptions.fillData(linkManagementBillboardPattern.ListItems , response.Access);
    ...
    linkManagementBillboardPattern.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'linkManagementBillboardPattern/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            linkManagementBillboardPattern.busyIndicator.isActive = false;
            linkManagementBillboardPattern.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/LinkManagement/linkManagementBillboardPattern/add.html',
                scope: $scope
            });
    ...
    linkManagementBillboardPattern.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'linkManagementBillboardPattern/', linkManagementBillboardPattern.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/LinkManagement/linkManagementBillboardPattern/edit.html',
                scope: $scope
            });
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            if (linkManagementBillboardPattern.fileIsExist(uploadFile.name)) { // File already exists
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    linkManagementBillboardPattern.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    linkManagementBillboardPattern.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/LinkManagement/linkManagementBillboardPattern/report.html',
            scope: $scope
        });
    }
