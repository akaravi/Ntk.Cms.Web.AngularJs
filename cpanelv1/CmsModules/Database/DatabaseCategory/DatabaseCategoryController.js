app.controller("dbCategoryCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    dbCategory.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'dbCategory/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/dbCategory/add.html',
                scope: $scope
            });
    ...
    dbCategory.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'dbCategory/', dbCategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            dbCategory.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Article/dbCategory/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'dbCategory/',  dbCategory.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'dbCategory/', dbCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        dbCategory.busyIndicator.isActive = false;
