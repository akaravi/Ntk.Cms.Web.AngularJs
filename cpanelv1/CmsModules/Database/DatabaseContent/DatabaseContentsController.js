app.controller("dbContentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $filter) {
    ...
    dbContent.addMenu = function () {
        $modal.open({
            templateUrl: "cpanelv1/CmsModules/Database/DBContent/modalMenu.html",
            scope: $scope
        });
    }
    ...
    dbContent.addNewCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DBCategory/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Database/DBCategory/add.html',
                scope: $scope
            });
    ...
    dbContent.EditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DBCategory/', dbContent.treeConfig.currentNode.Id, 'GET').success(function (response) {
            ...
            dbContent.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Database/dbCategory/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DBCategory/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DBCategory/', dbContent.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    dbContent.addNewContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'dbContent/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Database/dbContent/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    dbContent.editContentModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'dbContent/', dbContent.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            ...
            dbContent.filePickerFiles.fileId = null;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Database/DatabaseContent/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"dbContent/", dbContent.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    dbContent.selectedItemForDelete = response.Item;
                    console.log(dbContent.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"dbContent/", dbContent.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        dbContent.categoryBusyIndicator.isActive = false;
                        dbContent.treeConfig.showbusy = false;
                        dbContent.showIsBusy = false;
    ...
    dbContent.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Database/DatabaseContent/report.html',
            scope: $scope
        });
    }
