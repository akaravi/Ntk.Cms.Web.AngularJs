app.controller("mvGalleryContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    mvGalleryContentTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MovieGalleryContentTag/getall", mvGalleryContentTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            mvGalleryContentTag.gridOptions.fillData(mvGalleryContentTag.ListItems , response.Access);
    ...
    mvGalleryContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MovieGalleryContentTag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mvGalleryContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MovieGallery/MovieGalleryContentTag/add.html',
                scope: $scope
            });
    ...
    mvGalleryContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MovieGalleryContentTag/',  mvGalleryContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mvGalleryContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MovieGallery/MovieGalleryContentTag/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'MovieGalleryContentTag/',  mvGalleryContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'MovieGalleryContentTag/',  mvGalleryContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
    ...
    mvGalleryContentTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/MovieGallery/MovieGalleryContentTag/report.html',
            scope: $scope
        });
    }
