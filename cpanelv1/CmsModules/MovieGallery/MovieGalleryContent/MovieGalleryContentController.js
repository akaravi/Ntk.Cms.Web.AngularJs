app.controller("movieGalleryCtrl", ["$scope", '$state', "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', '$timeout', '$window', '$stateParams', '$filter', function ($scope,$state, $http, ajax, rashaErManage, $modal, $modalStack, $timeout, $window, $stateParams, $filter) {
    ...
    mvGallery.openUpload = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGallery/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mvGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/movieGallery/cmsmovieGallery/upload.html',
                scope: $scope,
                size: 'lg'
            });
        }).error(function (data, errCode, c, d) {
...
mvGallery.LinkCategoryIdSelector = {
    ...
    mvGallery.openNewFolder = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mvGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/movieGallery/cmsmovieGallery/folder.html',
                scope: $scope
            });
    ...
    mvGallery.getGalleriesByCategory = function (id) {
        ajax.call(cmsServerConfig.configApiServerPath+'movieGallery/', id, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    mvGallery.Showstatistics = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGallerycontent/', mvGallery.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            mvGallery.selectedItem = response1.Item;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/movieGallery/movieGalleryContent/statistics.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    mvGallery.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGallerycontent/', mvGallery.selectedItem, 'POST').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                if (mvGallery.selectedItem.ContentTags && mvGallery.selectedItem.ContentTags.length > 0) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryCategory/add.html',
                        scope: $scope
                    });
                    mvGallery.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    mvGallery.openEditCategoryModal = function () {
        ...
        mvGallery.addRequested = true;
        mvGallery.treeBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryCategory/', mvGallery.treeConfig.currentNode.Id, 'GET').success(function (response) {
            mvGallery.addRequested = false;
            mvGallery.treeBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            mvGallery.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryCategory/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    mvGallery.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryCategory/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    mvGallery.selectedItemForDelete = response.Item;
                    console.log(mvGallery.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryCategory/', mvGallery.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        mvGallery.treeBusyIndicator.isActive = false;
                        mvGallery.addRequested = true;
                        if (res.IsSuccess) {
    ...
    mvGallery.openEditModal = function () {
        ...
        mvGallery.modalTitle = 'ویرایش';
        ajax.call(cmsServerConfig.configApiServerPath+'movieGalleryContent/', mvGallery.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mvGallery.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryContent/edit.html',
                scope: $scope
            });
            //Load tagsInput
    ...
    mvGallery.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'movieGallerycontent/ViewModel', "", 'GET').success(function (response) {
            mvGallery.addRequested = false;
            mvGallery.treeBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            mvGallery.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryContent/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    mvGallery.openAddFromFolderModal = function (dirSelectable) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                ...
                Array.prototype.push.apply(mvGallery.dataForTheTree, response2.ListItems);
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryContent/addFolder.html',
                    scope: $scope
                });
                mvGallery.addRequested = false;
            }).error(function (data, errCode, c, d) {
...
//#help similar & otherinfo
   ...
   mvGallery.editOtherInfo = function(y) {
    ...
    mvGallery.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/movieGallery/movieGalleryContent/report.html',
            scope: $scope
        });
    }
