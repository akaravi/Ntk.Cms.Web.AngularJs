   ...
   /* emailOutBoxReciver.addRequested = false;
    emailOutBoxReciver.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'emailOutBoxReciver/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Email/emailOutBoxReciver/add.html',
                scope: $scope
            });
 ...
 /*   emailOutBoxReciver.openEditModal = function () {
   ...
   /* emailOutBoxReciver.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'emailOutBoxReciver/', emailOutBoxReciver.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'emailOutBoxReciver/', emailOutBoxReciver.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    emailOutBoxReciver.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Email/emailOutBoxReciver/report.html',
            scope: $scope
        });
    }
    ...
    emailOutBoxReciver.openBaseConfigModal = function (selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"emailOutBoxReciver/getonewithjsonformatter", engine, 'POST').success(function (response) {
            ...
            if (response.IsSuccess) {
                emailOutBoxReciver.selectedItem = response.Item;
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/Email/emailOutBoxReciver/preview.html',
                    scope: $scope
                });
                $builder.removeAllFormObject('default');
                //var customizeValue = JSON.parse(response.Item.PublicConfigJsonValues);
