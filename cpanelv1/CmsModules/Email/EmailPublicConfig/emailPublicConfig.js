app.controller("emailPublicConfigCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $builder, $state, $filter) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'EmailPublicConfig/getonewithjsonformatter',  emailPublicConfig.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/GetAll", filterModelParentRootFolders, 'POST').success(function (response1) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) {
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Email/EmailPublicConfig/edit.html',
                        scope: $scope
                    });
                    emailPublicConfig.addRequested = false;
                    emailPublicConfig.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'EmailPublicConfig/', emailPublicConfig.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'EmailPublicConfig/', emailPublicConfig.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    emailPublicConfig.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Email/EmailPublicConfig/report.html',
            scope: $scope
        });
    }
    ...
    emailPublicConfig.openBaseConfigModal = function (selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"EmailPublicConfig/getonewithjsonformatter", selectedId, 'GET').success(function (response) {
            ...
            if (response.IsSuccess) {
                emailPublicConfig.selectedItem = response.Item;
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/Email/EmailPublicConfig/preview.html',
                    scope: $scope
                });
                $builder.removeAllFormObject('default');
                //var customizeValue = JSON.parse(response.Item.PublicConfigJsonValues);
