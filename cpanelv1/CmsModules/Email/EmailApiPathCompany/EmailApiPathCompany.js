app.controller("emailapipathcompanyCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$state', '$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $state, $stateParams, $filter) {
    ...
    emailapipathcompany.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'emailapipathcompany/ViewModel', '', 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            emailapipathcompany.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Email/emailapipathcompany/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    emailapipathcompany.addNewRow = function (frm) {
        ...
        emailapipathcompany.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'emailapipathcompany/', emailapipathcompany.selectedItem, 'POST').success(function (response1) {
    ...
    emailapipathcompany.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'emailapipathcompany/', emailapipathcompany.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            emailapipathcompany.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Email/emailapipathcompany/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    emailapipathcompany.editRow = function (frm) {
        ...
        emailapipathcompany.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'emailapipathcompany/', emailapipathcompany.selectedItem, "PUT").success(function (response) {
            emailapipathcompany.addRequested = true;
            rashaErManage.checkAction(response);
    ...
    emailapipathcompany.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Email/emailapipathcompany/report.html',
            scope: $scope
        });
    }
