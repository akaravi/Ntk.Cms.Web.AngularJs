app.controller("deliveryMemberAbsentController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    deliveryMemberAbsent.init = function () {
        ...
        deliveryMemberAbsent.gridOptions.advancedSearchData.engine.AccessLoad = true;
        ajax.call(cmsServerConfig.configApiServerPath+"DeliveryMemberAbsent/getall", deliveryMemberAbsent.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            deliveryMemberAbsent.gridOptions.fillData(deliveryMemberAbsent.ListItems, response.Access);
    ...
    deliveryMemberAbsent.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberAbsent/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMemberAbsent.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberAbsent/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    deliveryMemberAbsent.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberAbsent/', deliveryMemberAbsent.selectedItem, 'POST').success(function (response) {
            deliveryMemberAbsent.addRequested = false;
            deliveryMemberAbsent.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    deliveryMemberAbsent.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberAbsent/', deliveryMemberAbsent.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMemberAbsent.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberAbsent/edit.html',
                scope: $scope
            });
    ...
    deliveryMemberAbsent.editRow = function (frm) {
        ...
        deliveryMemberAbsent.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberAbsent/', deliveryMemberAbsent.selectedItem, "PUT").success(function (response) {
            deliveryMemberAbsent.addRequested = true;
            rashaErManage.checkAction(response);
            deliveryMemberAbsent.busyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    deliveryMemberAbsent.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberAbsent/setTimes.html',
            scope: $scope
        });
    ...
    deliveryMemberAbsent.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberAbsent/report.html',
            scope: $scope
        });
    }
