app.controller("deliveryMethodController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    deliveryMethod.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"DeliveryMethod/getall", deliveryMethod.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            deliveryMethod.gridOptions.fillData(deliveryMethod.ListItems, response.Access);
    ...
    deliveryMethod.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMethod.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMethod/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    deliveryMethod.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/', deliveryMethod.selectedItem, 'POST').success(function (response) {
            deliveryMethod.addRequested = false;
            deliveryMethod.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    deliveryMethod.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/', deliveryMethod.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMethod.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMethod/edit.html',
                scope: $scope
            });
    ...
    deliveryMethod.editRow = function (frm) {
        ...
        deliveryMethod.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/', deliveryMethod.selectedItem, "PUT").success(function (response) {
            deliveryMethod.addRequested = true;
            rashaErManage.checkAction(response);
            deliveryMethod.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/', deliveryMethod.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMethod/', deliveryMethod.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    deliveryMethod.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMethod/setTimes.html',
            scope: $scope
        });
    ...
    deliveryMethod.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMethod/report.html',
            scope: $scope
        });
    }
