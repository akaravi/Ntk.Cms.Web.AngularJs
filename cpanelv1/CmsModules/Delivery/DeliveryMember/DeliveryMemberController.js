app.controller("deliveryMemberController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    deliveryMember.init = function () {
        ...
        deliveryMember.gridOptions.advancedSearchData.engine.AccessLoad = true;
    ...
    deliveryMember.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMember/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    deliveryMember.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/', deliveryMember.selectedItem, 'POST').success(function (response) {
            deliveryMember.addRequested = false;
            deliveryMember.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    deliveryMember.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/', deliveryMember.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMember/edit.html',
                scope: $scope
            });
    ...
    deliveryMember.editRow = function (frm) {
        ...
        deliveryMember.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/', deliveryMember.selectedItem, "PUT").success(function (response) {
            deliveryMember.addRequested = true;
            rashaErManage.checkAction(response);
            deliveryMember.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/', deliveryMember.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMember/', deliveryMember.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    deliveryMember.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMember/setTimes.html',
            scope: $scope
        });
    ...
    deliveryMember.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMember/report.html',
            scope: $scope
        });
    }
