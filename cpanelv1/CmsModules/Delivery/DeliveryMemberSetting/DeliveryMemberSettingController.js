app.controller("deliveryMemberSettingController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    deliveryMemberSetting.init = function () {
        ...
        deliveryMemberSetting.gridOptions.advancedSearchData.engine.AccessLoad = true;
        ajax.call(cmsServerConfig.configApiServerPath+"DeliveryMemberSetting/getall", deliveryMemberSetting.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            deliveryMemberSetting.gridOptions.fillData(deliveryMemberSetting.ListItems, response.Access);
    ...
    deliveryMemberSetting.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberSetting/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMemberSetting.busyIndicator.isActive = false;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberSetting/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    deliveryMemberSetting.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberSetting/', deliveryMemberSetting.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            deliveryMemberSetting.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberSetting/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberSetting/', deliveryMemberSetting.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DeliveryMemberSetting/', deliveryMemberSetting.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    deliveryMemberSetting.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberSetting/setTimes.html',
            scope: $scope
        });
    ...
    deliveryMemberSetting.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryMemberSetting/report.html',
            scope: $scope
        });
    }
