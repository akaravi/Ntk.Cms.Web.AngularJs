app.controller("deliveryInvoiceController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    deliveryInvoice.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryInvoice/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryInvoice/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    deliveryInvoice.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'DeliveryInvoice/', deliveryInvoice.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryInvoice/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'DeliveryInvoice/', deliveryInvoice.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'DeliveryInvoice/', deliveryInvoice.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    deliveryInvoice.openSetTimesModal = function (dateId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryInvoice/setTimes.html',
            scope: $scope
        });
    ...
    deliveryInvoice.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Delivery/DeliveryInvoice/report.html',
            scope: $scope
        });
    }
