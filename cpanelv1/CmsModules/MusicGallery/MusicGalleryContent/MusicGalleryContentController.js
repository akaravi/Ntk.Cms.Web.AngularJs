app.controller("musicGalleryCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', '$timeout', '$window','$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, $timeout, $window,$stateParams, $filter) {
   ...
   var date = moment().format();
    ...
    mscGallery.openUpload = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGallery/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/cmsMusicGallery/upload.html',
                scope: $scope,
                size: 'lg'
            });
        }).error(function (data, errCode, c, d) {
...
mscGallery.LinkCategoryIdSelector = {
    ...
    mscGallery.openNewFolder = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGallery.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/cmsMusicGallery/folder.html',
                scope: $scope
            });
    ...
    mscGallery.getGalleriesByCategory = function (id) {
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGallery/', id, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    mscGallery.Showstatistics = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGallerycontent/', mscGallery.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            mscGallery.selectedItem = response1.Item;
            $modal.open({
                templateUrl: "cpanelv1/CmsModules/MusicGallery/MusicGalleryContent/statistics.html",
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryCategory/ViewModel', "", 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryCategory/add.html',
                        scope: $scope
                    });
                    mscGallery.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    mscGallery.openEditCategoryModal = function () {
        ...
        mscGallery.treeBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryCategory/', mscGallery.treeConfig.currentNode.Id, 'GET').success(function (response) {
            mscGallery.addRequested = false;
            mscGallery.treeBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            mscGallery.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryCategory/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    mscGallery.addFromFolder = function (frm) {
        ...
        mscGallery.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'FileContent/GetFilesInCategoryId/'+ mscGallery.selectedItem.FolderId,"", 'GET').success(function (response) {
            mscGallery.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    mscGallery.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryCategory/', node.Id, 'GET').success(function (response) {
    ...
    mscGallery.openAddFromFolderModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/GetAll", filterModelParentRootFolders, 'POST').success(function (response1) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) {
                ...
                Array.prototype.push.apply(mscGallery.dataForTheTree, response2.ListItems);
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryContent/addFolder.html',
                    scope: $scope
                });
                mscGallery.addRequested = false;
            }).error(function (data, errCode, c, d) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryContent/ViewModel', '', 'GET').success(function (response) {
            mscGallery.selectedItem = response.Item;
    ...
    mscGallery.openEditModal = function () {
        ...
        mscGallery.modalTitle = 'ویرایش';
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryContent/', mscGallery.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGallery.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryContent/edit.html',
                scope: $scope
            });
            //Load tagsInput
    ...
    mscGallery.selectContent = function (node) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MusicGalleryContent/getall", mscGallery.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            rashaErManage.checkAction(response);
            mscGallery.ListItems = response.ListItems;
            //mscGallery.gridOptions.fillData(mscGallery.ListItems, response.Access); // Sending Access as an argument
            mscGallery.fetchImageURLs(mscGallery.ListItems);
    ...
    mscGallery.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGallerycontent/ViewModel', "", 'GET').success(function (response) {
            ...
            mscGallery.selectedItem.LinkCategoryId = node.Id;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryContent/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    mscGallery.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryContent/report.html',
            scope: $scope
        });
    }
