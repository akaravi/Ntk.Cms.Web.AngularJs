app.controller("mscGalleryCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    mscGalleryComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"MusicGalleryComment/getall", engine, 'POST').success(function (response) {
            ...
            mscGalleryComment.gridOptions.fillData(mscGalleryComment.ListItems , response.Access);
    ...
    mscGalleryComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGalleryComment.busyIndicator.isActive = false;
            mscGalleryComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryComment/add.html',
                scope: $scope
            });
    ...
    mscGalleryComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryComment/', mscGalleryComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGalleryComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryComment/', mscGalleryComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'MusicGalleryComment/', mscGalleryComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    mscGalleryComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/MusicGallery/MusicGalleryComment/report.html',
            scope: $scope
        });
    }
