 ...
 mscGalleryTag.LinkCategoryTagIdSelector = {
    ...
    mscGalleryTag.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"mscGallerytag/getall", mscGalleryTag.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            mscGalleryTag.gridOptions.fillData(mscGalleryTag.ListItems, response.Access); // Sending Access as an argument
    ...
    mscGalleryTag.addNewCategoryModel = function () {
        mscGalleryTag.addRequested = false;
        ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGalleryCategorytag/add.html',
                        scope: $scope
                    });
                    mscGalleryTag.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    mscGalleryTag.openEditCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/', mscGalleryTag.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            mscGalleryTag.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            mscGalleryTag.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGalleryCategorytag/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    mscGalleryTag.addNewCategory = function (frm) {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/', mscGalleryTag.selectedItem, 'POST').success(function (response) {
            buttonIsPressed = false;
            mscGalleryTag.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    mscGalleryTag.editCategory = function (frm) {
        ...
        mscGalleryTag.categoryBusyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/', mscGalleryTag.selectedItem, "PUT").success(function (response) {
            mscGalleryTag.addRequested = true;
    ...
    mscGalleryTag.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/', node.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    mscGalleryTag.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'mscGalleryCategorytag/', mscGalleryTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        mscGalleryTag.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
                            mscGalleryTag.gridOptions.advancedSearchData.engine.Filters = null;
                            mscGalleryTag.gridOptions.advancedSearchData.engine.Filters = [];
    ...
    mscGalleryTag.openAddModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'mscGallerytag/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            mscGalleryTag.selectedItem = response.Item;
            mscGalleryTag.selectedItem.LinkCategoryTagId = mscGalleryTag.CategoryTagId;
            //mscGalleryTag.clearPreviousData();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGallerytag/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    mscGalleryTag.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'mscGallerytag/', mscGalleryTag.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            mscGalleryTag.selectedItem = response1.Item;

            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGallerytag/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    mscGalleryTag.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'mscGallerytag/', mscGalleryTag.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            mscGalleryTag.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    mscGalleryTag.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'mscGallerytag/', mscGalleryTag.selectedItem, "PUT").success(function (response) {
            mscGalleryTag.categoryBusyIndicator.isActive = false;
            mscGalleryTag.addRequested = false;
            mscGalleryTag.treeConfig.showbusy = false;
            mscGalleryTag.showIsBusy = false;
    ...
    mscGalleryTag.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                mscGalleryTag.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"mscGallerytag/", mscGalleryTag.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    mscGalleryTag.showbusy = false;
                    mscGalleryTag.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    mscGalleryTag.selectedItemForDelete = response.Item;
                    console.log(mscGalleryTag.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"mscGallerytag/", mscGalleryTag.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        mscGalleryTag.categoryBusyIndicator.isActive = false;
                        mscGalleryTag.treeConfig.showbusy = false;
                        mscGalleryTag.showIsBusy = false;
    ...
    mscGalleryTag.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGalleryContent/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    mscGalleryTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", mscGalleryTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    mscGalleryTag.saveNewFile = function () {
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", mscGalleryTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    mscGalleryTag.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    mscGalleryTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/mscGallery/mscGallerytag/report.html',
            scope: $scope
        });
    }
