app.controller("jobPropertyDetailController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$location', '$state', '$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $location, $state, $stateParams, $filter) {
    ...
    jobPropertyDetail.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"JobPropertyDetail/EnumUiPropertyDesign", "", 'GET').success(function (response) {
            jobPropertyDetail.UiDesignType = response.ListItems;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
        // ajax.call(cmsServerConfig.configApiServerPath+"JobPropertyDetail/EnumInputDataType", "", 'GET').success(function (response) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"JobPropertyDetailGroup/getall", jobPropertyDetail.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            jobPropertyDetail.groupResultAccess = response.Access;
    ...
    jobPropertyDetail.openAddCategoryModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobPropertyDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPropertyDetailGroup/add.html',
                scope: $scope
            });
    ...
    jobPropertyDetail.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/', jobPropertyDetail.selectedItem, 'POST').success(function (response) {
            jobPropertyDetail.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    jobPropertyDetail.openEditCategoryModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/', jobPropertyDetail.treeConfig.currentNode.Id, 'GET').success(function (response1) {
            jobPropertyDetail.showbusy = false;
            rashaErManage.checkAction(response1);
            jobPropertyDetail.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPropertyDetailGroup/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobPropertyDetail.editCategory = function (frm) {
        ...
        jobPropertyDetail.categorybusyIndicator.isActive = true;
        jobPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/', jobPropertyDetail.selectedItem, "PUT").success(function (response) {
            jobPropertyDetail.addRequested = true;
    ...
    jobPropertyDetail.deleteCategory = function () {
        ...
        if (jobPropertyDetail.ListItems.length == 0) {
            rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
                if (isConfirmed) {
                    jobPropertyDetail.categorybusyIndicator.isActive = true;
                    ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/', node.Id, 'GET').success(function (response) {
                        rashaErManage.checkAction(response);
                        jobPropertyDetail.selectedItemForDelete = response.Item;
                        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetailGroup/', jobPropertyDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    jobPropertyDetail.selectContent = function (node) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"jobPropertyDetail/getall", jobPropertyDetail.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            jobPropertyDetail.gridOptions.fillData(jobPropertyDetail.ListItems, response.Access);
    ...
    jobPropertyDetail.openAddContentModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobPropertyDetail.selectedItem = response.Item;
            jobPropertyDetail.selectedItem.LinkPropertyDetailGroupId = node.Id;
            jobPropertyDetail.onMultipleChoiceChange();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/jobPropertyDetail/add.html',
                scope: $scope
            });
    ...
    jobPropertyDetail.addNewContent = function (frm) {
        ...
        jobPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetail/', jobPropertyDetail.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                // filter text
    ...
    jobPropertyDetail.openEditContentModal = function () {
        ...
        jobPropertyDetail.FieldName = "";
        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetail/', jobPropertyDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            jobPropertyDetail.selectedItem = response1.Item;
            ...
            jobPropertyDetail.onMultipleChoiceChange();
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/jobPropertyDetail/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobPropertyDetail.editContent = function (frm) {
        ...
        jobPropertyDetail.selectedItem.IconFont = $("#iconFont").val(); //Save selected icon name in the model
        jobPropertyDetail.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetail/', jobPropertyDetail.selectedItem, "PUT").success(function (response) {
            // jobPropertyDetail.addRequested = false;
            jobPropertyDetail.treeConfig.showbusy = false;
            jobPropertyDetail.showIsBusy = false;
            rashaErManage.checkAction(response);
    ...
    jobPropertyDetail.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                jobPropertyDetail.showIsBusy = true;
                ajax.call(cmsServerConfig.configApiServerPath+"jobPropertyDetail/", jobPropertyDetail.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    jobPropertyDetail.showbusy = false;
                    jobPropertyDetail.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    jobPropertyDetail.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+"jobPropertyDetail/", jobPropertyDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        jobPropertyDetail.treeConfig.showbusy = false;
                        jobPropertyDetail.showIsBusy = false;
    ...
    jobPropertyDetail.editStepGoDown = function (item, index) {
        ...
        jobPropertyDetail.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetail/', item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            jobPropertyDetail.selectedItem = response1.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetail/', jobPropertyDetail.selectedItem, "PUT").success(function (response2) {
                rashaErManage.checkAction(response2);
                if (response2.IsSuccess) {
                    ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetail/', jobPropertyDetail.ListItems[index + 1].Id, 'GET').success(function (response3) {
                        ...
                        ajax.call(cmsServerConfig.configApiServerPath+'JobPropertyDetail/', jobPropertyDetail.selectedItem, "PUT").success(function (response4) {
    ...
    jobPropertyDetail.editStepGoDownGroup = function (item) {
        ...
        jobPropertyDetail.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetailGroup/', item.Id, 'GET').success(function (response1) {
            rashaErManage.checkAction(response1);
            jobPropertyDetail.selectedItem = response1.Item;
            var temp = response1.Item.ShowInFormOrder;
            jobPropertyDetail.selectedItem.ShowInFormOrder = jobPropertyDetail.treeConfig.Items[index + 1].ShowInFormOrder;
            ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetailGroup/', jobPropertyDetail.selectedItem, "PUT").success(function (response2) {
                rashaErManage.checkAction(response2);
                if (response2.IsSuccess) {
                    ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetailGroup/', jobPropertyDetail.treeConfig.Items[index + 1].Id, 'GET').success(function (response3) {
                        rashaErManage.checkAction(response3);
                        jobPropertyDetail.selectedItem = response3.Item;
                        jobPropertyDetail.selectedItem.ShowInFormOrder = temp;
                        ajax.call(cmsServerConfig.configApiServerPath+'jobPropertyDetailGroup/', jobPropertyDetail.selectedItem, "PUT").success(function (response4) {
                            rashaErManage.checkAction(response4);
                            if (response4.IsSuccess) {
