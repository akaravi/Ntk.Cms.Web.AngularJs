app.controller("jobContractTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    jobContractType.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"jobcontracttype/getall", jobContractType.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            jobContractType.gridOptions.fillData(jobContractType.ListItems, response.Access);
    ...
    jobContractType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobContractType.busyIndicator.isActive = false;
            jobContractType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobContractType/add.html',
                scope: $scope
            });
    ...
    jobContractType.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/', jobContractType.selectedItem, 'POST').success(function (response) {
            jobContractType.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    jobContractType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/', jobContractType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobContractType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobContractType/edit.html',
                scope: $scope
            });
    ...
    jobContractType.editRow = function (frm) {
        ...
        jobContractType.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/', jobContractType.selectedItem, "PUT").success(function (response) {
            jobContractType.addRequested = true;
            rashaErManage.checkAction(response);
            jobContractType.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/', jobContractType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'jobcontracttype/', jobContractType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    jobContractType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/jobContractType/report.html',
            scope: $scope
        });
    }
