app.controller("jobPropertyController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    jobProperty.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"jobproperty/getAllwithalias", jobProperty.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            jobProperty.gridOptions.fillData(jobProperty.ListItems, response.Access);
    ...
    jobProperty.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobproperty/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobProperty/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobProperty.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobproperty/', jobProperty.selectedItem, 'POST').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"jobpropertydetailvalue/ViewModel", "", 'GET').success(function (response1) {
    ...
    jobProperty.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'jobproperty/', parseInt(jobProperty.gridOptions.selectedRow.item.Id), 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            jobProperty.selectedItem = response.Item;
            ...
            //*****************************************************************
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobProperty/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobProperty.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                jobProperty.busyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'jobproperty/', jobProperty.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    jobProperty.selectedItemForDelete = response.Item;
                    ajax.call(cmsServerConfig.configApiServerPath+'jobproperty/', jobProperty.selectedItemForDelete.Id, 'DELETE').success(function (res) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'JobContract/ViewModel', "", 'GET').success(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'JobContract/getall', model, 'POST').success(function (response) {
                    ...
                    jobProperty.contractsListresultAccess = response.Access;
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Job/JobProperty/addContract.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    rashaErManage.checkAction(data, errCode);
    ...
    jobProperty.deleteContract = function (index) {
        jobProperty.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'JobContract/', jobProperty.contractsList[index].Id, 'DELETE').success(function (res) {
            jobProperty.addRequested = false;
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
                jobProperty.contractsList.splice(index, 1);
    ...
    jobProperty.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/JobProperty/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
    ...
    jobProperty.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(cmsServerConfig.configApiServerPath+"FileContent/", jobProperty.fileIdToDelete, 'GET').success(function (response1) {
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    jobProperty.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
    ...
    jobProperty.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/JobProperty/report.html',
            scope: $scope
        });
    }
