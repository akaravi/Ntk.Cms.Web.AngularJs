app.controller("jobCertificateController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    jobCertificate.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcertificate/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobCertificate/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobCertificate.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcertificate/', jobCertificate.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobCertificate/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'jobcertificate/', jobCertificate.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'jobcertificate/', jobCertificate.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    jobCertificate.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/JobCertificate/report.html',
            scope: $scope
        });
    }
