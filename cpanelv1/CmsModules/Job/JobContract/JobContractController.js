app.controller("jobContractController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    jobContract.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontract/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobContract/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobContract.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobcontract/', jobContract.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobContract/edit.html',
                scope: $scope
            });
    ...
    jobContract.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/JobContract/report.html',
            scope: $scope
        });
    }
