app.controller("jobPositionController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    jobPosition.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobposition/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPosition/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobPosition.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobposition/', jobPosition.selectedItem, 'POST').success(function (response) {
            jobPosition.addRequested = false;
            jobPosition.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    jobPosition.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobposition/', jobPosition.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPosition/edit.html',
                scope: $scope
            });
    ...
    jobPosition.editRow = function (frm) {
        ...
        jobPosition.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'jobposition/', jobPosition.selectedItem, "PUT").success(function (response) {
            jobPosition.addRequested = true;
            rashaErManage.checkAction(response);
            jobPosition.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'jobposition/', jobPosition.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'jobposition/', jobPosition.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    jobPosition.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/JobPosition/report.html',
            scope: $scope
        });
    }
