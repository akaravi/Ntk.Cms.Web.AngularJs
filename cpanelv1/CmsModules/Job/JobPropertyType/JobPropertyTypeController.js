app.controller("jobPropertyTypeController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $window, $filter) {
    ...
    jobPropertyType.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"jobpropertytype/getall", jobPropertyType.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            jobPropertyType.gridOptions.fillData(jobPropertyType.ListItems, response.Access);
    ...
    jobPropertyType.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobpropertytype/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobPropertyType.busyIndicator.isActive = false;
            jobPropertyType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPropertyType/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    jobPropertyType.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'jobpropertytype/', jobPropertyType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            jobPropertyType.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Job/JobPropertyType/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'jobpropertytype/', jobPropertyType.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'jobpropertytype/', jobPropertyType.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    jobPropertyType.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Job/jobPropertyType/report.html',
            scope: $scope
        });
    }
