app.controller("bankPaymentPublicConfigController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $builder, $state, $filter) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'bankpaymentpublicconfig/getonewithjsonformatter', engine, 'POST').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/GetAll", filterModelParentRootFolders, 'POST').success(function (response1) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) {
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/BankPayment/BankPaymentPublicConfig/edit.html',
                        scope: $scope
                    });
                    publicConfig.addRequested = false;
                    publicConfig.busyIndicator.isActive = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'bankpaymentpublicconfig/', publicConfig.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'bankpaymentpublicconfig/', publicConfig.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    publicConfig.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/BankPayment/BankPaymentPublicConfig/report.html',
            scope: $scope
        });
    }
