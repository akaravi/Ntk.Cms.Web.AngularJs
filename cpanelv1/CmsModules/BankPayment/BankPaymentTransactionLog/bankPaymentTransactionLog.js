app.controller("bankPaymentTranscLogController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    trancsLog.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"bankpaymenttransactionlog/getall", trancsLog.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            trancsLog.gridOptions.fillData(trancsLog.ListItems, response.Access);
    ...
    trancsLog.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'bankpaymenttransactionlog/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/BankPayment/BankPaymentTransactionLog/add.html',
                scope: $scope
            });
    ...
    trancsLog.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'bankpaymenttransactionlog/', trancsLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            trancsLog.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/BankPayment/BankPaymentTransactionLog/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'bankpaymenttransactionlog/', trancsLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'bankpaymenttransactionlog/', trancsLog.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    trancsLog.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/BankPayment/BankPaymentTransactionLog/report.html',
            scope: $scope
        });
    }
