app.controller("ticketingAnswerController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $filter) {
    ...
    ticketingAnswer.addNewModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'TicketingAnswer/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Ticketing/TicketingAnswer/add.html',
                scope: $scope
            });
    ...
    ticketingAnswer.editmodel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'TicketingAnswer/', ticketingAnswer.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ticketingAnswer.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Ticketing/TicketingAnswer/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'TicketingAnswer/', ticketingAnswer.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'TicketingAnswer/', ticketingAnswer.selectedItemForDelete.Id , 'DELETE').success(function (res) {
