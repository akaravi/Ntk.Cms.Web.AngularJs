app.controller("ticketingFaqController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    ticketingFaq.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ticketingFaq/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Ticketing/ticketingFaq/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
    ...
    ticketingFaq.openEditmodal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ticketingFaq/', ticketingFaq.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            ticketingFaq.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Ticketing/ticketingFaq/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    ticketingFaq.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Ticketing/Faq/report.html',
            scope: $scope
        });
    }
