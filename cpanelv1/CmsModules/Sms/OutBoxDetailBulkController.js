app.controller("outBoxDetailBulkCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBoxDetailBulk.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "smsLogoutBoxDetailBulk/getall", outBoxDetailBulk.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    outBoxDetailBulk.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetailBulk.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetailBulk/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    outBoxDetailBulk.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/', outBoxDetailBulk.selectedItem, 'POST').success(function(response) {
            outBoxDetailBulk.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBoxDetailBulk.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/', outBoxDetailBulk.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetailBulk.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetailBulk/edit.html',
                scope: $scope
            });
    ...
    outBoxDetailBulk.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/', outBoxDetailBulk.selectedItem, "PUT").success(function(response) {
            outBoxDetailBulk.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBoxDetailBulk.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/', outBoxDetailBulk.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulk/', outBoxDetailBulk.selectedItemForDelete.Id, 'DELETE').success(function(res) {
