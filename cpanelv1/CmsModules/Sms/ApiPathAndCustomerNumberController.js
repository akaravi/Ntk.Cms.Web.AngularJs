app.controller("apiPathCustomerCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$rootScope', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $rootScope, $state, $filter) {
    ...
    apiPathCustomer.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "SmsMainApiPathAndCustomerNumber/getall", apiPathCustomer.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    apiPathCustomer.addNewModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainApiPathAndCustomerNumber/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            apiPathCustomer.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Sms/ApiPathAndCustomerNumber/add.html',
                scope: $scope
            });
    ...
    apiPathCustomer.editOpenModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'SmsMainApiPathAndCustomerNumber/', apiPathCustomer.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            apiPathCustomer.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Sms/ApiPathAndCustomerNumber/edit.html',
                scope: $scope
            });
    ...
    apiPathCustomer.editRow = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainApiPathAndCustomerNumber/', apiPathCustomer.selectedItem, "PUT").success(function(response) {
            apiPathCustomer.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    apiPathCustomer.addNewRow = function () {
        ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainApiPathAndCustomerNumber/', apiPathCustomer.selectedItem, 'POST').success(function(response) {
            apiPathCustomer.addRequested = false;
            apiPathCustomer.closeModal();
            apiPathCustomer.init();
            rashaErManage.checkAction(response);
    ...
    apiPathCustomer.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainApiPathAndCustomerNumber/', node.Id, 'GET').success(function(response) {
                    rashaErManage.checkAction(response);
                    apiPathCustomer.selectedItemForDelete = response.Item;
                    console.log(apiPathCustomer.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainApiPathAndCustomerNumber/', apiPathCustomer.selectedItemForDelete.Id, 'DELETE').success(function(res) {
                        console.log(res);
                        if (res.IsSuccess) {
