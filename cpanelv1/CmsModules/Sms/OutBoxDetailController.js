app.controller("outBoxDetailCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBoxDetail.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "SmsLogOutBoxDetail/getall", outBoxDetail.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    outBoxDetail.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetail/add.html',
                scope: $scope
            });
    ...
    outBoxDetail.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/', outBoxDetail.selectedItem, 'POST').success(function(response) {
            outBoxDetail.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBoxDetail.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/', outBoxDetail.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetail.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetail/edit.html',
                scope: $scope
            });
    ...
    outBoxDetail.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/', outBoxDetail.selectedItem, "PUT").success(function(response) {
            outBoxDetail.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBoxDetail.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/', outBoxDetail.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetail/', outBoxDetail.selectedItemForDelete.Id, 'DELETE').success(function(res) {
