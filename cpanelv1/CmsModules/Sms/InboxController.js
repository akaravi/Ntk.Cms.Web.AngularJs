app.controller("inboxCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    var inbox = this;
    inbox.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "smsloginbox/getall", inbox.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    inbox.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            inbox.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/inbox/add.html',
                scope: $scope
            });
    ...
    inbox.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/', inbox.selectedItem, 'POST').success(function(response) {
            inbox.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    inbox.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/', inbox.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            inbox.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/inbox/edit.html',
                scope: $scope
            });
    ...
    inbox.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/', inbox.selectedItem, "PUT").success(function(response) {
            inbox.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                inbox.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/', inbox.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginbox/', inbox.selectedItemForDelete.Id, 'DELETE').success(function(res) {
