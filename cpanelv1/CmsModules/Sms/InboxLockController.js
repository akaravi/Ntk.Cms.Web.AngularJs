app.controller("inboxLockCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    inboxLock.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "smsLogInboxLock/getall", inboxLock.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    inboxLock.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            inboxLock.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/inboxLock/add.html',
                scope: $scope
            });
    ...
    inboxLock.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/', inboxLock.selectedItem, 'POST').success(function(response) {
            inboxLock.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    inboxLock.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/', inboxLock.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            inboxLock.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/inboxLock/edit.html',
                scope: $scope
            });
    ...
    inboxLock.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/', inboxLock.selectedItem, "PUT").success(function(response) {
            inboxLock.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                inboxLock.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/', inboxLock.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLoginboxLock/', inboxLock.selectedItemForDelete.Id, 'DELETE').success(function(res) {
