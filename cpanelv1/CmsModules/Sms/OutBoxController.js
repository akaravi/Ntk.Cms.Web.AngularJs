app.controller("outBoxCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBox.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "SmsLogoutbox/getall", outBox.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    outBox.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBox.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBox/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    outBox.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/', outBox.selectedItem, 'POST').success(function(response) {
            outBox.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBox.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/', outBox.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBox.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBox/edit.html',
                scope: $scope
            });
    ...
    outBox.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/', outBox.selectedItem, "PUT").success(function(response) {
            outBox.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBox.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/', outBox.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBox/', outBox.selectedItemForDelete.Id, 'DELETE').success(function(res) {
