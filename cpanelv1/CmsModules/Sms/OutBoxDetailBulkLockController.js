app.controller("outBoxDetailBulkLockCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBoxDetailBulkLock.init = function () {
        ajax.call(cmsServerConfig.configApiServerPath + "smsLogoutBoxDetailBulkLock/getall", outBoxDetailBulkLock.gridOptions.advancedSearchData.engine, 'POST').success(function(response) {
    ...
    outBoxDetailBulkLock.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/ViewModel', "", 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetailBulkLock.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetailBulkLock/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    outBoxDetailBulkLock.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/', outBoxDetailBulkLock.selectedItem, 'POST').success(function(response) {
            outBoxDetailBulkLock.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBoxDetailBulkLock.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/', outBoxDetailBulkLock.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
            rashaErManage.checkAction(response);
            outBoxDetailBulkLock.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/News/outBoxDetailBulkLock/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    outBoxDetailBulkLock.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/', outBoxDetailBulkLock.selectedItem, "PUT").success(function(response) {
            outBoxDetailBulkLock.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBoxDetailBulkLock.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/', outBoxDetailBulkLock.gridOptions.selectedRow.item.Id, 'GET').success(function(response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath + 'smsLogoutBoxDetailBulkLock/', outBoxDetailBulkLock.selectedItemForDelete.Id, 'DELETE').success(function(res) {
