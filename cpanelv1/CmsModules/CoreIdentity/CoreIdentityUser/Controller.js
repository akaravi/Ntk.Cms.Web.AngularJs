app.controller("coreIdentityUserController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', "$rootScope", '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $rootScope, $filter) {
    ...
    coreIdentityUser.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"CoreIdentityUser/getall", coreIdentityUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            coreIdentityUser.gridOptions.fillData(coreIdentityUser.ListItems, response.Access);
    ...
    coreIdentityUser.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'CoreIdentityUser/ViewModel', '', 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/CoreIdentity/CoreIdentityUser/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    coreIdentityUser.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'CoreIdentityUser/', coreIdentityUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            coreIdentityUser.selectedItem = response.Item;
            ...
            coreIdentityUser.filePickerFiles.fileId = null;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/CoreIdentity/CoreIdentityUser/edit.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
...
//#help//
    ...
    coreIdentityUser.openUploadModal = function () {
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Biography/CoreIdentityUser/upload_modal.html',
            size: 'lg',
            scope: $scope
        });
            ...
            ajax
              ...
              .error(function(data) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/ViewModel", "", 'GET').success(function (response) {
                    ...
                    coreIdentityUser.FileItem.UploadFileGUID = uploadFile.errorExceptionResult.Item.FileKey;
...
//#help//
    ...
    coreIdentityUser.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/CoreIdentity/CoreIdentityUser/report.html',
            scope: $scope
        });
    }
