app.controller("chartShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    chartShareServerCategory.EditCategoryModel = function () {
        ...
        chartShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            chartShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            chartShareServerCategory.selectedItem = response.Item;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        chartShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    chartShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareServerCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            chartShareServerCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/chart/chartShareServerCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    chartShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareServerCategory/', chartShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            chartShareServerCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/chart/chartShareServerCategory/edit.html',
                scope: $scope
            });
    ...
    chartShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                chartShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"chartShareServerCategory/", chartShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    chartShareServerCategory.showbusy = false;
                    chartShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    chartShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(chartShareServerCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"chartShareServerCategory/", chartShareServerCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        chartShareServerCategory.categoryBusyIndicator.isActive = false;
                        chartShareServerCategory.treeConfig.showbusy = false;
                        chartShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    chartShareServerCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Chart/chartShareServerCategory/report.html',
            scope: $scope
        });
    }
