app.controller("chartContentTagCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    chartContentTag.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ChartContentTag/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Chart/ChartContentTag/add.html',
                scope: $scope
            });
    ...
    chartContentTag.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ChartContentTag/',  chartContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            ...
            chartContentTag.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Chart/ChartContentTag/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ChartContentTag/',  chartContentTag.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ChartContentTag/',  chartContentTag.selectedItemForDelete.Id , 'DELETE').success(function (res) {
    ...
    chartContentTag.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Chart/ChartContentTag/report.html',
            scope: $scope
        });
    }
