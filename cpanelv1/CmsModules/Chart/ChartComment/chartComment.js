app.controller("chartCommentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    chartComment.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"chartComment/getall", chartComment.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            chartComment.gridOptions.fillData(chartComment.ListItems, response.Access);
    ...
    chartComment.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'chartComment/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            chartComment.busyIndicator.isActive = false;
            chartComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Chart/ChartComment/add.html',
                scope: $scope
            });
    ...
    chartComment.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'chartComment/', chartComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            chartComment.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/Chart/ChartComment/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'chartComment/', chartComment.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'chartComment/', chartComment.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    chartComment.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Chart/ChartComment/report.html',
            scope: $scope
        });
    }
