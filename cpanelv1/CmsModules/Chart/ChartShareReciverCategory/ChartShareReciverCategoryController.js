app.controller("chartShareReciverCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    chartShareReciverCategory.addNewCategoryModel = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/ViewModel', "", 'GET').success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Chart/ChartShareMainAdminSetting/add.html',
                        scope: $scope
                    });
                    chartShareReciverCategory.addRequested = false;
                }).error(function (data, errCode, c, d) {
    ...
    chartShareReciverCategory.EditCategoryModel = function () {
        ...
        chartShareReciverCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareReciverCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            chartShareReciverCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            chartShareReciverCategory.selectedItem = response.Item;
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"FileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(cmsServerConfig.configApiServerPath+"FileContent/GetFilesInCategoryId/","", 'GET').success(function (response2) { //Get files in root
                    ...
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/Chart/ChartShareMainAdminSetting/edit.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    console.log(data);
    ...
    chartShareReciverCategory.addNewCategory = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareReciverCategory.selectedItem, 'POST').success(function (response) {
            chartShareReciverCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    chartShareReciverCategory.EditCategory = function (frm) {
        ...
        chartShareReciverCategory.categoryBusyIndicator.isActive = true;
        chartShareReciverCategory.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareReciverCategory.selectedItem, "PUT").success(function (response) {
    ...
    chartShareReciverCategory.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    chartShareReciverCategory.selectedItemForDelete = response.Item;
                    console.log(chartShareReciverCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+'ChartShareMainAdminSetting/', chartShareReciverCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        chartShareReciverCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    chartShareReciverCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareReciverCategory/ViewModel', "", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            chartShareReciverCategory.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/chart/chartShareReciverCategory/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    chartShareReciverCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareReciverCategory/', chartShareReciverCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
            chartShareReciverCategory.selectedItem = response1.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/chart/chartShareReciverCategory/edit.html',
                scope: $scope
            });
    ...
    chartShareReciverCategory.addNewContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareReciverCategory/', chartShareReciverCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            chartShareReciverCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    chartShareReciverCategory.editContent = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'chartShareReciverCategory/', chartShareReciverCategory.selectedItem, "PUT").success(function (response) {
            chartShareReciverCategory.categoryBusyIndicator.isActive = false;
            chartShareReciverCategory.addRequested = false;
            chartShareReciverCategory.treeConfig.showbusy = false;
    ...
    chartShareReciverCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                chartShareReciverCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(cmsServerConfig.configApiServerPath+"chartShareReciverCategory/", chartShareReciverCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    chartShareReciverCategory.showbusy = false;
                    chartShareReciverCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    chartShareReciverCategory.selectedItemForDelete = response.Item;
                    console.log(chartShareReciverCategory.selectedItemForDelete);
                    ajax.call(cmsServerConfig.configApiServerPath+"chartShareReciverCategory/", chartShareReciverCategory.selectedItemForDelete.Id, 'DELETE').success(function (res) {
                        chartShareReciverCategory.categoryBusyIndicator.isActive = false;
                        chartShareReciverCategory.treeConfig.showbusy = false;
                        chartShareReciverCategory.showIsBusy = false;
    ...
    chartShareReciverCategory.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/Chart/chartShareReciverCategory/report.html',
            scope: $scope
        });
    }
