app.controller("UploadedFilesController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    uploadedFiles.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ApiTelegramUploadedFiles/getall", engine, 'POST').success(function (response) {
            ...
            uploadedFiles.gridOptions.fillData(uploadedFiles.ListItems, response.Access);
    ...
    uploadedFiles.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramUploadedFiles/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            uploadedFiles.busyIndicator.isActive = false;
            uploadedFiles.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramUploadedFiles/add.html',
                scope: $scope
            });
    ...
    uploadedFiles.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramUploadedFiles/', uploadedFiles.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            uploadedFiles.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramUploadedFiles/edit.html',
                scope: $scope
            });
    ...
    uploadedFiles.openSendMessageToSender = function (item) {
        ...
        uploadedFiles.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramUploadedFiles/', item.Id, 'GET').success(function (response) {
            uploadedFiles.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramUploadedFiles/sendMessageModal.html',
                scope: $scope
            });
    ...
    uploadedFiles.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramUploadedFiles/report.html',
            scope: $scope
        });
    }
