app.controller("memberInfoController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
 ...
 //#help/ سلکتور  عضو 
    ...
    memberInfo.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegrammemberInfo/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            memberInfo.busyIndicator.isActive = false;
            memberInfo.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegrammemberInfo/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    memberInfo.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegrammemberInfo/', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            memberInfo.selectedItem = response.Item;
            if (memberInfo
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegrammemberInfo/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegrammemberInfo/', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegrammemberInfo/', memberInfo.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    memberInfo.openInfoModal = function (selectedId, memberId, botConfigId, chatId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ApiTelegramMemberInfo/", selectedId, "GET").success(function (response) {
            ...
            ajax.call(cmsServerConfig.configApiServerPath+"MemberUser/", memberId, "GET").success(function (response) {
                ...
                memberInfo.busyIndicator.isActive = false;
                $modal.open({
                    templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramMemberInfo/memberInfo.html',
                    scope: $scope
                });
            }).error(function (data, errCode, c, d) {
                rashaErManage.checkAction(data, errCode);
    ...
    memberInfo.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramMemberInfo/report.html',
            scope: $scope
        });
    }
