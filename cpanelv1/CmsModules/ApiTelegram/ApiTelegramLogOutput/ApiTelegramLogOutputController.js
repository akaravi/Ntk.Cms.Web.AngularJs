app.controller("logOutputGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    logOutputCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogOutput/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogOutput/add.html',
                scope: $scope
            });
    ...
    logOutputCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogInput/', logOutputCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            logOutputCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogOutput/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogOutput/', logOutputCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogOutput/', logOutputCtrl.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    logOutputCtrl.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogOutput/report.html',
            scope: $scope
        });
    }
