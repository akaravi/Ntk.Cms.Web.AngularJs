app.controller("botConfigGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $rootScope, $filter) {
    ...
    botConfigCtrl.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"apitelegrambotconfig/getall", botConfigCtrl.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            botConfigCtrl.gridOptions.fillData(botConfigCtrl.ListItems, response.Access);
    ...
    botConfigCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramBotConfig/add.html',
                scope: $scope
            });
    ...
    botConfigCtrl.openEditModal = function () {
        ...
        botConfigCtrl.addRequested = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/', botConfigCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            botConfigCtrl.addRequested = false;
            rashaErManage.checkAction(response);
            botConfigCtrl.selectedItem = response.Item;
            if (botConfigCtrl.LinkUniversalMenuIdOnUndetectableKey > 0)
                botConfigCtrl.selectUniversalMenuOnUndetectableKey = true;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramBotConfig/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/', botConfigCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/', botConfigCtrl.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    botConfigCtrl.openGetInfoModal = function (selectedIndex, selectedId) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/getmeasync', selectedId, 'GET').success(function (response1) {
            ...
            if (response1.IsSuccess) {
                ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/', selectedId, 'GET').success(function (response2) {
                    rashaErManage.checkAction(response2);
                    botConfigCtrl.selectedBotConfig = response2.Item;
                    $modal.open({
                        templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramBotConfig/getInfo.html',
                        scope: $scope
                    });
    ...
    botConfigCtrl.qrCode = function () {
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramBotConfig/', 1, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            botConfigCtrl.selectedItem = response.Item;
    ...
    botConfigCtrl.openSendToAllModal = function (selectedIndex, selectedId) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramBotConfig/sendToAllModal.html',
            scope: $scope
        });
    }
    ...
    botConfigCtrl.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramBotConfig/report.html',
            scope: $scope
        });
    }
