app.controller("logInputGridController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    logInputCtrl.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+"ApiTelegramLogInput/getall", logInputCtrl.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            logInputCtrl.gridOptions.fillData(logInputCtrl.ListItems, response.Access);
...
logInputCtrl.treeOptions = {
    ...
    logInputCtrl.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogInput/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            logInputCtrl.busyIndicator.isActive = false;
            logInputCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogInput/add.html',
                scope: $scope
            });
    ...
    logInputCtrl.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogInput/', logInputCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            logInputCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogInput/edit.html',
                scope: $scope
            });
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'apitelegramloginput/', logInputCtrl.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogInput/', logInputCtrl.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    logInputCtrl.openSendMessageToSender = function (item) {
        ...
        logInputCtrl.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramLogInput/', item.Id, 'GET').success(function (response) {
            logInputCtrl.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogInput/sendMessageModal.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    logInputCtrl.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramLogInput/report.html',
            scope: $scope
        });
    }
