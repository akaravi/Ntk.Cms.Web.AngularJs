app.controller("ReceivedFilesController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    receivedFiles.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramreceivedFiles/ViewModel', "", 'GET').success(function (response) {
            ...
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramReceivedFiles/add.html',
                scope: $scope
            });
    ...
    receivedFiles.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramreceivedFiles/', receivedFiles.selectedItem, 'POST').success(function (response) {
            receivedFiles.addRequested = false;
            receivedFiles.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    receivedFiles.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramreceivedFiles/', receivedFiles.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            receivedFiles.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramReceivedFiles/edit.html',
                scope: $scope
            });
    ...
    receivedFiles.editRow = function (frm) {
        ...
        receivedFiles.busyIndicator.isActive = true;
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramReceivedFiles/', receivedFiles.selectedItem, "PUT").success(function (response) {
            receivedFiles.addRequested = true;
            rashaErManage.checkAction(response);
            receivedFiles.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramReceivedFiles/', receivedFiles.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramReceivedFiles/', receivedFiles.selectedItemForDelete.Id, 'DELETE').success(function (res) {
    ...
    receivedFiles.openSendMessageToSender = function (item) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'ApiTelegramReceivedFiles/', item.Id, 'GET').success(function (response) {
            receivedFiles.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramReceivedFiles/sendMessageModal.html',
                scope: $scope
            });
    ...
    receivedFiles.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/CmsModules/ApiTelegram/ApiTelegramReceivedFiles/report.html',
            scope: $scope
        });
    }
