...
var fileMangerList = function (id, extension) {
    var ret = '<ul class="jqueryFileTree">';
    var Filter_value = {
        PropertyName: "LinkParentId",
        IntValue1: parseInt(id),
        SearchType: 0,
        IntValueForceNullSearch: true
    };
    var FilterModel = {};
    FilterModel.RowPerPage = 500;
    ...
    $.ajax({
        ...
        success: function (request) {
            $.each(request.ListItems, function (index, element) {
                ret += '<li class="directory collapsed"><a rel="' + element.Id + '">' + element.Title + '</a></li>';
            });
        },
        error: function (msg) {
            console.log(msg);
    ...
    $.ajax({
        ...
        success: function (request2) {
            $.each(request2.ListItems, function (index, element) {
                if (!(extension == undefined || extension.length == 0 || extension.indexOf(element.Extension.toLowerCase()) >= 0))
                    ret += '<li class="file ext_' + element.Extension.toLowerCase() + '"><a rel="' + element.Id + '">' + element.FileName + '</a></li>';
            });
            ret = ret + '</ul>';
        },
        error: function (msg) {
...
var angularJsTreefileMangerList = function () {
    var ret = {};
    ret.list = [];
    ret.fileList = [];
    var FilterModel = {};
    ...
    function fileTreeMakeCategory(valList, fileListItems, categoryParent) {
        ...
        $.each(valList, function (index1, category) {
            if (categoryParent == category.LinkParentId) {
                ...
                category.Children = category.Children.concat(fileTreeMakeFile(fileListItems, category.Id));
                ret.push(category);
            }
        });
        return ret;
    ...
    function fileTreeMakeFile(valList, categoryId) {
        ...
        if (!findChild)
            ret.push({
                FileName: 'No file in this folder'
            });
        return ret;
    }
    ...
    angular.module('builder.controller', ['builder.provider']).controller('fbFormObjectEditableController', [
        ...
        function ($scope, $injector) {
            ...
            $scope.setupScope = function (formObject) {
                ...
                $scope.$watch('[label,fieldname, description, placeholder, required, options, validation, multiple, minLength, maxLength, dateRangeStart, dateRangeEnd, disableWeekends, maxDate, requireConfirmation, readOnly, minRange, maxRange, nextXDays, performCreditCheck, creditCheckRequired, performMeterLookup, cprCountry, logic, category, conversionType, searchScope, searchPeriod, transformations, uploaderType]', function () {
                    ...
                    formObject.LinkModuleId = $scope.LinkModuleId; // LinkModuleId
                    formObject.LinkContentId = $scope.LinkContentId; //LinkContentId
            ...
            return $scope.data = {
                ...
                rollback: function () {
                    ...
                    $scope.creditCheckRequired = this.model.creditCheckRequired;
        ...
        function ($scope, $injector) {
            ...
            $.ajaxSetup({
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('Authorization', userglobaltoken);
                }
            });
            $.ajax({
                type: "POST",
                //async: false,
                url: cmsServerConfig.configApiServerPath +"CoreModule/GetAll/",
                contentType: "application/json",
                success: function (msg) {
                    if (msg == undefined || msg.Item == undefined || msg.Item.PageJsonValue == undefined) {
                        //console.log('اطلاعاتی یافت نشد');
                    } else {
                        myJson = JSON.parse(msg.Item.PageJsonValue);
                        loadPage(myJson);
                    }
                ...
                error: function (msg) {
                    console.log(msg);
                    $('.mfn-wrapper-load-alert').html(msg);
                }
            });
            $builder = $injector.get('$builder');
...
(function () {
    angular.module('builder.directive', ['builder.provider', 'builder.controller', 'builder.drag', 'validator'])
        .directive('richText',
        [
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        extendPastWeekend = function (from, daysOffset) {
                            ...
                            count = 0;
                            for (i = _i = 0;
                                0 <= daysOffset ? _i <= daysOffset : _i >= daysOffset;
                                i = 0 <= daysOffset ? ++_i : --_i) {
                            ...
                            up: function (e, isHover, draggable) {
                                ...
                                if (!isHover && draggable.mode === 'drag') {
                                    ...
                                    if (draggable.mode === 'mirror') {
                                        $builder.insertFormObject(scope.formName,
                                            $(element).find('.empty').index('.fb-form-object-editable'),
                                            {
            ...
            '$injector', function ($injector) {
                ...
                return {
                    ...
                    link: function (scope, element) {
                        ...
                        scope.$watch('$component.template',
                            function (template) {
                                var view;
                                if (!template) {
                                    return;
                        ...
                        popover = {};
                        scope.$watch('$component.popoverTemplate',
                            function (template) {
                                if (!template) {
                                    return;
                                }
            ...
            function () {
                ...
                return {
                    ...
                    link: function (scope, element, attrs, ngModelCtrl) {
                        ...
                        scope.selectedNodes = [];
                        scope.expandedNodes = [];
                        var retData = angularJsTreefileMangerList();
                        scope.dataForTheTree = retData.list;
                        ...
                        function nodeIdsAnalisys() {
                            if (scope.inputText != undefined && scope.inputText.length > 0) {
                                var nodeIds = [];
                                nodeIds = scope.inputText.split(',');

                                $.each(retData.fileList,
                                    function (index, file) {
                                        if (nodeIds.indexOf(file.Id + '') > -1 &&
                                            file.FileName != undefined &&
                                            file.FileName.length > 0) {
                                            scope.selectedNodes.push(file);
                        ...
                        scope.showSelected = function (node, selected) {
                            ...
                            scope.selectedNodes = scope.selectedNodes.filter(function (item, index, inputArray) {
                                return inputArray.indexOf(item) == index;
                            });


                            $.each(scope.selectedNodes,
                                function (index, node) {
                                    if (scope.inputText.length > 0) scope.inputText = scope.inputText + ',';
                                    scope.inputText = scope.inputText + '' + node.Id;
                                });
                            if (scope.inputText != undefined && scope.inputText.length > 0) {

                                var arr = scope.inputText.split(',')
                                    .filter(function (item, index, inputArray) {
                                        return inputArray.indexOf(item) == index;
                                    });
                                scope.inputText = arr.join();
                            }
                        }
            ...
            '$injector', function ($injector) {
                ...
                return {
                    ...
                    link: function (scope, element) {
                        ...
                        return scope.$watch('component.template',
                            function (template) {
                                var view;
                                if (!template) {
                                    return;
            ...
            '$injector', function ($injector) {
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                                ...
                                function (newValue, oldValue) {
                                    ...
                                    for (index in scope.inputArray) {
                                        if (scope.inputArray[index]) {
                                            if (index !== 'diff') {
                                                checked
                                                    .push((_ref = scope.options[index]) != null
                                                        ? _ref
                                                        : scope.inputArray[index]);
                                            }
                                        }
                                    }
                        ...
                        return scope.$watch("default['" + scope.formObject.id + "']",
                            function (value) {
                                if (!value) {
                                    return;
                                }
        ...
        .directive('process',
            function ($http) {
                return {
                    template:
                        '<select ng-model="moduleListSelect" ng-change="moduleListSelectFun(moduleListSelect)"><option ng-repeat="c in moduleList" value="{{c.value}}" >{{c.name}}</option></select><br/>' +
                            '<select ng-model="processListSelect" ng-change="processListSelectFun(processListSelect)"><option ng-repeat="c in processList" value="{{c.value}}" >{{c.name}}</option></select><br/>' +
                            '<select ng-model="processCustomizeListSelect" ng-change="processCustomizeListSelectFun(processCustomizeListSelect)"><option ng-repeat="c in processCustomizeList" value="{{c.value}}" >{{c.name}}</option></select><br/>' +
                            '<select ng-model="inputText" ><option ng-repeat="c in universalmenuProcessesList" value="{{c.value}}" >{{c.name}}</option></select>',
                    restrict: 'A',
                    link: function (scope, elem, attrs) {
                        scope.moduleList = [];
                        scope.processList = [];
                        scope.processCustomizeList = [];
                        scope.universalmenuProcessesList = [];


                        scope.moduleList.push({ name: 'انتخاب کنید', value: '0' });
                        $http({
                            'method': 'POST',
                            'url': cmsServerConfig.configApiServerPath +'CoreModule/getall',
                            'data': {},
                            ...
                            .success(function (response) {
                                ...
                                angular.forEach(response.ListItems,
                                    function (val, key) {
                        ...
                        scope.moduleListSelectFun = function (newValue) {
                            ...
                            engine.Filters = null;
                            engine.Filters = [];
                            engine.Filters.push(Filter_value);

                            if (angular.isDefined(newValue)) {
                                $http({
                                    'method': 'POST',
                                    'url': cmsServerConfig.configApiServerPath +'CoreModuleProcess/getall',
                                    'data': engine,
                                    'async': false,
                                    'headers': {
                                        'Content-Type': 'application/json',
                        ...
                        scope.processListSelectFun = function (newValue) {
                            if (newValue == 0) return;
                            scope.processCustomizeList = [];
                            scope.universalmenuProcessesList = [];
                            var Filter_value = {
                            ...
                            if (angular.isDefined(newValue)) {
                                $http({
                                    'method': 'POST',
                                    'url': cmsServerConfig.configApiServerPath +'CoreModuleProcessCustomize/getall',
                                    'data': engine,
                                    'async': false,
                                    'headers': {
                                        'Content-Type': 'application/json',
                                    ...
                                    .success(function (response) {
                                        angular.forEach(response.ListItems,
                                            function (val, key) {
                                                scope.processCustomizeList.push({
                                                    name: val.Title,
                                                    value: val.Id
                                                });
                                            });
                        ...
                        scope.processCustomizeListSelectFun = function (newValue) {
                            if (newValue == 0) return;
                            scope.universalmenuProcessesList = [];
                            var Filter_value = {
                                PropertyName: "LinkModuleProcessCustomizeId",
                                IntValue1: parseInt(newValue),
                                SearchType: 0
                            }
                            ...
                            if (angular.isDefined(newValue)) {
                                $http({
                                    'method': 'POST',
                                    'url': cmsServerConfig.configApiServerPath +'UniversalMenuProcesses/getall',
                                    'data': engine,
                                    'async': false,
                                    'headers': {
                                        'Content-Type': 'application/json',
                                    ...
                                    .success(function (response) {
                                        angular.forEach(response.ListItems,
                                            function (val, key) {
                                                scope.universalmenuProcessesList.push({
                                                    name: val.Title,
                                                    value: val.Id
                                                });
                                            });
                        ...
                        //scope.universalmenuProcessesFun = function (newValue) {
            ...
            //    return {
            //        template: '<select ng-model="content" ng-options="c.name for c in contents"></select>',
            //        restrict: 'E',
            //        scope: {
            //            content: '@'
            //        },
            ...
            //            scope.$root.$watch('category', function (newValue, oldValue) {
            //                scope.contents = [];
            ...
            //                     if ((newValue !== oldValue) && angular.isDefined(newValue.value)) {
            //                         //console.log(newValue, oldValue, "----------------");

            //                         scope.inputText = newValue.value;
            //                         //return
            //                     }
            //                 }, true);
            ...
            //})
                     ...
                     .directive('contentType', function () {
                         return {
                             template: '<select ng-model="ssss" ng-options="c.name for c in items"></select>',
                             restrict: 'E',
                     ...
                     .directive('category', function ($http) {
                         return {
                             template: '<select ng-model="category" ng-options="c.name for c in items"></select>',
                             restrict: "E",
                             scope: {
                                 category: '@'
                             },
                             ...
                             link: function (scope) {
                                 scope.items = [];
                                 scope.items.push({ name: 'انتخاب کنید', value: '0' });
                                 ...
                                 scope.$watch('category',
                                        function (newValue, oldValue) {
                                            if ((newValue !== oldValue) && angular.isDefined(newValue.value)) {
                                                scope.$root.category = newValue.value;
                                            }
                                        }, true);
                             },
                         };
    ...
    angular.module('builder.provider', []).provider('$builder', function () {
        ...
        this.removeAllFormObject = (function (_this) {
            return function (name) {
                ...
                formObjects = _this.forms[name];
                //for (index = _i = 0, _ref = formObjects.length; _i < _ref; index = _i += 1) {
                if (formObjects != undefined && formObjects.length > 0)
