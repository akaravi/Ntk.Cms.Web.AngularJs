app.controller("apiPathSettingsCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$rootScope', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $rootScope, $state, $filter) {
    ...
    apiSetting.init = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "SmsMainApiPathCompany/getall", "", 'POST').success(function(response) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + "SmsMainApiPath/getall", "", 'POST').success(function(response) {
            rashaErManage.checkAction(response);
    ...
    apiSetting.addNewRow = function () {
        ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainapipath/', apiSetting.selectedItem, 'POST').success(function(response) {
    ...
    apiSetting.editRow = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath + 'SmsMainapipath/', apiSetting.selectedItem, "PUT").success(function(response) {
            apiSetting.addRequested = false;
            rashaErManage.checkAction(response);
            apiSetting.goBack();
        }).error(function (data, errCode, c, d) {
