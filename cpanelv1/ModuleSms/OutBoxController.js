app.controller("outBoxCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBox.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBox/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    outBox.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBox/',  outBox.selectedItem , 'POST').success(function (response) {
            outBox.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBox.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBox/', outBox.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            outBox.selectedItem = response.Item;
            $modal.open({
    ...
    outBox.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBox/',  outBox.selectedItem , "PUT").success(function (response) {
            outBox.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBox.addRequested = false;
