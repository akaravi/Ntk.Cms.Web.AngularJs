app.controller("outBoxDetailBulkCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBoxDetailBulk.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetailBulk/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    outBoxDetailBulk.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetailBulk/', outBoxDetailBulk.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            outBoxDetailBulk.selectedItem = response.Item;
            $modal.open({
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetailBulk/',  outBoxDetailBulk.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetailBulk/',  outBoxDetailBulk.selectedItemForDelete.Id , 'DELETE').success(function (res) {
