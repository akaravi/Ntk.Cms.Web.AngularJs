app.controller("outBoxDetailCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    outBoxDetail.openAddModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/ViewModel', "", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    outBoxDetail.addNewRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/', outBoxDetail.selectedItem, 'POST').success(function (response) {
            outBoxDetail.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    outBoxDetail.openEditModal = function () {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/', outBoxDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            outBoxDetail.selectedItem = response.Item;
            $modal.open({
    ...
    outBoxDetail.editRow = function (frm) {
        ...
        ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/', outBoxDetail.selectedItem, "PUT").success(function (response) {
            outBoxDetail.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                outBoxDetail.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/', outBoxDetail.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(cmsServerConfig.configApiServerPath+'outBoxDetail/', outBoxDetail.selectedItemForDelete.Id, 'DELETE').success(function (res) {
